...
| B3, B6, G3, H1–H2 | — | 🟡 | With PR 6 merged this is the WHOLE of what is left, and H2 is underway. B3 is a `platform.ai_vendor_credential` repoint under PR 8, not code. G3 (regression cover for TTS / open-in-canvas / table→xlsx) belongs with PR 8's harness, which is where a regression pass can live. B6 + H1–H2 are PR 8. F1 is "good, preserve" — not a remediation item, and deliberately absent from this list. Every other finding A1–J5 has a row above, once each. |

**Recommended next PR:** PR 8 (evaluation & observability — B6, G3, H1, H2) — the last one. Seven of the eight milestones are closed and no P0 or P1 remains outside it. Its H2 half (AI health telemetry + the AI Control panel) is in progress on `claude/praxis-ai-pr8-eval`.
