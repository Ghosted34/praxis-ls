...
jest.mock("../../src/modules/costing/costing/costing.service", () => ({
  createDraft: jest.fn(async (_client, { data, actor }) => ({
    costing_id: "c-1",
    _data: data,
    _actor: actor,
  })),
...
const {
  buildCatalogue,
  buildExecutorMap,
} = require("../../src/services/ai/action-registrar");
...
describe("AI write surface — WIRING ONLY (an executor exists per catalogued write)", () => {
  it("enables a money-path set of write actions, not just one", () => {
    const enabled = buildCatalogue()
      .filter((r) => r.is_write && r.ai_enabled)
      .map((r) => r.action_key);
  ...
  it("executes an AI-proposed write through the module service (propose→confirm→record)", async () => {
    const r = await registry.create_costing({
      client: {},
      user: { user_id: "u1" },
      payload: { dossier_id: "d1", margin_percent: 20 },
    });
    expect(costing.createDraft).toHaveBeenCalledWith(
      {},
      {
        data: { dossier_id: "d1", margin_percent: 20 },
        actor: { user_id: "u1" },
      },
    );
...
describe("ingest honours the ai.vectorization toggle", () => {
  ...
  const flag = (state) => ({
    query: jest.fn().mockResolvedValue({ rows: state ? [{ state }] : [] }),
  });
  ...
  it("ingestTenantCards skips embedding when the flag is off", async () => {
    ...
    const r = await ingest.ingestTenantCards(client, [
      { ref: "dossier:X", text: "hi" },
    ]);
  ...
  it("reembedEntity is a no-op when the flag is off", async () => {
    const r = await ingest.reembedEntity(flag("off"), {
      entityRef: "dossier:X",
    });
