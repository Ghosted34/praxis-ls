...
function useActorNames() {
  ...
  return React.useMemo(() => {
    ...
    (rows || []).forEach((u) => {
      m[u.user_id] = u.full_name || u.email || u.user_id;
    });
    return m;
  }, [rows]);
}
...
function ReviewDetail({
  ...
  const detail = useResource<Review>(
    () => tenant<Review>(`/audit/reviews/${review.review_id}`),
    [review.review_id],
  );
  ...
  async function decide(entryId: string, decision: string) {
    ...
    setError(null);
    try {
      await tenant(`/audit/reviews/${review.review_id}/entries/${entryId}`, {
        method: "PATCH",
  ...
  return (
    <Modal
      ...
      size="xl"
      title={review.name}
      description="Recertify each user's access. Decisions are recorded against the live identity schema, so a review always reflects real access."
    >
      <div className="space-y-4">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div className="text-sm text-muted-foreground">
            <span className="num font-medium text-foreground">{decided}</span>{" "}
            of{" "}
            ...
            {done && (
              <Pill tone="ok" className="ml-2">
                Completed {dateFmt(detail.data?.completed_at)}
              </Pill>
            )}
          </div>
          {!done && (
            <Button
              size="sm"
              onClick={complete}
              loading={busy === "__complete"}
              disabled={!entries.length || decided < entries.length}
            >
              Complete review
            </Button>
          )}
        ...
        {!done && decided < entries.length && (
          ...
          >
            <Input
              ...
              placeholder="Reason for revoking / flagging"
            />
          </Field>
        )}
        ...
        {detail.error ? (
          ...
          <span className="micro">Loading entries…</span>
        ) : (
          <div className="max-h-96 space-y-2 overflow-auto">
            {entries.map((e) => {
              const roles = Array.isArray(e.roles_snapshot)
                ? (e.roles_snapshot as unknown[]).map(String)
                : [];
              return (
                <div
                  key={e.entry_id}
                ...
                >
                  <div className="min-w-0">
                    <div className="font-medium text-foreground">
                      {actorName[e.user_id] || shortId(e.user_id)}
                    </div>
                    <div className="micro">
      ...
      <form className="space-y-4" onSubmit={submit}>
        <Field label="Name" required hint="e.g. Q3 2026 access recertification">
          <Input
            ...
            placeholder="Q3 2026 access recertification"
          />
        </Field>
        {error && <ErrorState message={error} />}
        <div className="flex justify-end gap-2 pt-2">
          ...
          <Button type="submit" loading={busy} disabled={busy || !name}>
            Start review
          </Button>
        </div>
      </form>
...
export function AuditPage() {
  const [tab, setTab] = React.useState<
    "ledger" | "events" | "reviews" | "restores"
  >("ledger");
  const actorName = useActorNames();
  const [q, setQ] = React.useState("");

  const ledger = useList<LedgerEntry>(tab === "ledger" ? "/audit" : null);
  const events = useList<SecurityEvent>(
    tab === "events" ? "/audit/events" : null,
  );
  const reviews = useList<Review>(tab === "reviews" ? "/audit/reviews" : null);
  const softDeletes = useList<SoftDelete>(
    tab === "restores" ? "/audit/soft-deletes" : null,
  );
  ...
  const actor = (id?: string | null) =>
    id ? actorName[id] || shortId(id) : "—";

  async function softDeleteAction(
    id: string,
    kind: "request-restore" | "restore",
  ) {
    setBusy(id);
    setError(null);
    try {
  ...
  const ledgerCols: Column<LedgerEntry>[] = [
    ...
    },
    {
      ...
      label: "Module",
      render: (r) =>
        r.module_key ? <Pill tone="mute">{r.module_key}</Pill> : "—",
    },
    {
      key: "entity_ref",
      label: "Entity",
      render: (r) => (
        <span className="num text-muted-foreground">{r.entity_ref || "—"}</span>
      ),
    },
    {
      key: "actor_user_id",
      label: "Actor",
      render: (r) => actor(r.actor_user_id),
    },
    {
      key: "created_at",
      label: "When",
  ...
  const reviewCols: Column<Review>[] = [
    ...
    },
    {
      ...
      render: (r) => (
        <RowActions>
          <Button size="sm" variant="outline" onClick={() => setOpenReview(r)}>
            Open
          </Button>
        </RowActions>
      ),
  ...
  const restoreCols: Column<SoftDelete>[] = [
    ...
    },
    {
      key: "deleted_at",
      label: "Deleted",
      render: (r) => <span className="num">{dateFmt(r.deleted_at)}</span>,
    },
    {
      key: "state",
      label: "State",
      render: (r) =>
        r.restore_requested_by ? (
          <Pill tone="warn">Restore requested</Pill>
        ) : (
          <Pill tone="mute">Soft-deleted</Pill>
        ),
    },
    {
      key: "_a",
        ...
        <RowActions>
          {!r.restore_requested_by && (
            <Button
              size="sm"
              variant="outline"
              disabled={busy === r.soft_delete_id}
              onClick={() =>
                softDeleteAction(r.soft_delete_id, "request-restore")
              }
            >
              Request restore
            </Button>
          ...
          <Button
            ...
            variant="outline"
            disabled={busy === r.soft_delete_id}
            onClick={() => softDeleteAction(r.soft_delete_id, "restore")}
          >
            Restore
          </Button>
        </RowActions>
  ...
  const ledgerRows = (ledger.rows || []).filter((r) =>
    hit(`${r.action} ${r.entity_ref || ""} ${r.module_key || ""}`),
  );
  const eventRows = (events.rows || []).filter((r) =>
    hit(`${r.event_type_key} ${r.entity_ref || ""} ${r.module_key || ""}`),
  );
  const highEvents = (events.rows || []).filter(
  ...
  return (
    <section className={shell}>
      <PageHeader
        ...
        action={
          tab === "reviews" ? (
            <Button onClick={() => setNewReview(true)}>New review</Button>
          ) : undefined
        }
      />
      <Segmented
        ...
        <>
          <div className="mb-3 flex justify-end">
            <Input
              ...
              placeholder="Search action, entity or module…"
              className="w-full max-w-xs"
            />
          </div>
          <DataList
            columns={ledgerCols}
            rows={ledger.loading ? null : ledgerRows}
            error={ledger.error}
      ...
      {tab === "events" && (
        <>
          ...
          <DataList
            columns={eventCols}
            rows={events.loading ? null : eventRows}
            error={events.error}
      ...
      {tab === "restores" && (
        <>
          <div className="mb-4 rounded-xl border border-[rgb(var(--warn))]/40 bg-[rgb(var(--warn)/0.08)] px-4 py-3 text-sm">
            Restore is maker-checker: the person who deleted a record cannot be
            the one who restores it, and the database enforces that.
          </div>
          <DataList
            columns={restoreCols}
            rows={softDeletes.rows}
            error={softDeletes.error}
            loading={softDeletes.loading}
