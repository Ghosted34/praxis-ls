...
const { nextAuxCode, mirrorCodeFor, chooseMirrorCode } = require("../../src/modules/master/party-accounting.service");
...
describe("chooseMirrorCode — collision-safe allocation (PR3 §12)", () => {
  it("keeps the suffix-shared code when it is free", () => {
    expect(chooseMirrorCode({ auxCode: "41110007", mirrorParent: "4191", suffixTaken: false, maxMirrorChild: null })).toBe("41910007");
  });
  ...
  it("when the suffix collides, allocates the next child from the mirror parent's OWN sequence", () => {
    ...
    expect(chooseMirrorCode({ auxCode: "41120007", mirrorParent: "4191", suffixTaken: true, maxMirrorChild: "41910007" })).toBe("41910008");
  });

  it("falls back cleanly when the mirror parent already has unrelated children", () => {
    expect(chooseMirrorCode({ auxCode: "40110003", mirrorParent: "4091", suffixTaken: true, maxMirrorChild: "40910042" })).toBe("40910043");
    // ...and starts the sequence at 0001 if somehow taken with no children yet.
    expect(chooseMirrorCode({ auxCode: "40110003", mirrorParent: "4091", suffixTaken: true, maxMirrorChild: null })).toBe("40910001");
  });
});
