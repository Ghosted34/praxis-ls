...
function WorkOrderDetail({
  ...
  const parts = useResource(
    () => api.getWorkOrderParts(order.work_order_id),
    [order.work_order_id],
  );
  ...
  async function toState(s: string) {
    ...
    setError(null);
    try {
      setOrder(await api.setWorkOrderStatus(order.work_order_id, s));
      onChanged();
    } catch (e) {
      setError(errMsg(e));
    } finally {
  ...
  async function addPart(e: React.FormEvent) {
    ...
    setError(null);
    try {
      const updated = await api.addWorkOrderPart(order.work_order_id, {
        label: p.label,
        qty: Number(p.qty) || 1,
        unit_cost: Number(p.unit_cost) || 0,
      });
      setOrder((o) => ({ ...o, cost: updated.cost }));
      setP({ label: "", qty: "1", unit_cost: "" });
      parts.reload();
      onChanged();
    } catch (e) {
      setError(errMsg(e));
  ...
  return (
    <Modal
      ...
      onClose={onClose}
      size="xl"
      title={`${enumLabel(order.kind)} · ${order.registration || order.work_order_id.slice(0, 8)}`}
      description={order.description || undefined}
    >
      <div className="space-y-5">
        <StepBar
          steps={["OPEN", "IN_PROGRESS", "DONE"]}
        ...
        >
          ...
          <DocButton
            ...
            title={`Work order ${order.registration || order.work_order_id.slice(0, 8)}`}
            label="View document"
          />
        </StatusActionBar>
        ...
        <LineTable
          ...
          columns={[
            { label: "Part / labour", render: (r) => r.label },
            {
              label: "Qty",
              align: "right",
              render: (r) => <span className="num">{num(r.qty)}</span>,
            },
            {
              label: "Unit cost",
              align: "right",
              render: (r) => <span className="num">{money(r.unit_cost)}</span>,
            },
            {
              label: "Line total",
              align: "right",
              render: (r) => (
                <span className="num">
                  {money(Number(r.qty) * Number(r.unit_cost))}
                </span>
              ),
            },
          ]}
          ...
          >
            ...
            <Field label={tr("Unit cost")}>
              <Input
                ...
                value={p.unit_cost}
                onChange={(e) =>
                  setP((s) => ({ ...s, unit_cost: e.target.value }))
                }
              />
            </Field>
            <Button type="submit" loading={busy === "add"} disabled={!p.label}>
              Add
            </Button>
          </form>
        )}
...
export function WorkOrdersPage() {
  ...
  const cols: Column<api.WorkOrder>[] = [
    ...
    },
    {
      ...
      label: "Kind",
      render: (o) => (
        <span className="text-muted-foreground">{enumLabel(o.kind)}</span>
      ),
    },
    {
      key: "desc",
    ...
    },
    {
      ...
      label: "Status",
      render: (o) => (
        <Pill tone={STATUS_TONE[o.status] || "mute"}>
          {enumLabel(o.status)}
        </Pill>
      ),
    },
    {
      ...
      render: (o) => <span className="num">{money(o.cost)}</span>,
    },
    {
      key: "opened",
      label: "Opened",
  ...
  return (
    <section className={shell}>
      ...
      <DataList
        ...
        rowKey={(o) => o.work_order_id}
        empty={{
          title: "No work orders",
          hint: "Open one to start logging maintenance.",
        }}
      />
      {view && (
        <WorkOrderDetail
