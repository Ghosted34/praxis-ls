...
function attachFinancialDictionaryAutocomplete(inputEl, opts) {
  ...
  const doSearch = debounce(async () => {
    ...
    try {
      ...
      data = await apiGet(
        `../../api/finance/financial_dictionary/search.php?q=${encodeURIComponent(q)}&lang=${encodeURIComponent(lang)}`,
      );
    } catch (e) {
    ...
    box.innerHTML = items
      .map((it) => {
        const name =
          opts.getLang?.() === "FR"
            ? it.name_fr || it.name_en
            : it.name_en || it.name_fr;
        const safeName = String(name || "").replace(
          /[<>&]/g,
          (s) => ({ "<": "&lt;", ">": "&gt;", "&": "&amp;" })[s],
        );
        const safeCode = String(it.code || "").replace(
          /[<>&]/g,
          (s) => ({ "<": "&lt;", ">": "&gt;", "&": "&amp;" })[s],
        );
