 ...
 * whichever row Postgres handed back first (the old `LIMIT 1`). Migration 13950
 * repairs legacy drift and adds a partial unique index so two bases cannot exist
 * going forward, but a tenant that has not yet taken 13950 could still be in the
  ...
  if (rows.length > 1) {
    throw new AppError(
      ...
      "More than one base currency is flagged (" +
        ...
        "). Apply migration 13950 or set the base again to repair before FX can resolve.",
      500,
    );
  }
