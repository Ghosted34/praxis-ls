...
- **Delivery notes: whole-row click opens the snapshot modal, edit-while-DRAFT confirmed, and the document page shows the real note.** The delivery-notes list now opens its detail modal on a click anywhere in the row (the same `onRowClick` gesture as the transit-orders list, with the ref cell as the keyboard/AT activator), matching the transit-order pattern. The in-app document page (`/documents/DELIVERY_NOTE/:id`, and every other doc type) now renders the paper template itself — the exact HTML the `/document-templates/:docType/preview` endpoint feeds to the PDF generator — instead of a generic FROM/DATE/ROUTE/ITEMS card body that had nothing to do with the printed document. The delivery note template was already the real thing (head block, consignee, cargo table, container manifest padded to 18, reservations box, received-by block), so the generic-view fallback in `document-view.tsx` was the only drift source; it is deleted. Download PDF is unchanged (vaulted, QR-verifiable). Edit-while-DRAFT was already fully wired (detail modal Edit button gated on `DRAFT` → `PATCH /delivery-notes/:id` with header + lines + containers; `rules.EDITABLE` is exactly `{DRAFT}`) and is verified end-to-end by the delivery-note lifecycle tests.
