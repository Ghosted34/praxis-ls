...
jest.mock("../../src/modules/portal/portal.repo", () => ({
  auditLedger: jest.fn(async () => [{ ledger_id: 1, action: "invoice.posted", actor_name: "Jane" }]),
}));
...
describe("investorView KPIs (OHADA basis — PRD Q4 resolved)", () => {
  test("derives net margin + expense ratio from the statements", async () => {
    const out = await portal.investorView({}, { params: { from: "2026-01-01", to: "2026-06-30" } });
  ...
  test("margins are null (not 0 or Infinity) when revenue is zero", async () => {
    ...
    report.run.mockImplementation(async (c, args) =>
      args.reportKey === "income_statement" ? { data: { produits: 0, charges: 0, result: 0 } } : orig(c, args),
    );
...
describe("auditorView (curated, period-scoped)", () => {
  ...
  test("composes statements + procurement + a named audit trail, OHADA basis", async () => {
    const out = await portal.auditorView({}, { params: { from: "2026-01-01", to: "2026-12-31", entity_id: "e1" } });
    ...
    expect(out.audit_trail).toEqual([{ ledger_id: 1, action: "invoice.posted", actor_name: "Jane" }]);
  ...
  test("queries the ledger with a finance/document allow-list, never HR/security", async () => {
    await portal.auditorView({}, { params: { from: "2026-01-01", to: "2026-12-31" } });
    const arg = repo.auditLedger.mock.calls[0][1];
    expect(arg.from).toBe("2026-01-01");
    expect(arg.to).toBe("2026-12-31");
    expect(arg.prefixes).toEqual(expect.arrayContaining(["invoice", "journal", "document", "costing"]));
    for (const banned of ["permission", "role", "godmode", "payroll", "employee", "user", "session"]) {
