...
function LogFillForm({ vehicles, presetVehicle, onClose, onSaved }: {
  ...
  const [f, setF] = React.useState({ vehicle_id: presetVehicle, odometer: "", litres: "", cost: "", dossier_id: "" });
  ...
  return (
    <Modal open onClose={onClose} title="Log fuel fill" description="Record a fill. The odometer can't go below the last recorded reading.">
      <form className="space-y-4" onSubmit={submit}>
        <Field label="Vehicle" required>
          <Select value={f.vehicle_id} onChange={(e) => set("vehicle_id", e.target.value)}>
            <option value="">—</option>
            {vehicles.map((v) => <option key={v.vehicle_id} value={v.vehicle_id}>{v.registration || v.vehicle_id.slice(0, 8)}</option>)}
          </Select>
        </Field>
        <div className="grid gap-4 sm:grid-cols-3">
          <Field label="Odometer (km)"><Input type="number" className="num text-right" value={f.odometer} onChange={(e) => set("odometer", e.target.value)} /></Field>
          <Field label="Litres"><Input type="number" step="any" className="num text-right" value={f.litres} onChange={(e) => set("litres", e.target.value)} /></Field>
          <Field label="Cost"><Input type="number" step="any" className="num text-right" value={f.cost} onChange={(e) => set("cost", e.target.value)} /></Field>
        </div>
        <Field label="Dossier" hint="Optional — attribute the fuel cost to an operation">
        ...
        <div className="flex justify-end gap-2 pt-2">
          ...
          <Button type="submit" loading={busy} disabled={!f.vehicle_id || busy}>Log fill</Button>
        </div>
      </form>
    </Modal>
  );
...
export function FuelLogPage() {
  ...
  const vehMap = React.useMemo(() => {
    ...
    (vehicles.data || []).forEach((v) => { m[v.vehicle_id] = v.registration || v.vehicle_id.slice(0, 8); });
    return m;
  }, [vehicles.data]);
  ...
  const eff = React.useMemo(() => {
    ...
    const odos = rows.map((r) => Number(r.odometer)).filter((n) => Number.isFinite(n));
  ...
  const cols: Column<api.FuelLog>[] = [
    { key: "when", label: "Date", render: (r) => <span className="num text-muted-foreground">{dateFmt(r.created_at)}</span> },
    ...(!veh ? [{ key: "veh", label: "Vehicle", render: (r: api.FuelLog) => <span className="num font-medium text-foreground">{vehMap[r.vehicle_id || ""] || "—"}</span> } as Column<api.FuelLog>] : []),
    { key: "odo", label: "Odometer", className: "text-right", render: (r) => <span className="num">{num(r.odometer)}</span> },
    { key: "litres", label: "Litres", className: "text-right", render: (r) => <span className="num">{num(r.litres)}</span> },
    { key: "cost", label: "Cost", className: "text-right", render: (r) => <span className="num">{money(r.cost)}</span> },
  ];
    ...
    <section className={shell}>
      ...
      {eff && (
        <div className="mb-5 grid grid-cols-2 gap-3 sm:grid-cols-5">
          ...
          <Stat label="Distance" value={eff.distance ? `${num(eff.distance)} km` : "—"} />
          <Stat label="L / 100km" value={eff.l100 != null ? eff.l100.toFixed(1) : "—"} />
        </div>
      )}
