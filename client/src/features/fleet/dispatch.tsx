...
function OdometerModal({
  ...
  async function submit(e: React.FormEvent) {
    ...
    setError(null);
    try {
      await api.setDispatchStatus(
        dispatch.fleet_dispatch_id,
        to,
        odo === "" ? undefined : Number(odo),
      );
      onSaved();
      onClose();
    } catch (err) {
  ...
  return (
    <Modal
      open
      onClose={onClose}
      title={to === "OUT" ? "Check out vehicle" : "Check in vehicle"}
      description={`${dispatch.registration || "Vehicle"} · ${dispatch.driver_name || "driver"}`}
    >
...
export function DispatchPage() {
  ...
  async function cancel(d: api.Dispatch) {
    ...
    try {
      await api.setDispatchStatus(d.fleet_dispatch_id, "CANCELLED");
      disp.reload();
    } catch (e) {
      reportActionError(e);
    } finally {
  ...
  const cols: Column<api.Dispatch>[] = [
    ...
    },
    {
      ...
      render: (d) => (
        <span className="num text-muted-foreground">
          {d.check_out_at
            ? `${dateFmt(d.check_out_at)}${d.odometer_out != null ? ` · ${num(d.odometer_out)}km` : ""}`
            : "—"}
        </span>
      ),
    },
    {
      ...
      render: (d) => (
        <span className="num text-muted-foreground">
          {d.check_in_at
            ? `${dateFmt(d.check_in_at)}${d.odometer_in != null ? ` · ${num(d.odometer_in)}km` : ""}`
            : "—"}
        </span>
      ),
    },
            ...
            ? [
                {
                  ...
                  label: "Cancel",
                  variant: "outline",
                  loading: busy === d.fleet_dispatch_id,
                },
          ...
          <div className="flex items-center justify-end gap-2">
            ...
            <TransitionButtons
              ...
              onTransition={(to) =>
                to === "CANCELLED"
                  ? cancel(d)
                  : setOdo({ dispatch: d, to: to as "OUT" | "RETURNED" })
              }
            />
          </div>
        );
  ...
  return (
    <section className={shell}>
      ...
      <DataList
        ...
        rows={disp.data}
        error={disp.error}
        loading={disp.loading}
        rowKey={(d) => d.fleet_dispatch_id}
        empty={{
          title: "No dispatches",
          hint: "Assign a vehicle to get started.",
        }}
      />
      {creating && (
        <NewDispatchForm
