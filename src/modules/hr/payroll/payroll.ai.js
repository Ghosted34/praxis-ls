...
module.exports = {
  ...
  writes: [
    ...
    { key: "compute_payroll", service: (c, p, actor) => service.compute(c, { id: p.payroll_run_id, config: p.config || null, actor }), schema: validator.schemas.aiCompute, permission: { module: "MOD-17", action: "edit" }, confirm: true, describe: "Compute payslips for all active employees (CNPS/IRPP/CAC/CFC/FNE per KB §9)." },
    { key: "set_payroll_status", service: (c, p, actor) => service.setStatus(c, { id: p.payroll_run_id, status: p.status, actor }), schema: validator.schemas.aiStatus, permission: { module: "MOD-17", action: "approve" }, confirm: true, describe: "Advance a payroll run by id (OPEN→COMPUTED→SUBMITTED→APPROVED→VALIDATED→DISBURSED, or REJECTED); validation posts the GL entry." },
  ],
};
