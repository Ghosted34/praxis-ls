...
describe("spend — period maths", () => {
  ...
  test("an explicit window is honoured verbatim, and `to` is INCLUSIVE", () => {
    ...
    expect(r.monthKeys(p.from, p.to)).toEqual([
      "2026-01",
      "2026-02",
      "2026-03",
    ]);
  ...
  test("garbage dates fall back to the default window instead of throwing", () => {
    ...
    expect(p.from).toBe("2025-09-01");
    expect(
      r.normalisePeriod({ from: "2026-13-45", to: "2026-02-30" }).to,
    ).toBeTruthy();
  });

  test("monthKeys spans a year boundary and is capped", () => {
    expect(r.monthKeys("2025-11-15", "2026-02-02")).toEqual([
      "2025-11",
      "2025-12",
      "2026-01",
      "2026-02",
...
describe("spend — series assembly", () => {
  ...
  test("the month axis is DENSE — a month with no spend is a zero row, not a gap", () => {
    ...
    const { months: rows } = r.spendSeries(months, {
      actual: [
        { month: "2026-01", amount: "1000", count: 2 },
        { month: "2026-03", amount: "500", count: 1 },
      ],
    });
  ...
  test("the three lenses merge onto one axis and total independently", () => {
    ...
    expect(
      totals.estimated_count + totals.committed_count + totals.actual_count,
    ).toBe(6);
  });
  ...
  test("Postgres NUMERIC arrives as a string and must not concatenate", () => {
    const { totals } = r.spendSeries(["2026-01", "2026-02"], {
      actual: [
        { month: "2026-01", amount: "1000.50", count: "2" },
        { month: "2026-02", amount: "2000.25", count: "1" },
      ],
    });
  ...
  test("a month outside the axis is dropped rather than corrupting the totals", () => {
    const { totals, months: rows } = r.spendSeries(["2026-02"], {
      actual: [{ month: "2026-01", amount: 999, count: 1 }],
    });
...
describe("cost evolution — rate timeline & trend", () => {
  ...
  test("orders oldest-first regardless of the order the rows arrive in", () => {
    expect(
      r.rateTimeline(rates, "2026-07-01").map((x) => x.expense_rate_id),
    ).toEqual(["a", "b", "c"]);
  });

  test("`in_force` is answered AS OF a date, not as of now", () => {
    const inForceOn = (day) =>
      r.rateTimeline(rates, day).find((x) => x.in_force);
  ...
  test("rows with no effective_from are dropped, not sorted to the front", () => {
    expect(
      r.rateTimeline(
        [...rates, { rate: 1, effective_from: null }],
        "2026-07-01",
      ),
    ).toHaveLength(3);
  });
  ...
  test("trend measures first → last, and distinguishes 'flat' from 'no data'", () => {
    ...
    expect(r.rateTrend([{ rate: 5 }, { rate: 5 }])).toMatchObject({
      delta: 0,
      delta_pct: 0,
      direction: "flat",
      points: 2,
    });
    expect(r.rateTrend([])).toMatchObject({
      first: null,
      last: null,
      delta: null,
      delta_pct: null,
    ...
    expect(r.rateTrend([{ rate: 42 }])).toMatchObject({
      first: 42,
      last: 42,
      delta: 0,
      points: 1,
  ...
  test("a rise from zero has no percentage — it is undefined, not infinite", () => {
    expect(r.rateTrend([{ rate: 0 }, { rate: 500 }])).toMatchObject({
      delta: 500,
      delta_pct: null,
      direction: "up",
    });
...
describe("proof obligations", () => {
  test("ALWAYS_REQUIRED and requires_justification are independent triggers", () => {
    expect(
      r.proofObligation({ receipt_requirement: "ALWAYS_REQUIRED" }).reasons,
    ).toEqual(["ALWAYS_REQUIRED"]);
    expect(r.proofObligation({ requires_justification: true }).reasons).toEqual(
  ...
  test("CONDITIONALLY_REQUIRED is NOT an obligation — a flag raised on a maybe is noise", () => {
    expect(
      r.proofObligation({ receipt_requirement: "CONDITIONALLY_REQUIRED" })
        .required,
    ).toBe(false);
    expect(
      r.proofObligation({ receipt_requirement: "NOT_REQUIRED" }).required,
    ).toBe(false);
  ...
  test("the severity is WARN — advisory, never a hard block", () => {
    expect(
      r.proofObligation({ receipt_requirement: "ALWAYS_REQUIRED" }).severity,
    ).toBe("WARN");
  ...
  test("missingProof fires only when an obliged item has no proof attached", () => {
    ...
    expect(r.missingProof({ receipt_requirement: "NOT_REQUIRED" }, {})).toBe(
      false,
    );
  });
...
describe("import — cell parsing", () => {
  test("booleans accept the forms a French/English sheet actually contains", () => {
    for (const v of ["Y", "yes", "TRUE", "1", "oui", "x"])
      expect(r.parseBool(v)).toBe(true);
    for (const v of ["N", "no", "false", "0", "non"])
      expect(r.parseBool(v)).toBe(false);
...
describe("import — row validation", () => {
  ...
  test("THE rule: a row with no complete OHADA mapping is rejected", () => {
    const res = r.validateImportRow(
      { ...good(), purchase_debit: "", purchase_credit: "" },
      ctx,
    );
  ...
  test("accounts must be postable, and tax codes must exist", () => {
    expect(
      r
        .validateImportRow({ ...good(), purchase_debit: "9999" }, ctx)
        .reasons.join(" "),
    ).toMatch(/not a postable account/);
    expect(
      r
        .validateImportRow({ ...good(), purchase_tax_code: "TVA99" }, ctx)
        .reasons.join(" "),
    ).toMatch(/tax code "TVA99" does not exist/);
    const ok = r.validateImportRow(
      { ...good(), purchase_tax_code: "TVA19" },
      ctx,
    );
  ...
  test("enums are checked against the catalogue, with the offending value quoted", () => {
    expect(
      r
        .validateImportRow({ ...good(), direction: "EXPENCE" }, ctx)
        .reasons.join(" "),
    ).toMatch(/Direction "EXPENCE"/);
    expect(
      r
        .validateImportRow({ ...good(), category: "widget" }, ctx)
        .reasons.join(" "),
    ).toMatch(/Category "widget"/);
    expect(
      r
        .validateImportRow({ ...good(), receipt_requirement: "SOMETIMES" }, ctx)
        .reasons.join(" "),
    ).toMatch(/Receipt requirement/);
  });
  ...
  test("service keys resolve to ids, and a SERVICE_SCOPED row must name one", () => {
    const scoped = r.validateImportRow(
      {
        ...good(),
        applicability_mode: "SERVICE_SCOPED",
        service_type_keys: "SEA_IMPORT, AIR_IMPORT",
        tier: "ADVANCED",
      },
      ctx,
    );
    ...
    expect(
      r
        .validateImportRow(
          { ...good(), applicability_mode: "SERVICE_SCOPED" },
          ctx,
        )
        .reasons.join(" "),
    ...
    expect(
      r
        .validateImportRow({ ...good(), service_type_keys: "NOPE" }, ctx)
        .reasons.join(" "),
  ...
  test("numbers and booleans reject rather than coerce", () => {
    expect(
      r
        .validateImportRow({ ...good(), default_price: "abc" }, ctx)
        .reasons.join(" "),
    ).toMatch(/is not a number/);
    expect(
      r
        .validateImportRow({ ...good(), default_price: "-5" }, ctx)
        .reasons.join(" "),
    ).toMatch(/cannot be negative/);
    expect(
      r
        .validateImportRow({ ...good(), is_billable: "perhaps" }, ctx)
        .reasons.join(" "),
    ).toMatch(/must be Y or N/);
    expect(
      r
        .validateImportRow({ ...good(), currency: "XAFF" }, ctx)
        .reasons.join(" "),
    ).toMatch(/3-letter code/);
  });

  test("every reason for a bad row is reported at once, not one per round trip", () => {
    const res = r.validateImportRow(
      { label_fr: "x", category: "nope", direction: "nope" },
      ctx,
    );
...
describe("import — partitioning a sheet", () => {
  ...
  test("PARTIAL: good rows survive a sheet that also contains bad ones", () => {
    const rows = [
      row({ label_fr: "One" }),
      row({ label_fr: "Two", direction: "NOPE" }),
      row({ label_fr: "Three" }),
    ];
  ...
  test("a duplicate name inside the file is rejected rather than minting two codes", () => {
    const { valid, rejected } = r.partitionImport(
      [row({ label_fr: "Same" }), row({ label_fr: "same" })],
      ctx,
    );
    expect(valid).toHaveLength(1);
