...
type Tpl = { docType: string; title: { fr?: string; en?: string }; module: string; fields: string[]; report?: boolean };
...
const Check = ({ label: l, checked, onChange }: { label: string; checked: boolean; onChange: (v: boolean) => void }) => (
...
export function TemplateStudioPage() {
  ...
  React.useEffect(() => {
    if (!docType) return;
    let live = true;
    setError(null); setDirty(false); setRecordId("");
    (async () => {
      try {
        const q = entityId ? `?entity_id=${entityId}` : "";
        const c = await tenant<{ tenant_default: Cfg; entity_override: Cfg | null }>(`/document-templates/${docType}/config${q}`);
        if (live) setCfg({ ...(c.tenant_default || {}), ...(c.entity_override || {}) });
        const recs = await tenant<Rec[]>(`/document-templates/${docType}/records`);
  ...
  React.useEffect(() => {
    ...
    const id = setTimeout(async () => {
      try {
        const r = await tenant<{ html: string }>(`/document-templates/${docType}/preview`, {
          method: "POST", body: { entity_id: entityId || undefined, record_id: recordId || undefined, config: cfg },
        });
  ...
  const set = (patch: Cfg) => { setCfg((c) => ({ ...c, ...patch })); setDirty(true); };
  const setNested = (key: string, patch: Cfg) => { setCfg((c) => ({ ...c, [key]: { ...(c[key] as Cfg), ...patch } })); setDirty(true); };
  const show = (cfg.show as Cfg) || {};
  const logo = (cfg.logo as Cfg) || {};
  const sig = (cfg.signature as Cfg) || {};
  const s = (v: unknown, d = "") => (v === undefined || v === null ? d : String(v));

  async function save() {
    setBusy("save"); setError(null);
    try { await tenant(`/document-templates/${docType}/config`, { method: "PUT", body: { entity_id: entityId || undefined, config: cfg } }); setDirty(false); }
    catch (e) { setError(errMsg(e)); } finally { setBusy(null); }
  }
  async function generate() {
    setBusy("gen"); setError(null);
    try {
      const out = await tenant<{ verify?: string }>(`/document-templates/${docType}/generate`, { method: "POST", body: { entity_id: entityId || undefined, record_id: recordId || undefined } });
      alert("PDF generated & vaulted." + (out.verify ? `\nVerify: ${out.verify}` : ""));
    ...
    <section className={shell}>
      ...
      <div className="grid gap-5 lg:grid-cols-[1fr_320px]">
        ...
        <div className="lux-card overflow-hidden p-0">
          ...
          <iframe title="preview" srcDoc={html} className="h-[78vh] w-full bg-white" sandbox="" />
        </div>
        ...
        <div className="lux-card max-h-[82vh] space-y-4 overflow-auto p-4">
          <div>
            ...
            <div className="space-y-2">
              <Field label="Accent colour"><Input type="color" value={s(cfg.accent, "#F5821F")} onChange={(e) => set({ accent: e.target.value })} className="h-9 w-full" /></Field>
              <Field label="Language"><Select value={s(cfg.language, "bilingual")} onChange={(e) => set({ language: e.target.value })}><option value="bilingual">Bilingual FR/EN</option><option value="fr">Français</option><option value="en">English</option></Select></Field>
              <div className="grid grid-cols-2 gap-2">
                <Field label="Paper"><Select value={s(cfg.paper, "A4")} onChange={(e) => set({ paper: e.target.value })}><option>A4</option><option>Letter</option></Select></Field>
                <Field label="Margin (mm)"><Input type="number" value={s(cfg.margin_mm, "16")} onChange={(e) => set({ margin_mm: Number(e.target.value) })} /></Field>
              </div>
              <Check label="Show logo" checked={logo.show !== false} onChange={(v) => setNested("logo", { show: v })} />
              <Field label="Logo align"><Select value={s(logo.align, "left")} onChange={(e) => setNested("logo", { align: e.target.value })}><option value="left">Left</option><option value="right">Right</option></Select></Field>
            </div>
          </div>

          <div>
            <div className="micro mb-2">Sections</div>
            <Check label="Tax breakdown" checked={show.tax_breakdown !== false} onChange={(v) => setNested("show", { tax_breakdown: v })} />
            <Check label="Notes" checked={show.notes !== false} onChange={(v) => setNested("show", { notes: v })} />
            <Check label="Bank details" checked={show.bank !== false} onChange={(v) => setNested("show", { bank: v })} />
            <Check label="Terms & conditions" checked={show.terms !== false} onChange={(v) => setNested("show", { terms: v })} />
            <Check label="Signature block" checked={show.signature !== false} onChange={(v) => setNested("show", { signature: v })} />
            <Check label="Verify QR line" checked={show.qr !== false} onChange={(v) => setNested("show", { qr: v })} />
          </div>
          ...
          <div>
            ...
            <div className="space-y-2">
              ...
              <Field label="Watermark"><Input value={s(cfg.watermark)} onChange={(e) => set({ watermark: e.target.value })} placeholder="DRAFT / PAID / COPY" /></Field>
            </div>
          </div>
        </div>
      </div>
