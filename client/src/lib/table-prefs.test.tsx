...
describe("useRowSelection", () => {
  it("toggles one row at a time", () => {
    const { result } = renderHook(() =>
      useRowSelection(rows("a", "b", "c"), key),
    );
  ...
  it("shift-click selects the range from the last plain click, in visual order", () => {
    const { result } = renderHook(() =>
      useRowSelection(rows("a", "b", "c", "d", "e"), key),
    );
    act(() => result.current.toggle("b"));
  ...
  it("extends a range upwards as readily as downwards", () => {
    const { result } = renderHook(() =>
      useRowSelection(rows("a", "b", "c", "d"), key),
    );
...
describe("useColumnVisibility", () => {
  ...
  it("hides a column and remembers it", () => {
    const { result, unmount } = renderHook(() =>
      useColumnVisibility("t3", COLUMNS),
    );
    ...
    expect(again.result.current.columns.map((c) => c.key)).toEqual([
      "id",
      "name",
      "_a",
    ]);
  ...
  it("PERSISTS THE HIDDEN SET, so a new column is visible to everyone", () => {
    ...
    const { result, unmount } = renderHook(() =>
      useColumnVisibility("t4", COLUMNS),
    );
    act(() => result.current.setVisible("name", false));
    unmount();

    const withNewColumn: Column<Row>[] = [
      ...COLUMNS.slice(0, 3),
      { key: "vat", label: "VAT" },
      COLUMNS[3],
    ];
  ...
  it("re-reads when the screen swaps its storage key", () => {
    const { result, rerender } = renderHook(
      ({ k }) => useColumnVisibility(k, COLUMNS),
      {
        initialProps: { k: "tabA" },
      },
    );
...
describe("toCsv", () => {
  ...
  it("writes a header row and CRLF line endings", () => {
    expect(toCsv(cols, [{ code: "706", label: "Ventes" }])).toBe(
      "Code,Label\r\n706,Ventes",
    );
  });
  ...
  it("NEUTRALISES A FORMULA, which is the whole reason this is shared code", () => {
    ...
    const out = toCsv(cols, [
      { code: "706", label: '=HYPERLINK("http://evil","x")' },
    ]);
  ...
  it("neutralises every formula lead character", () => {
    for (const lead of ["=", "+", "-", "@"]) {
      expect(toCsv(cols, [{ code: `${lead}1`, label: "x" }])).toContain(
        `'${lead}1`,
      );
    }
  });
  ...
  it("quotes and doubles per RFC 4180, and leaves plain values unquoted", () => {
    ...
    expect(toCsv(cols, [{ code: "706100", label: "Ventes" }])).toContain(
      "706100,Ventes",
    );
  });

  it("renders null and undefined as empty, not as the words", () => {
    expect(toCsv(cols, [{ code: null, label: undefined }])).toBe(
      "Code,Label\r\n,",
    );
  });
});
