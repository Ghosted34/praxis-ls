...
function fakeClient({ enabled, grant = null, period = null, spent = 0 }) {
  return {
    query: async (sql) => {
      if (/FROM feature_state/.test(sql)) return { rows: [{ state: enabled ? "on" : "off" }] };
...
describe("assistant governance gate", () => {
  ...
  test("enabled + no explicit grant → allowed (a grant is opt-out, not required)", async () => {
    const v = await governance.canUseFeature(fakeClient({ enabled: true }), { userId: user.user_id, featureKey: "assistant" });
  ...
  test("enabled + budget hard-capped → blocked", async () => {
    const client = fakeClient({ enabled: true, period: { period_id: "p1", soft_cap_xaf: 100, hard_cap_xaf: 200 }, spent: 250 });
