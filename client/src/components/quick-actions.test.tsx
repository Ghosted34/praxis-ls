...
describe("QuickActionsMenu", () => {
  ...
  it("is a real menu button, not a hover cluster of divs", async () => {
    ...
    expect(
      screen.getByRole("menuitem", { name: /Praxis AI/ }),
    ).toBeInTheDocument();
    expect(
  ...
  it("PUTS THE UNREAD COUNT IN THE ACCESSIBLE NAME, not just in a badge", () => {
    ...
    expect(
      screen.getByRole("button", { name: "Quick actions, 3 unread" }),
    ).toBeInTheDocument();
  });
  ...
  it("caps a large count without lying about it in the name", () => {
    ...
    expect(
      screen.getByRole("button", { name: "Quick actions, 120 unread" }),
    ).toBeInTheDocument();
  ...
  it("does NOT carry the clock — that moved to its own title-bar chip", async () => {
    ...
    expect(
      screen.queryByRole("menuitem", { name: /Clock (in|out)/ }),
    ).toBeNull();
  });
