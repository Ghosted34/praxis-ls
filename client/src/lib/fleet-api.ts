...
const byVehicle = (vehicleId?: string) =>
...
export const listDispatch = (vehicleId?: string) =>
  tenant<Dispatch[]>(`/dispatch${byVehicle(vehicleId)}`);
...
export const setDispatchStatus = (
  ...
  tenant<Dispatch>(`/dispatch/${id}/status`, {
    method: "POST",
    body: { status, odometer: odometer ?? null },
  });
...
export const listWorkOrders = (vehicleId?: string) =>
  tenant<WorkOrder[]>(`/work-orders${byVehicle(vehicleId)}`);
...
export const setWorkOrderStatus = (id: string, status: string) =>
  tenant<WorkOrder>(`/work-orders/${id}/status`, {
    method: "POST",
    body: { status },
  });
export const getWorkOrderParts = (id: string) =>
  tenant<WorkOrderPart[]>(`/work-orders/${id}/parts`);
export const addWorkOrderPart = (
  ...
  tenant<WorkOrder & { parts: WorkOrderPart[] }>(`/work-orders/${id}/parts`, {
    method: "POST",
    body,
  });
...
export const setIncidentStatus = (id: string, status: string) =>
  tenant<Incident>(`/incidents/${id}/status`, {
    method: "POST",
    body: { status },
  });
...
export const setVehicleStatus = (id: string, status: string) =>
  tenant<Vehicle>(`/vehicles/${id}`, { method: "PATCH", body: { status } });
export const listDrivers = () =>
  tenant<{ employee_id: string; full_name?: string | null }[]>("/employees");

export const listVehicleCompliance = (vehicleId?: string) =>
  tenant<VehicleCompliance[]>(`/vehicle-compliance${byVehicle(vehicleId)}`);
export const createCompliance = (body: {
  vehicle_id: string;
  kind: string;
  expires_on?: string;
}) =>
...
export const renewCompliance = (id: string, expires_on: string) =>
  tenant<VehicleCompliance>(`/vehicle-compliance/${id}`, {
    method: "PATCH",
    body: { expires_on },
  });

export const listFuel = (vehicleId?: string) =>
  tenant<FuelLog[]>(`/fuel${byVehicle(vehicleId)}`);
export const createFuel = (body: {
  vehicle_id: string;
  odometer?: number;
  litres?: number;
  cost?: number;
...
export const listIncidents = (vehicleId?: string) =>
  tenant<Incident[]>(`/incidents${byVehicle(vehicleId)}`);
...
export const renewLicense = (id: string, expires_on: string) =>
  tenant<DriverLicense>(`/drivers/${id}`, {
    method: "PATCH",
    body: { expires_on },
  });
