...
**Next up for another engineer:** PR 8 (eval harness/observability — B6, H1, H2, G3). It is the last one: PR 1–5 and PR 7 are closed, and PR 6 (conversation management/Spaces — J1–J5) merged in [#428](https://github.com/tomblakeasaah196/praxis-ls/pull/428). The write-contract backlog is **empty** — `src/services/ai/write-contract-baseline.json` is `[]`, so the ratchet now fails any NEW non-conforming write rather than grandfathering a queue. **Whoever closes a milestone updates this section** (the milestone row, the finding rows, and Appendix A where relevant).

**Two modules are deliberately NOT wired to the AI, and the reason is a constraint rather than a backlog item** — both carry an `// ai:none` opt-out naming it, so the coverage gate is honest rather than merely green. `dashboard/support`: tenant→Praxis tickets live in the CENTRAL platform DB (`platform.support_ticket`, keyed by `tenant_id`) and the service is addressed by `(tenantId, email)`, not by a tenant connection — every AI tool runs on the caller's tenant client, which cannot reach that table, and the actor carries no `tenant_id`. Wiring it means widening the actor at every AI entry point (the controller, the transcribe job) first, which is its own change. `vault/document_verification`: its only surface is `resolve(code)` on the public `/v/:code` portal, whose routes file records that the rate limiter is the SOLE defence against enumerating a plaintext 12-character code — an AI tool is an unlimited authenticated path to that lookup, and a model can call it in a loop. Neither is hard; both are decisions somebody should take on purpose. [#423](https://github.com/tomblakeasaah196/praxis-ls/pull/423)
...
| PR 2 | Completeness, model & no‑truncation (B1, B2, B4, B5) | ✅ merged | B1 + B4 in [#404](https://github.com/tomblakeasaah196/praxis-ls/pull/404); B2 (working Gemini fallback via the OpenAI‑compat gateway + startup health check) and B5 (one shared, cached system‑prompt builder) in [#414](https://github.com/tomblakeasaah196/praxis-ls/pull/414). (B3 — trial a stronger primary model — is a config repoint, not code, and stays open under PR 8.) |
| PR 3 | "Create anything": one write contract (C1–C4) | ✅ merged | Contract + gate + the named creates + 31 create actions in [#406](https://github.com/tomblakeasaah196/praxis-ls/pull/406); the remaining **154 writes migrated and the baseline emptied** in [#423](https://github.com/tomblakeasaah196/praxis-ls/pull/423) |
...
| PR 5 | Reliability, timeouts & performance (E1–E4, G2) | ✅ merged | E1 + E2 + E4 in [#419](https://github.com/tomblakeasaah196/praxis-ls/pull/419); E3 verified already efficient (documented, no change). The FRONTEND half — the per‑screen copilot on the SSE path (E1), the bounded non‑streaming ask (G2) and the provider‑vs‑transient error surface (G4) — in [#423](https://github.com/tomblakeasaah196/praxis-ls/pull/423) |
| PR 6 | Conversation management & Spaces UX (J1–J5) | ✅ merged | All five in [#428](https://github.com/tomblakeasaah196/praxis-ls/pull/428): migration `13930` (`deleted_at`/`archived_at`/`pinned_at` on `ai_conversation`), `PATCH`/`DELETE /ai/conversations/:id`, the per-row overflow menu and the collapsible Spaces section. |
| PR 7 | Module → AI governance (I1–I4) | ✅ merged | CLAUDE.md rule in [#400](https://github.com/tomblakeasaah196/praxis-ls/pull/400) (I1); the **coverage gate, the catalogue‑drift check and 20 missing manifests** in [#423](https://github.com/tomblakeasaah196/praxis-ls/pull/423) |
| PR 8 | Evaluation, quality bar & observability (B6, H1, H2, G3) | 🟡 built | Telemetry (`13940 ai_health_event`), the AI Control health panel, the golden-set grader gated in CI, the B6 style contract and the G3 regression cover — on `claude/praxis-ai-pr8-eval`. B3 (the model repoint) stays open as configuration. |
...
| B2 — declared fallback vendor can't answer | P1 | ✅ | `gemini` now resolves to the OpenAI‑compat gateway (secrets table → `.env`, no new knobs); a primary CONFIG error degrades to the working fallback (loud, not the stub); `llm.checkVendorHealth()` runs at boot and flags an unresolved/native‑endpoint/non‑distinct vendor. [#414](https://github.com/tomblakeasaah196/praxis-ls/pull/414) |
| B5 — system prompt large, uncached & duplicated | P2 | ✅ | One `SYSTEM_RULES`/`buildSystemPrompt()` shared by `ask`/`askStream` (kills the drift); static prefix marked cacheable per vendor (`cache_control` for explicit vendors, stable‑prefix plain for auto, degrade for unknown). Unblocks PR 4's D3 wider window. [#414](https://github.com/tomblakeasaah196/praxis-ls/pull/414) |
...
| G1 — `scope`/`mode` collected but never honoured | P1 | ✅ | Closed with D1/D5: the client's chosen area biases tool selection/retrieval and each mode appends a real posture directive server‑side (`selectTools` reads `scope`; `modeDirective(mode)`), so the composer's controls are no longer theatre. [#404](https://github.com/tomblakeasaah196/praxis-ls/pull/404) |
...
| I1 — CLAUDE.md never mentioned manifests | P0 | ✅ | "Wire every module to the AI" rule added. [#400](https://github.com/tomblakeasaah196/praxis-ls/pull/400). (Recorded here as "I3" until [#423](https://github.com/tomblakeasaah196/praxis-ls/pull/423); §2 numbers the CLAUDE.md gap I1 and the measured drift I3, and this row was carrying the wrong one while I1 also sat in the not-started list.) |
| I2 — nothing enforced coverage or catalogue sync | P0 | ✅ | Two gates, both in `ci-local.js` and `ci.yaml`. `check-ai-manifest-coverage.js`: a module with a public controller declares its AI surface or carries an `// ai:none <reason>` opt-out — the reason is required, a bare marker fails. `sync-actions.js --check`: manifest-side drift with no DB (unique keys, every manifest parses, every `ai_enabled` action resolves to an executor, every write has a schema and a verb `action-authz` can map). Both were proven to FAIL on a planted defect, not just to pass. [#423](https://github.com/tomblakeasaah196/praxis-ls/pull/423) |
| I3 — 38 of 119 modules had no manifest | P0 | ✅ | 20 manifests authored (credit_note, q_ticket, workspace Tasks/Calendar, onboarding, succession, hr_query, hr_sanction, document_vault, signature_request, signature_wet, qes, template, master_config, rate_provider, geo_place, audit_room, dashboard, catalogue, deliverability, signature) — 86 new action keys, catalogue 409 → 495, every write conforming to the contract from day one and each declared on its screen in `screen-registry.json`. 23 modules carry a reviewed opt-out. Two of the audit's hints are opted out rather than wired, for constraints named in §0. [#423](https://github.com/tomblakeasaah196/praxis-ls/pull/423) |
| I4 — confirm `live` AND `sandbox` are synced | P1 | ✅ | The audit's premise was stale: `sync-actions` already branched to `sandbox`. The REAL gap is that nothing calls `syncCatalogue` on provision — this script is its only caller in the tree — so a provisioned tenant carries whatever the seed left in both schemas. The migrations job now runs the sync THEN `--check --tenant`, and a second pass finding nothing to change is the assertion that it is complete, idempotent and reaches sandbox. [#423](https://github.com/tomblakeasaah196/praxis-ls/pull/423) |
| C-backlog — 154 pre-contract writes | P1 | ✅ | `write-contract-baseline.json` is `[]`. Three defect classes fixed: the actor dropped by a 2-arg wrapper (82, mechanical); the flat payload landing in the wrong parameter on a bare service ref (~45, incl. `post_journal_entry`, `record_cost`, `send_mail`, `sign_document`); and ~20 update/status writes whose payload carried NO record id, which needed an `ai*` schema variant (the `asset.aiUpdate` convention). HTTP schemas untouched. [#423](https://github.com/tomblakeasaah196/praxis-ls/pull/423) |
| G2 — non‑streaming fallback inherits the client HTTP timeout | P2 | ✅ | `tenant()` sets no timeout, which is right for a list and wrong for an ask: the server allows each model call 120s and a turn is several of them, so a stalled turn was a spinner that never resolved. `AI_ASK_TIMEOUT_MS` (180s — longer than the server's own per-call cap so a slow multi-hop answer is never cut off by us, finite so one that is never coming back says so), composed with any caller signal; the stream's two fallbacks inherit it too. [#423](https://github.com/tomblakeasaah196/praxis-ls/pull/423) |
| G4 — error surfacing | P3 | ✅ | A misconfigured vendor chain does not arrive as an error: the server answers 200 with prose, so every signal except one says the turn went fine. That one is `provider: null` — always sent, never carried by the client type. `classifyAiFailure` reads the field, not the prose (wording changes; an ANSWER about vendor config is not a config failure). Both kinds keep the retry; a provider failure adds where the fix lives. [#423](https://github.com/tomblakeasaah196/praxis-ls/pull/423) |
| E1 (frontend) — streaming everywhere | P1 | ✅ | `PraxisActions`, the in-screen copilot on every list, was the last caller of the non-streaming `askPraxis` — the path the general chat had already abandoned. Now on SSE: first tokens in ~500ms, the 15s heartbeat, an ephemeral status line, and an abort that reaches the server when the panel closes. [#423](https://github.com/tomblakeasaah196/praxis-ls/pull/423) |
...
| E1 — hard per‑call timeouts abort real work | P1 | ✅ | The 60s/120s axios caps were baked in; a multi‑hop `ask` tripped one and it was misread as transient (→ fallback). Now `AI_REQUEST_TIMEOUT_MS` (120s) / `AI_STREAM_TIMEOUT_MS` (300s); the SSE heartbeat + client‑disconnect abort still govern a live stream's body. (Frontend streaming‑everywhere is a follow‑up.) [#419](https://github.com/tomblakeasaah196/praxis-ls/pull/419) |
| E2 — every confirm fired an extra full `ask()` + dropped tags | P1 | ✅ | `allowed` is threaded controller → `confirm`/`confirmBatch` → the follow‑up (was `allowed: undefined`, dropping confidentiality tags); `ask({ skipRetrieval })` skips the embed + vector search AND the condense on the auto‑continue, so it costs a model call, not a second recall pass. [#419](https://github.com/tomblakeasaah196/praxis-ls/pull/419) |
| E3 — non‑stream re‑issues a final toolless call each round | P2 | ✅ | Verified no change needed post‑PR 4: the loop does a single toolless final pass only when forced (MAX rounds / pending write / duplicate groove); a plain chain ends on the continuation call that stops reaching. Documented rather than refactoring the scar‑covered tool loop. [#419](https://github.com/tomblakeasaah196/praxis-ls/pull/419) |
| E4 — embeddings a silent grounding dependency | P2 | ✅ | `embeddings.checkEmbeddingsHealth()` (resolution‑only) + a boot WARN when no embeddings vendor resolves ("grounding limited — tool reads only, no KB recall"). The AI Control panel is PR 8. [#419](https://github.com/tomblakeasaah196/praxis-ls/pull/419) |
| J1 — a conversation cannot be deleted or archived | P1 | ✅ | Soft-delete + archive + a confirm-gated hard purge (`13930`; `assistant.repo.softDeleteConversation` / `purgeConversation`; `DELETE /ai/conversations/:id[?purge=true]`). The FK that blocked this is handled rather than repointed — altering a constraint on a pre-existing table above 13791 breaks a fresh tenant's sandbox pass, so the purge DETACHES `ai_action_run` rows that reached EXECUTED (the only record in the tree of a real ERP change, and there is no second audit table behind it) and DELETEs the rest, whose payloads are verbatim thread content. A soft-deleted thread also stops being loadable BY ID, so a stale `?c=` cannot put a removed transcript back on screen. **Found while closing this:** the ask path took the request's `conversation_id` unverified (`orchestrator.history_`, validated only as a uuid), so a borrowed id replayed another user's transcript into the caller's prompt and appended the turn to their thread — on the path that WRITES, while `service.history()` had always checked. Now resolved through `conversationBelongsToUser`, which is both the ownership check and the reason a removed thread cannot be written to from a second open tab. [#428](https://github.com/tomblakeasaah196/praxis-ls/pull/428) |
| J2 — a conversation cannot be pinned | P1 | ✅ | `pinned_at`, ordered `DESC NULLS LAST` ahead of `last_at` server-side so the rail can group on what arrives; `groupConversations` lifts pinned threads into their own leading group rather than marking them in place — a pin whose thread still slides from "Today" to "Previous 7 days" has pinned nothing. [#428](https://github.com/tomblakeasaah196/praxis-ls/pull/428) |
| J3 — a conversation cannot be renamed | P2 | ✅ | `PATCH /ai/conversations/:id` with `{ title }`, through `usePrompt()`. The `title` column has existed since 0400 and had never been written to. An EMPTY title stores NULL, which restores the derived title (the first user message) via the list query's existing `COALESCE(NULLIF(...))` — reset with no second control. The mutation answers with the row in the rail's own shape, from the same shared SELECT, so a cleared title comes back derived rather than null. [#428](https://github.com/tomblakeasaah196/praxis-ls/pull/428) |
| J4 — the Spaces list is not collapsible | P1 | ✅ | Collapsible section, remembered in `localStorage` keyed by **user id** — a shared workstation is two people and one localStorage. [#428](https://github.com/tomblakeasaah196/praxis-ls/pull/428) |
| J5 — row-level affordances are missing | P2 | ✅ | Per-row overflow menu (pin / rename / archive / delete), documented as a reusable pattern in `doc/FRONTEND_GUIDE.md` §8.1. Two sibling buttons in a layout div, not a menu nested in the row's button; the trigger is transparent-until-hover-or-focus rather than absent, so it exists for the keyboard. Delete's irreversible tier is an opt-in `<Checkbox>` inside the `useConfirm()` body — no `window.confirm`, per CLAUDE.md. [#428](https://github.com/tomblakeasaah196/praxis-ls/pull/428) |
| H2 — failure telemetry is thin | P2 | 🟡 | `ai_health_event` (13940) + `services/ai/health.service.js`: truncation, fallback, vendor config error, provider exhausted, timeout, tool-round cap, groove, stall nudge. The sharpest is truncation — `finish_reason` was never read, before or after B1 raised `max_tokens`, so that fix has been unverifiable in production in exactly the way B1 itself was. Detected in `llm.service` (the only layer that can see a fallback), recorded through `recordUsage` (the one choke point every model call already passes). AI Control → Health reports RATES per 1k turns with every kind shown including zeros — a row that vanishes when a signal stops firing disappears as it becomes good news. |
| H1 — no evaluation harness | P1 | 🟡 | Golden set (12 cases, each citing its finding) + a PURE grader (`services/ai/eval/grade.js`). The split is the design: `tests/unit/ai-eval-grader.test.js` gates CI on the RULES, against recorded answers, no DB and no vendor; `scripts/ai/eval.js --tenant=<slug>` is the live pass and is deliberately not a gate, because a build that reddens when a model rephrases is a build people stop reading. Includes the audit's named A1 case verbatim — a figure ≥ 100,000,000 XAF reported exactly, any redaction marker treated as a failure — and each rule is tested against the defect it exists to catch, not only against a good answer. |
| B6 — no style/grammar contract | P3 | 🟡 | A WRITING block in `SYSTEM_RULES`, inside the cacheable invariant prefix (B5) so it does not re-bill per question. The date line is the one that is not cosmetic: the product is day-first end to end and `check-date-format.js` gates the code, but it cannot reach a sentence the model composes — so an assistant writing `07/03/2026` for 3 July re-introduces the defect in the one surface nobody can lint. The grader scores it. |
| G3 — regression cover for the kept affordances | P2 | 🟡 | xlsx already had `tests/unit/ai-export.test.js`; listen-aloud and the canvas Markdown download had none, and they are the two that break QUIETLY — a voice reading "asterisk asterisk Total" and a draft saved as `.txt` both look right in a screenshot. `client/src/features/ai/answer-affordances.test.tsx`. |
| B3 | — | ⬜ | The only thing left that is not code: a `platform.ai_vendor_credential` repoint to a stronger tool-calling primary. Worth doing AFTER the eval above, which is what makes "stronger" measurable rather than felt. |

**Recommended next PR:** none — PR 8 (evaluation & observability) is the last milestone and is built on `claude/praxis-ai-pr8-eval`: H2 (telemetry + the AI Control health panel), H1 (golden set + grader, gated in CI), B6 (style contract) and G3 (regression cover). **B3 is all that remains after it, and B3 is a config repoint, not code** — and it is deliberately last, because the eval is what turns "a stronger model" from a claim into a measurement.

About PR 8 (evaluation & observability — B6, G3, H1, H2). It is last on purpose: a golden-set eval is only worth wiring once the behaviour it pins has stopped moving, and PRs 1–5 and 7 are what stopped it moving. Its grounding cases should include the A1 regression directly — a seeded figure ≥ 100,000,000 XAF reported exactly, and a `[NUM]` in an answer treated as a failure.
...
### PR 3 — "Create anything": one write contract (closes C1, C2, C3, C4; guards F1) · **P0** — ✅ landed ([#406](https://github.com/tomblakeasaah196/praxis-ls/pull/406), [#423](https://github.com/tomblakeasaah196/praxis-ls/pull/423))
...
### PR 5 — Reliability, timeouts & performance (closes E1, E2, E3, E4; G2, G4) · **P1** — ✅ landed ([#419](https://github.com/tomblakeasaah196/praxis-ls/pull/419) backend, [#423](https://github.com/tomblakeasaah196/praxis-ls/pull/423) frontend)
...
### PR 6 — Conversation management & Spaces UX (closes J1, J2, J3, J4, J5) · **P1** — ✅ landed ([#428](https://github.com/tomblakeasaah196/praxis-ls/pull/428))
...
### PR 7 — Module → AI governance: close the drift, wire the gaps (closes I1, I2, I3, I4) · **P0** — ✅ landed ([#400](https://github.com/tomblakeasaah196/praxis-ls/pull/400), [#423](https://github.com/tomblakeasaah196/praxis-ls/pull/423))
...
### PR 8 — Evaluation, quality bar & observability (closes B6, H1, H2; G3, G4) · **P1/P2** — 🟡 built on `claude/praxis-ai-pr8-eval`
...
**Status: COMPLETE ([#423](https://github.com/tomblakeasaah196/praxis-ls/pull/423)).** `write-contract-baseline.json` is `[]`. Every catalogued write is an inline wrapper that maps the AI's snake_case payload to its service's real argument shape and forwards the full actor, and `tests/unit/ai-write-contract.test.js` now fails any NEW non-conforming write with nothing left to grandfather it. The three defect classes behind the 154 are described in the `C-backlog` row of §0; the one worth repeating here is the third, because it was invisible to the original gate: roughly twenty update/status writes advertised a payload with **no record id in it at all** — the HTTP route takes the record in the URL and a copilot call has no URL — so they conformed to nothing and could never have executed. Those needed an `ai*` schema variant carrying the id (the convention `asset.aiUpdate` and `debt.aiDrawdown` already set), and the HTTP schemas were left untouched.

**Status after PR 3 ([#406](https://github.com/tomblakeasaah196/praxis-ls/pull/406)), for the record:** the write contract is defined (`service(client, payload, actor)`, full actor forwarded — `action-registrar.writeAdapter` + `doc/AI_ARCHITECTURE.md` §2) and enforced by `tests/unit/ai-write-contract.test.js` (`services/ai/write-contract.js`). The confirmed-broken writes are fixed and proven at runtime: `create_client` ✅, `create_supplier` ✅ (was C1 — threw), `create_lead` ✅ (was C2 — actor dropped), `create_opportunity` ✅ (was C2), `create_purchase_request` ✅ (was C3 — field-shape + actor), `draft_purchase_order` ✅. In the same pass, **31 create actions** were migrated to the contract (every `{ data, actor }`-shaped create across fleet, HR, master, sales, finance, WMS, procurement — so "create anything" holds for the create surface). The remaining **154 writes** (updates, transitions, status/settle actions, and camelCase-mapped creates) were inventoried in `src/services/ai/write-contract-baseline.json`; the ratchet forbade new non-conforming writes and shrank as each was migrated. All 154 were migrated in [#423](https://github.com/tomblakeasaah196/praxis-ls/pull/423) and that file is now `[]`.
