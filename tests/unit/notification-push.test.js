...
jest.mock("../../src/shared/push/push.service", () => ({ getPublicKey: jest.fn() }));
...
describe("notification push opt-in", () => {
  test("pushPublicKey surfaces the VAPID public key", async () => {
    ...
    await expect(svc.pushPublicKey()).resolves.toEqual({ public_key: "VAPID_PUB" });
  });
  ...
  test("subscribePush stores endpoint + keys for the caller", async () => {
    ...
    const subscription = { endpoint: "https://push.example/abc", keys: { p256dh: "k1", auth: "k2" } };
    await expect(svc.subscribePush(client, actor, { subscription, userAgent: "UA" })).resolves.toEqual({ subscribed: true });
  ...
  test("subscribePush rejects a malformed subscription (422)", async () => {
    await expect(svc.subscribePush(client, actor, { subscription: { endpoint: "x" } }))
  ...
  test("unsubscribePush removes the endpoint for the caller", async () => {
    ...
    await expect(svc.unsubscribePush(client, actor, { endpoint: "https://push.example/abc" }))
      .resolves.toEqual({ unsubscribed: true });
    expect(repo.deletePushSubscription).toHaveBeenCalledWith(client, "u-1", "https://push.example/abc");
  });
});
