...
describe("F2 company profile",()=>{
 ...
 test("derived SQL includes service type and commodity without commercially sensitive fields",()=>{expect(repoSource).not.toMatch(/\b(cost|margin|profit|unit_price|purchase_price)\b/i);expect(repoSource).toContain("SELECT count(*)::int FROM vehicle");expect(repoSource).toContain("service_type");expect(repoSource).toContain("commodity");expect(repoSource).toMatch(/GROUP BY COALESCE\(st\.name_en,st\.name_fr\), COALESCE\(NULLIF\(BTRIM\(d\.commodity_desc\)/);expect(migration).toContain("computed_at");});
 test("consent is three-state, shared and defaults anonymised-by-policy",()=>{for(const consent of ["NOT_ASKED","ANONYMISED_ONLY","NAMED"])expect(clientMaster.consent.parse({consent}).consent).toBe(consent);expect(()=>clientMaster.consent.parse({consent:"PUBLIC"})).toThrow();expect(migration).toContain("DEFAULT 'NOT_ASKED'");});
