  ...
  const client = {
    ...
    async query(sql, params = []) {
      ...
      if (/^INSERT INTO geo_place/i.test(s)) {
        ...
        if (state.rows.some((r) => r.query_key === queryKey))
          return { rows: [] };
        const row = place({
          query_key: queryKey,
          name,
          ...
          confidence,
          is_reference_point: isReferencePoint,
      ...
      if (/^SELECT .* FROM geo_place/i.test(s))
        return { rows: rankedSearch(s, params, state.rows) };
...
function rankedSearch(sql, params, rows) {
  ...
  const [code, term, prefix, contains] = ranked
    ? params.slice(-5, -1)
    : [null, null, null, null];
...
describe("search ranking", () => {
  ...
  test("an apostrophe is folded — ndjamena finds N'Djamena", async () => {
    const c = fakeClient([
      place({ name: "N'Djamena", country: "TD", kind: "CITY" }),
    ]);
  ...
  test("an inactive place is never offered", async () => {
    const c = fakeClient([
      place({ name: "Closed Terminal", kind: "TERMINAL", is_active: false }),
    ]);
  ...
  test("…but it still RESOLVES, so a file already routed through it keeps working", async () => {
    ...
    expect(found.get("Closed Terminal")).toMatchObject({
  ...
  test("country and kind filters both apply", async () => {
    const c = fakeClient(CATALOGUE());
    const { places } = await service.search(c, {
      q: null,
      country: "CM",
      kinds: ["AIRPORT"],
    });
...
describe("when the provider is consulted", () => {
  ...
  test("yes, when asked and the catalogue has only near misses", async () => {
    ...
    const out = await service.search(c, {
      q: "zone industrielle bassa",
      provider: true,
    });
  ...
  test("a suggestion the catalogue already holds is folded out", async () => {
    ...
    geoapify.searchPlaces.mockResolvedValue({
      ...
      results: [
        ...
        },
        {
          ...
          formatted: "Kribi Deep Sea Port, Cameroon",
          country: "CM",
          region: null,
          latitude: 2.8,
          longitude: 9.85,
    ...
    expect(out.provider.results.map((r) => r.provider_place_id)).toEqual([
      "p2",
    ]);
  });
...
describe("confirming a provider suggestion", () => {
  test("the coordinate is taken from the provider, not from the request", async () => {
    ...
    const row = await service.confirmSuggestion(c, {
      query: "bassa",
      providerPlaceId: "pid-1",
    });
    ...
    expect(geoapify.searchPlaces).toHaveBeenCalledWith(
      "bassa",
  ...
  test("confirming twice yields one row, not two", async () => {
    ...
    const first = await service.confirmSuggestion(c, {
      query: "bassa",
      providerPlaceId: "pid-1",
    });
    const second = await service.confirmSuggestion(c, {
      query: "bassa",
      providerPlaceId: "pid-1",
    });
    expect(second.geo_place_id).toBe(first.geo_place_id);
    expect(
      c.state.rows.filter((r) => r.query_key === first.query_key),
    ).toHaveLength(1);
  });
      ...
      results: [
        {
          ...
          formatted: "Zone Industrielle, Douala, Chad",
          region: "Logone",
          country: "TD",
          latitude: 12.1,
          longitude: 15.05,
...
describe("adding a place by hand", () => {
  ...
  test("adding the same place twice returns the first one", async () => {
    const c = fakeClient([]);
    const a = await service.createManual(c, {
      name: "Depot 12",
      latitude: 4.05,
      longitude: 9.7,
    });
    const b = await service.createManual(c, {
      name: "Depot 12",
      latitude: 4.05,
      longitude: 9.7,
    });
  ...
  test("reusing a name for a DIFFERENT point is refused, with the fix in the message", async () => {
    const c = fakeClient([]);
    await service.createManual(c, {
      name: "Depot 12",
      latitude: 4.05,
      longitude: 9.7,
    });
    await expect(
      service.createManual(c, {
        name: "Depot 12",
        latitude: 12.13,
        longitude: 15.05,
      }),
...
describe("resolveVerified", () => {
  ...
  test("blank and duplicate values cost one query, not several", async () => {
    ...
    expect(
      c.state.sql.filter((s) => s.includes("query_key = ANY")),
    ).toHaveLength(1);
  });
});
