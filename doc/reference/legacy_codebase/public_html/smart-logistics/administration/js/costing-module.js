...
(function () {
  ...
  const bsOffcanvas = el.offcanvas
    ? new bootstrap.Offcanvas(el.offcanvas)
    : null;
  ...
  // ---- LANGUAGE TOGGLE ----
  if (el.langEN)
    el.langEN.addEventListener("change", () => {
      if (el.langEN.checked) state.lang = "EN";
    });
  if (el.langFR)
    el.langFR.addEventListener("change", () => {
      if (el.langFR.checked) state.lang = "FR";
    });
  ...
  async function loadOpsFilesSelect() {
    ...
    const data = await apiGet(
      `../../api/operations/files/list_for_costing.php`,
    );

    const items = Array.isArray(data.items) ? data.items : [];
    el.linkOps.innerHTML =
  ...
  function detectGroup(serviceType) {
    ...
    if (s.includes("REPRESENTATION") || s.includes("BUSINESS_REP"))
      return "BUSINESS_REP";
    return "ALL";
  }
  ...
  function smartRoute(d, group) {
    if (group === "SEA")
      return d.sea_pol && d.sea_pod ? `${d.sea_pol} → ${d.sea_pod}` : null;
    if (group === "AIR")
      return d.air_origin && d.air_dest
        ? `${d.air_origin} → ${d.air_dest}`
        : null;
  ...
  async function loadSSDCFromDB(ref) {
    ...
    const data = await apiGet(
      `../../api/operations/files/get_ssdc.php?ref=${encodeURIComponent(ref)}`,
    );
    const d = data.item;
    if (!d) return;
    ...
    // Weight
    const w = d.gross_weight
      ? `${d.gross_weight} ${d.weight_unit || ""}`
      : null;
  ...
  function recalcTotals() {
    ...
    if (el.grandTTC)
      el.grandTTC.innerText = `${format2(ttc)} ${state.currency}`;
  ...
  function addLine(initial = {}) {
    ...
    const lineNo = el.linesBody
      ? el.linesBody.querySelectorAll("tr").length + 1
      : 1;
    ...
    tr.id = rowId;
    tr.dataset.dictId = initial.financial_dictionary_id
      ? String(initial.financial_dictionary_id)
      : "";
    ...
    tr.querySelector(".cost-input")?.addEventListener("input", () =>
      recalcRow(tr),
    );
    tr.querySelector(".vat-check")?.addEventListener("change", () =>
      recalcRow(tr),
    );
  ...
  window.openCostingOffcanvas = async (id) => {
    ...
    const data = await apiGet(
      `../../api/finance/costings/get.php?id=${encodeURIComponent(id)}`,
    );
  ...
  function buildPayload(nextStatus) {
    ...
    const ssdcClientName =
      document.getElementById("ssdc-client")?.innerText || "";
    const ssdcService =
      document.getElementById("ssdc-service")?.innerText || "";
    ...
    document.querySelectorAll("#lines-body tr").forEach((tr, idx) => {
      ...
      lines.push({
        line_no: idx + 1,
        financial_dictionary_id: tr.dataset.dictId
          ? Number(tr.dataset.dictId)
          : null,
        code,
  ...
  window.submitForValidation = async function () {
    try {
      ...
      const data = await apiPost(
        `../../api/finance/costings/transition.php`,
        payload,
      );
      ...
      if (el.badge) {
        el.badge.className = "badge bg-primary";
        el.badge.innerText = "SUBMITTED_FOR_VALIDATION";
      }
  ...
  window.validateCosting = async function () {
    try {
      ...
      state.currentStatus = data.status;
      if (el.badge) {
        el.badge.className = "badge bg-success";
        el.badge.innerText = "VALIDATED";
      }
  ...
  window.approveCosting = async function () {
    try {
      ...
      state.currentStatus = data.status;
      if (el.badge) {
        el.badge.className = "badge bg-dark";
        el.badge.innerText = "APPROVED_LOCKED";
      }
  ...
  if (el.currencySel) {
    el.currencySel.addEventListener("change", async () => {
      ...
      if (newCurr !== "XAF") {
        const input = prompt(
          `Enter Exchange Rate for ${newCurr} to XAF:`,
          "655.957",
        );
