 ...
 * own 360, which supplies a reference and a client and no stage. A row that
 * predates the set (13920's single `milestone_instance_id`) reads as a set of
 * one, so an older payload still round-trips through the form unchanged.
 */
...
/**
 * "SL3213 · Pre-alert +2" — how a card names a file and its stages in one
 * chip. The FIRST stage by name and the rest as a count, because a board card
 * is read in a column at a glance and three stage names would be a paragraph
 * where a token belongs; the panel lists them all.
 */
