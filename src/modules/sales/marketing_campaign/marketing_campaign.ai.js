...
module.exports = {
  ...
  writes: [
    ...
    { key: "transition_campaign", service: (c, p, actor) => service.transition(c, { id: p.campaign_id, to: p.to, actor }), schema: validator.schemas.aiTransition, permission: { module: "MOD-22", action: "edit" }, confirm: true, describe: "Submit a campaign for approval, or pause, resume or end a running one. Cannot approve — that is a deliberate act on the campaign itself." },
    { key: "record_campaign_actuals", service: (c, p, actor) => service.update(c, { id: p.campaign_id, patch: p, actor }), schema: validator.schemas.aiRecordActuals, permission: { module: "MOD-22", action: "edit" }, confirm: true, describe: "Record the hand-entered leads, opportunities and deals won from an external ad-manager report. Only accepted once the campaign is running." },
  ],
};
