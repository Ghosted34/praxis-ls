...
describe("valuesFrom · dates", () => {
  it("seeds a plain calendar date unchanged", () => {
    expect(
      valuesFrom(row({ incorporation_date: "2021-09-21" })).incorporation_date,
    ).toBe("2021-09-21");
  });
  ...
  it("does not move the day for a reader west of UTC", () => {
    ...
    expect(
      valuesFrom(row({ dissolution_date: "2026-08-14" })).dissolution_date,
    ).toBe("2026-08-14");
  });

  it("leaves an unset date empty rather than rendering 'null'", () => {
    const v = valuesFrom(
      row({ incorporation_date: null, dissolution_date: undefined }),
    );
...
describe("entityFormBody · dates round-trip", () => {
  it("sends back exactly what was seeded, so an untouched form is a no-op", () => {
    const seeded = valuesFrom(
      row({
        incorporation_date: "2021-09-21T00:00:00.000Z",
