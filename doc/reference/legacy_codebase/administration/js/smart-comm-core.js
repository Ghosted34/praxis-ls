...
(function () {
  ...
  document.addEventListener("DOMContentLoaded", () => {
    ...
    // 3. Mode Switchers
    if (UI.btnChannels)
      UI.btnChannels.addEventListener("click", () => switchMode("CHANNELS"));
  ...
  function switchMode(newMode) {
    ...
    // Visual Updates
    if (UI.btnChannels)
      UI.btnChannels.classList.toggle("active", newMode === "CHANNELS");
  ...
  async function loadCoreChannels() {
    ...
    try {
      ...
      if (!channels.length) {
        UI.tabs.innerHTML =
          '<div class="small text-danger p-2">No channels available.</div>';
        return;
      }
      ...
      UI.tabs.innerHTML =
        '<div class="small text-danger p-2">Connection failed.</div>';
    }
  }
  ...
  async function loadUserList() {
    ...
    try {
      ...
      if (json.data && json.data.length > 0) {
        ...
        UI.tabs.innerHTML =
          '<div class="small text-muted p-2">No users found.</div>';
      }
    } catch (e) {
      UI.tabs.innerHTML =
        '<div class="small text-danger p-2">List failed.</div>';
    }
  }
  ...
  async function handleUserSearch(query) {
    ...
    try {
      const res = await fetch(
        `${CONFIG.API_URL}?action=search_users&q=${encodeURIComponent(query)}`,
      );
      const json = await res.json();
      ...
      UI.tabs.innerHTML =
        '<div class="small text-danger p-2">Search Error</div>';
    }
  }
  ...
  async function openDmChat(targetId, name) {
    try {
      const res = await fetch(
        `${CONFIG.API_URL}?action=open_dm&target_user_id=${targetId}`,
      );
      const json = await res.json();
  ...
  function renderMessages(msgs) {
    msgs.forEach((msg) => {
      ...
      let badgeHtml = "";
      if (urgency === "URGENT")
        badgeHtml = '<div class="urgency-badge urgency-urgent">URGENT</div>';
      if (urgency === "CRITICAL")
        badgeHtml =
          '<div class="urgency-badge urgency-critical">CRITICAL</div>';
  ...
  function startHeartbeat() {
    setInterval(async () => {
      ...
      try {
        ...
        const res = await fetch(
          `${CONFIG.API_URL}?action=heartbeat&_t=${new Date().getTime()}`,
        );
        const json = await res.json();
