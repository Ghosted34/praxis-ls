...
**Next up for another engineer:** PR 8 (eval harness/observability — B6, H1, H2, G3). It is the last one: PR 1–5 and PR 7 are closed, and PR 6 (conversation management/Spaces — J1–J5) is built and awaiting merge in [#428](https://github.com/tomblakeasaah196/praxis-ls/pull/428) — flip its rows and J1–J5 to ✅ when it lands. The write-contract backlog is **empty** — `src/services/ai/write-contract-baseline.json` is `[]`, so the ratchet now fails any NEW non-conforming write rather than grandfathering a queue. **Whoever closes a milestone updates this section** (the milestone row, the finding rows, and Appendix A where relevant).
...
| PR 6 | Conversation management & Spaces UX (J1–J5) | 🟡 awaiting merge | All five in [#428](https://github.com/tomblakeasaah196/praxis-ls/pull/428): migration `13930` (`deleted_at`/`archived_at`/`pinned_at` on `ai_conversation`), `PATCH`/`DELETE /ai/conversations/:id`, the per-row overflow menu and the collapsible Spaces section. Flip to ✅ on merge. |
...
| B3, B6, G3, H1–H2 | — | ⬜ | Not started, and with PR 6 built this is the WHOLE of what is left. B3 is a `platform.ai_vendor_credential` repoint under PR 8, not code. G3 (regression cover for TTS / open-in-canvas / table→xlsx) belongs with PR 8's harness, which is where a regression pass can live. B6 + H1–H2 are PR 8. F1 is "good, preserve" — not a remediation item, and deliberately absent from this list. Every other finding A1–J5 has a row above, once each. |

**Recommended next PR:** PR 8 (evaluation & observability — B6, G3, H1, H2), now that PR 6 is built. Seven of the eight milestones are closed or built and no P0 or P1 remains outside PR 8.
...
### PR 6 — Conversation management & Spaces UX (closes J1, J2, J3, J4, J5) · **P1** — 🟡 built, awaiting merge ([#428](https://github.com/tomblakeasaah196/praxis-ls/pull/428))
