...
jest.mock("../../src/services/email.repo", () => ({ identityFor: jest.fn(), recordSend: jest.fn() }));
...
const FB = {
  from: "no-reply@praxisls.com", support_from: "support@praxisls.com", from_name: null,
...
test("successful send records a SENT log row with the provider message-id", async () => {
  emailRepo.identityFor.mockResolvedValue({
    email_identity_id: "id-1", purpose: "BILLING", from_address: "billing@acme.cm",
    from_name: "Acme", smtp_host: "smtp.acme.cm", smtp_port: 587,
  });
  emailRepo.recordSend.mockResolvedValue({ email_send_id: "log-1" });
  const tx = { sendMail: async () => ({ messageId: "abc-123" }) };
  const info = await send(client, { to: "x@y.cm", subject: "Invoice", purpose: "BILLING", entityRef: "invoice:rec-1" }, tx);
  ...
  expect(row).toMatchObject({ status: "SENT", provider_message_id: "abc-123", to_address: "x@y.cm", entity_ref: "invoice:rec-1", email_identity_id: "id-1" });
});

test("failed send records a FAILED log row and still throws", async () => {
  emailRepo.identityFor.mockResolvedValue({
    email_identity_id: "id-1", purpose: "BILLING", from_address: "billing@acme.cm", smtp_host: "h",
  });
  emailRepo.recordSend.mockResolvedValue({ email_send_id: "log-1" });
  const tx = { sendMail: async () => { throw new Error("smtp down"); } };
  await expect(send(client, { to: "x@y.cm", subject: "Invoice", purpose: "BILLING" }, tx)).rejects.toThrow("smtp down");
  expect(emailRepo.recordSend).toHaveBeenCalledTimes(1);
  expect(emailRepo.recordSend.mock.calls[0][1]).toMatchObject({ status: "FAILED", error: "smtp down" });
});

test("no log is written when client is null (injectable transport path)", async () => {
  const tx = { sendMail: async () => ({ messageId: "m" }) };
  await send(null, { to: "x@y.cm", subject: "Hi", purpose: "NOTIFICATIONS" }, tx);
...
test("a from override is NOT honoured on the fallback sender (deliverability)", async () => {
  ...
  const tx = { sendMail: async (m) => { sentFrom = m.from; return {}; } };
  let sentFrom = null;
  await send(client, { to: "x@y.cm", subject: "Campaign", from: "ceo@acme.cm", purpose: "NOTIFICATIONS" }, tx);
...
test("a from override IS honoured when the tenant has its own host", async () => {
  emailRepo.identityFor.mockResolvedValue({
    email_identity_id: "id-1", purpose: "BILLING", from_address: "billing@acme.cm", smtp_host: "smtp.acme.cm", smtp_port: 587,
  });
  emailRepo.recordSend.mockResolvedValue({ email_send_id: "log" });
  const tx = { sendMail: async (m) => { sentFrom = m.from; return {}; } };
  let sentFrom = null;
  await send(client, { to: "x@y.cm", subject: "Invoice", from: "ceo@acme.cm", purpose: "BILLING" }, tx);
