...
describe("onTaskOpened", () => {
  test("notifies every active role holder except the triggering actor", async () => {
    ...
    const sent = await na.onTaskOpened(client, {
      entityRef: "invoice:9f8c1234deadbeef",
      roleId: "r1",
      amountXaf: 1500000,
      excludeUserId: "actor",
    });
    ...
    expect(service.notify.mock.calls.map((c) => c[1].userId)).toEqual([
      "a",
      "b",
    ]);
    expect(service.notify.mock.calls[0][1]).toMatchObject({
      category: "approvals",
      eventTypeKey: "approval.pending",
    });
    expect(service.notify.mock.calls[0][1].body).toMatch(
      /Invoice 9f8c1234 is awaiting your approval — 1,500,000 XAF/,
    );
  });

  test("no role → no notifications", async () => {
    expect(
      await na.onTaskOpened(client, { entityRef: "x:1", roleId: null }),
    ).toBe(0);
  ...
  test("is best-effort: a producer failure is swallowed", async () => {
    ...
    await expect(
      na.onTaskOpened(client, { entityRef: "x:1", roleId: "r1" }),
    ).resolves.toBe(0);
  });
});
...
describe("onOutcome", () => {
  test("notifies the requester on approval (NORMAL)", async () => {
    ...
    await na.onOutcome(client, {
      entityRef: "invoice:9f8c1234",
      approved: true,
      actorUserId: "appr",
    });
    expect(service.notify).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        userId: "req",
        eventTypeKey: "approval.approved",
        priority: "NORMAL",
      }),
    );
  });
  ...
  test("notifies the requester on rejection (HIGH)", async () => {
    ...
    expect(service.notify).toHaveBeenCalledWith(
      client,
      expect.objectContaining({
        eventTypeKey: "approval.rejected",
        priority: "HIGH",
      }),
    );
  });
