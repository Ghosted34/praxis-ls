...
const schemas = {
  create: z.object({ entity_id: z.string().uuid(), dossier_id: z.string().uuid().optional().nullable(), customs_regime: z.enum(["IM4", "IM7", "IM8", "EX1", "EX2"]).optional().nullable(), service_direction: z.string().optional(), declared_value: z.number().nonnegative().optional().nullable(), submitted_docs: z.array(z.any()).optional(), lines: z.array(z.object({ inventory_item_id: z.string().uuid().optional().nullable(), label: z.string().optional(), packages: z.number().nonnegative().optional(), weight: z.string().optional() })).optional(), date: z.string().regex(/^\d{4}-\d{2}-\d{2}$/).optional() }),
  update: z.object({ submitted_docs: z.array(z.any()) }),
  // AI-facing: transit_order_id in the payload → list_transit_orders picker.
  aiUpdate: z.object({ transit_order_id: z.string().uuid(), submitted_docs: z.array(z.any()) }),
};
