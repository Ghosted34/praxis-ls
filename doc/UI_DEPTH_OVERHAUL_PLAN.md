...
**Problem in one line:** the backend modules carry real domain lifecycles, but the client wraps each in a generic `CrudResource` (list / create / edit / delete) — so the _actions_ that make the system perform are missing, and edit/delete sit where a workflow belongs.
...
Each screen should become the _shape of the work it supports_, not a table with an edit pencil. The archetypes we need:
