...
module.exports = {
  ...
  writes: [
    { key: "create_opportunity", service: (c, p, actor) => service.create(c, { data: p, actor }), schema: validator.schemas.create, permission: { module: "MOD-24", action: "create" }, confirm: true, describe: "Create a pipeline opportunity. Lands in the first open stage and inherits that stage's probability unless one is given." },
    { key: "move_opportunity", service: (c, p, actor) => service.moveStage(c, { id: p.opportunity_id, pipelineStageId: p.pipeline_stage_id, actor }), schema: validator.schemas.aiMove, permission: { module: "MOD-24", action: "edit" }, confirm: true, describe: "Move an opportunity (by id) to a pipeline stage. A settled opportunity refuses to move." },
    { key: "win_opportunity", service: (c, p, actor) => service.win(c, { id: p.opportunity_id, createDossier: p.create_dossier, entityId: p.entity_id, serviceTypeId: p.service_type_id, actor }), schema: validator.schemas.aiWin, permission: { module: "MOD-24", action: "edit" }, confirm: true, describe: "Mark an opportunity (by id) won (optionally open the delivery file)." },
  ],
};
