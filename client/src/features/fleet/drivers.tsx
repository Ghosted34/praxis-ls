...
const EXP_TONE: Record<ExpiryLevel, Tone> = {
  valid: "ok",
  soon: "warn",
  expired: "bad",
  none: "mute",
};
const EXP_LABEL: Record<ExpiryLevel, string> = {
  valid: "Valid",
  soon: "Due soon",
  expired: "Expired",
  none: "No date",
  ...
  async function submit(e: React.FormEvent) {
    ...
    try {
      await api.createLicense({
        ...
        issued_on: f.issued_on || undefined,
      ...
      <form className="space-y-4" onSubmit={submit}>
        ...
        <Field label={tr("Certification")}>
          <Input
            ...
            placeholder="ADR / dangerous goods…"
          />
        </Field>
        ...
        <div className="flex justify-end gap-2 pt-2">
          ...
          <Button
            type="submit"
            loading={busy}
            disabled={!f.employee_id || !f.license_class || busy}
          >
            Add licence
          </Button>
        </div>
...
function RenewModal({
  ...
  async function submit(e: React.FormEvent) {
    ...
    setError(null);
    try {
      await api.renewLicense(row.driver_license_id, date);
      onSaved();
      onClose();
    } catch (err) {
      setError(errMsg(err));
  ...
  return (
    <Modal
      ...
      onClose={onClose}
      title="Renew licence"
      description={`${row.driver_name || "Driver"} · ${row.license_class || ""}`}
    >
      <form className="space-y-4" onSubmit={submit}>
        <Field
        ...
        <div className="flex justify-end gap-2 pt-2">
          ...
          <Button type="submit" loading={busy} disabled={!date || busy}>
            Renew
          </Button>
        </div>
      </form>
...
export function DriversPage() {
  ...
  const [renewing, setRenewing] = React.useState<api.DriverLicense | null>(
    null,
  );
  ...
  const cols: Column<api.DriverLicense>[] = [
    ...
    },
    {
      ...
      label: "Class",
      render: (r) => (
        <span className="text-muted-foreground">{r.license_class || "—"}</span>
      ),
    },
    {
      key: "num",
    ...
    },
    {
      ...
      label: "Certification",
      render: (r) => (
        <span className="text-muted-foreground">{r.certification || "—"}</span>
      ),
    },
    {
      key: "exp",
    ...
    },
    {
      key: "status",
      label: "Status",
      render: (r) => <ExpiryPill date={r.expires_on} />,
    },
    {
      key: "_a",
      label: "",
  ...
  return (
    <section className={shell}>
      ...
      <DataList
        ...
        rowKey={(r) => r.driver_license_id}
        empty={{
          title: "No licences",
          hint: "Record driver licences to track expiry.",
        }}
      />
      {creating && (
        <NewLicenceForm
