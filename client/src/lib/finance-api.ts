...
export const createInvoiceDraft = (body: {
  ...
  tenant<{ final_invoice_id?: string; id?: string }>("/final-invoices", {
    method: "POST",
    body,
  });
...
export const getInvoice = (id: string) =>
  tenant<InvoiceDetail>(`/final-invoices/${id}`);

export const updateInvoiceDraft = (
  id: string,
  body: { client_id?: string; dossier_id?: string; lines?: InvoiceLineInput[] },
...
export const listPeriods = (entityId?: string) =>
  tenant<{ periods: Period[] }>(
    `/statements/periods${entityId ? `?entity_id=${entityId}` : ""}`,
  );

export const closePeriod = (body: {
  period_id: string;
  to: "FROZEN" | "CLOSED";
...
export const listDeclarations = (entityId?: string) =>
  tenant<TaxDeclaration[]>(
    `/tax/declarations${entityId ? `?entity_id=${entityId}` : ""}`,
  );

export const getDeclaration = (id: string) =>
  tenant<TaxDeclaration>(`/tax/declarations/${id}`);
...
export const approveDeclaration = (id: string) =>
  tenant<TaxDeclaration>(`/tax/declarations/${id}/approve`, {
    method: "POST",
    body: {},
  });

export const submitDeclaration = (id: string, body: { filed_ref?: string }) =>
  tenant<TaxDeclaration>(`/tax/declarations/${id}/submit`, {
    method: "POST",
    body,
  });
...
export const getCreditNote = (id: string) =>
  tenant<CreditNoteDetail>(`/credit-notes/${id}`);
...
export type DebtEngagement = {
  debt_engagement_id: string;
  entity_id?: string | null;
  dossier_id?: string | null;
  lender_kind?: string | null;
...
export type DebtRepayment = {
  debt_repayment_id: string;
  principal_part: number | string;
  interest_part: number | string;
  paid_on: string;
};
export type DebtDetail = DebtEngagement & {
  repayments?: DebtRepayment[];
  repaid?: { principal: number; interest: number };
...
export const createDebt = (body: DebtInput) =>
  tenant<DebtEngagement>("/financing", { method: "POST", body });
export const updateDebt = (
  id: string,
  body: Partial<
    Pick<
      DebtInput,
      "lender_name" | "interest_rate" | "due_on" | "started_on" | "dossier_id"
    >
  >,
) => tenant<DebtEngagement>(`/financing/${id}`, { method: "PATCH", body });
export const deleteDebt = (id: string) =>
  tenant<{ ok: boolean }>(`/financing/${id}`, { method: "DELETE" });
export const drawdownDebt = (
  id: string,
  body: {
    entity_id?: string;
    entry_date: string;
...
export const createAccount = (body: AccountInput) =>
  tenant<Account>("/chart-of-accounts", { method: "POST", body });
export const updateAccount = (
  code: string,
  body: Partial<Omit<AccountInput, "code" | "class">> & { is_active?: boolean },
...
export const getTrialBalance = () =>
  tenant<TrialBalance>("/statements/trial-balance");
...
export const getInvoiceTotals = (id: string, entryDate?: string) =>
  tenant<InvoiceTotals>(
    `/final-invoices/${id}/totals${entryDate ? `?entry_date=${entryDate}` : ""}`,
  );
...
export type Asset = {
  ...
  coa_asset_code?: string | null;
  coa_depr_code?: string | null;
};
...
export const createAsset = (body: AssetInput) =>
  tenant<Asset>("/assets", { method: "POST", body });
export const updateAsset = (
  id: string,
  body: Partial<
    Pick<AssetInput, "label" | "tag" | "coa_asset_code" | "coa_depr_code">
  >,
) => tenant<Asset>(`/assets/${id}`, { method: "PATCH", body });
export const depreciateAsset = (id: string, period_code: string) =>
  tenant<{ depreciation: AssetScheduleRow; posted_to_gl: boolean }>(
    `/assets/${id}/depreciate`,
    { method: "POST", body: { period_code } },
  );
export const disposeAsset = (
  id: string,
  body: { disposed_on?: string; proceeds?: number },
) =>
  tenant<{
    asset: Asset;
    net_book_value: number;
    proceeds: number;
    gain_loss: number;
  }>(`/assets/${id}/dispose`, { method: "POST", body });
