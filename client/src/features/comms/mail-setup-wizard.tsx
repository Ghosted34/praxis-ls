...
function CopyRow({ value }: { value: string }) {
  ...
  return (
    <div className="flex items-center gap-2 rounded-lg border border-border bg-muted/40 px-2.5 py-1.5">
      <code className="num min-w-0 flex-1 break-all text-[11px] text-foreground">
        {value}
      </code>
...
function DnsRow({
  ...
  const status =
    check.ok === true ? "OK" : check.ok === false ? "Missing" : "Unchecked";
  return (
    <div className="rounded-xl border border-border bg-card p-3.5">
      ...
      <div className="mt-2 space-y-1.5 text-[11px] leading-relaxed text-muted-foreground">
        {check.ok === true && check.records && check.records.length > 0 && (
          <div>
            {check.records.map((r) => (
              <div key={`${r.host}:${r.priority}`} className="num">
                {r.host}{" "}
        ...
        {check.ok === true && check.selector && (
          <div>
            Selector: <span className="num">{check.selector}</span>
          </div>
        )}
        ...
        {check.ok === false && check.suggest && check.suggest.length > 0 && (
          <div className="space-y-1">
            <div className="text-foreground">
              Add one of these TXT records to the domain:
            </div>
            {check.suggest.map((s) => (
              <CopyRow key={s} value={s} />
            ))}
          </div>
        )}
...
export function MailSetupWizard({
  ...
  const active = React.useMemo(
    () => (senders.data || []).filter((s) => s.is_active),
    [senders.data],
  );
  ...
  const dnsPassed = dns
    ? (["mx", "spf", "dkim"] as const).every(
        (k) => dns[k].ok === true || manual[k],
      )
    : false;
  const smtpPassed = smtp?.ok === true;
  const sendPassed = send?.ok === true;
  const passed: Record<StepId, boolean> = {
    sender: active.length > 0,
    dns: dnsPassed,
    smtp: smtpPassed,
    test: sendPassed,
  };
  ...
  const checkDns = React.useCallback(async () => {
    ...
    setManual({ mx: false, spf: false, dkim: false });
    try {
      setDns(await scapi.dnsCheckEmail(domain));
    } catch (err) {
      setDnsErr(err);
    } finally {
      setDnsBusy(false);
  ...
  const runSmtp = React.useCallback(async () => {
    setSmtpBusy(true);
    try {
      setSmtp(await scapi.testEmail());
    } catch (err) {
      setSmtp({ ok: false, error: errMsg(err) });
    } finally {
      setSmtpBusy(false);
    }
  }, []);
  ...
  const sendTest = React.useCallback(async () => {
    ...
    setSend(null);
    try {
      setSend(await scapi.testSendEmail({ to: to.trim() }));
    } catch (err) {
      setSend({ ok: false, error: errMsg(err) });
    } finally {
      setSendBusy(false);
    }
  }, [to]);
    ...
    <Modal
      ...
      description={
        summary
          ...
          : "Four steps to verified outbound email. Each check runs against your real configuration."
      }
      footer={
        summary ? (
          <Button
          ...
          <>
            ...
            <Button size="sm" onClick={continue_} disabled={continueDisabled}>
      ...
      {!summary && (
        <div className="mb-4">
          ...
          <div className="mt-2 h-1 overflow-hidden rounded-full bg-muted">
            <div
              className="h-full rounded-full bg-[rgb(var(--ok))] transition-all"
          ...
          {smtp && smtp.ok && (
            <div className="flex items-center gap-2 rounded-xl border border-border bg-card p-3">
              ...
              <span className="text-sm">
                ...
                {typeof smtp.smtp_host === "string" ? (
                  <span className="num"> · {smtp.smtp_host}</span>
                ) : null}
              </span>
            </div>
          )}
    ...
    <div className="rounded-xl border border-border bg-card p-3.5">
      ...
      <div className="mt-3 flex items-center justify-end gap-3">
        ...
        <Button
          ...
          loading={busy}
          disabled={
            busy || !f.purpose.trim() || !f.from.trim() || !f.name.trim()
          }
          onClick={add}
        >
          Add sender
