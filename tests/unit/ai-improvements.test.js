...
describe("entity card builders", () => {
  ...
  describe("our own corporate entities", () => {
    ...
    test("the card states the share capital as the stored figure it is", () => {
      const card = builder("corporate_entity").card({
        ...
        share_capital_paid_up: "100000000",
    ...
    test("partly-called capital is reported as such, not silently as the full figure", () => {
      const card = builder("corporate_entity").card({
        ...
        share_capital_paid_up: "2500000",
        share_capital_currency: "XAF",
      });
      expect(card.text).toContain(
        "share capital 10,000,000 XAF (2,500,000 paid up)",
      );
    });
    ...
    test("group position is stated either way, so silence never implies standalone", () => {
      const child = builder("corporate_entity").card({
        ...
        parent_code: "SBX",
        parent_name: "SmartBox SARL",
      });
      expect(child.text).toContain("subsidiary of SBX — SmartBox SARL");
      expect(builder("corporate_entity").card({ code: "SBX" }).text).toContain(
  ...
  test("confidential entities are tagged as confidential", () => {
    const confidential = entityCards.BUILDERS.filter(
      (b) => b.card({}).confidentiality === "confidential",
    ).map((b) => b.key);
...
describe("OHADA domain boost in retrieval", () => {
  ...
  const DOMAIN_KEYWORDS =
    /\b(ohada|syscohada|débours|disbursement|journal entry|posting|chart of accounts|VAT|TVA|tax declaration|withholding|précompte| acompte|IS\b|BIC|TVA|CNPS|NIU|patente|financial statement|bilan|compte de résultat|TAFIRE|GL|general ledger|double.entry|depreciation|amortissement)\b/i;
  ...
  test("accounting query boosts OHADA docs", () => {
    const hits = [
      ...
      { ref: "doc/OHADA_KB.md", title: "OHADA Accounting KB", sim: 0.7 },
    ];
    const boosted = boostDomainHits(
      hits,
      "How do I post a journal entry for VAT?",
    );
  ...
  test("boost is capped at 1.0", () => {
    const hits = [{ ref: "doc/OHADA_KB.md", title: "OHADA KB", sim: 0.95 }];
...
describe("feedback and preference prompt blocks", () => {
  test("negative feedback block format", () => {
    const feedback = [
      {
        comment: "Wrong account number for débours",
    ...
    const block = feedback.length
      ? "\n\nPATTERNS USERS DISLIKED (avoid these mistakes — users left specific feedback):\n" +
        feedback
          .map(
            (f, i) =>
              `  ${i + 1}. User said: "${f.comment}"${f.actions.length ? ` (on actions: ${f.actions.join(", ")})` : ""}`,
          )
          .join("\n")
      : "";
