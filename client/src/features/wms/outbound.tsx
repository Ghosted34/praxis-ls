...
function OrderDetail({
  ...
  const lines = useResource(
    () => api.getOutboundLines(order.outbound_order_id),
  ...
  async function toState(s: string) {
    ...
    setError(null);
    try {
      setOrder(await api.setOutboundStatus(order.outbound_order_id, s));
      onChanged();
    } catch (e) {
      setError(errMsg(e));
    } finally {
  ...
  async function flip(l: api.OutboundLine, key: "picked" | "packed") {
    setBusy(l.outbound_line_id + key);
    setError(null);
    try {
      await api.setOutboundLineFlags(
        order.outbound_order_id,
        l.outbound_line_id,
        { [key]: !l[key] },
      );
  ...
  async function add(e: React.FormEvent) {
    ...
    setError(null);
    try {
      await api.addOutboundLine(order.outbound_order_id, {
        inventory_item_id: addItem,
        qty: Number(addQty) || 1,
      });
      setAddItem("");
      setAddQty("1");
  ...
  const transitions: Transition[] = (TRANSITIONS[order.status] || []).map(
    (s) => ({
      to: s,
  ...
  return (
    <Modal
      ...
      onClose={onClose}
      size="xl"
      title={`Outbound · ${order.outbound_order_id.slice(0, 8)}`}
      description={
        order.dispatched_at
          ? `Dispatched ${dateFmt(order.dispatched_at)}`
          : undefined
      }
    >
      <div className="space-y-5">
        <StepBar
          steps={["CREATED", "PICKING", "PACKED", "DISPATCHED"]}
          ...
          columns={[
            {
              ...
              render: (l) =>
                l.inventory_item_id
                  ? invMap[l.inventory_item_id] ||
                    l.inventory_item_id.slice(0, 8)
                  : "—",
            },
            {
              label: "Qty",
              align: "right",
              render: (l) => <span className="num">{num(l.qty)}</span>,
            },
            {
              label: "Pick",
              align: "right",
              render: (l) => (
                <Toggle
                  ...
                  label="Pick"
                  doneLabel="Picked"
                  busy={busy === l.outbound_line_id + "picked"}
                  disabled={locked}
                  onClick={() => flip(l, "picked")}
                />
              ),
            },
            {
              ...
              render: (l) => (
                <Toggle
                  ...
                  label="Pack"
                  doneLabel="Packed"
                  busy={busy === l.outbound_line_id + "packed"}
                  disabled={!l.picked || locked}
                  onClick={() => flip(l, "packed")}
                />
              ),
            },
          ]}
        ...
        {canEditLines && (
          ...
          >
              ...
              >
                ...
                {inventory.map((i) => (
                  <option key={i.inventory_item_id} value={i.inventory_item_id}>
                    {i.description}
                    {i.sku ? ` (${i.sku})` : ""} · {num(i.qty_on_hand)} on hand
                  </option>
                ))}
              </Select>
            </Field>
            ...
            <Button type="submit" loading={busy === "add"} disabled={!addItem}>
              Add line
            </Button>
          </form>
        )}
...
export function OutboundPage() {
  ...
  const invMap = React.useMemo(() => {
    ...
    (inv.data || []).forEach((i) => {
      m[i.inventory_item_id] = i.description;
    });
    return m;
  }, [inv.data]);
  ...
  async function newOrder() {
    setCreating(true);
    try {
      const o = await api.createOutbound();
      orders.reload();
      setView(o);
    } catch (e) {
      reportActionError(e);
  ...
  const cols: Column<api.OutboundOrder>[] = [
    ...
    },
    {
      ...
      label: "Status",
      render: (o) => (
        <Pill tone={STATUS_TONE[o.status] || "mute"}>{o.status}</Pill>
      ),
    },
    {
      key: "created",
      label: "Created",
      render: (o) => <span className="num">{dateFmt(o.created_at)}</span>,
    },
    {
      key: "dispatched",
      label: "Dispatched",
      render: (o) =>
        o.dispatched_at ? (
          <span className="num">{dateFmt(o.dispatched_at)}</span>
        ) : (
  ...
  return (
    <section className={shell}>
      ...
      <DataList
        ...
        rowKey={(o) => o.outbound_order_id}
        empty={{
          title: "No outbound orders",
          hint: "Create an order to start picking.",
        }}
      />
      {view && (
        <OrderDetail
