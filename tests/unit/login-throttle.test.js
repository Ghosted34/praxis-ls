...
const { throttleFor } = require("../../src/modules/security/app_user/app_user.service");
...
describe("SEC-C3 — login cooldown curve", () => {
  ...
  it("doubles the wait past the free attempts", () => {
    const at = (n) => throttleFor({ failed_logins: n, last_failed_login_at: agoSeconds(0) }, NOW);
    expect(at(6)).toBe(2);
    expect(at(7)).toBe(4);
    expect(at(8)).toBe(8);
  ...
  it("DECAYS — the wait shrinks as time passes", () => {
    const user = (elapsed) => ({ failed_logins: 10, last_failed_login_at: agoSeconds(elapsed) });
  ...
  it("is inert when there is no recorded failure time", () => {
    ...
    expect(throttleFor({ failed_logins: 99, last_failed_login_at: null }, NOW)).toBe(0);
