...
describe("reading an itinerary", () => {
  it("shows each leg with where it came from", async () => {
    ...
    expect(
      await screen.findByRole("group", { name: /Leg 1: Main carriage/ }),
    ).toBeInTheDocument();
    expect(
      screen.getByRole("group", { name: /Leg 2: Final delivery/ }),
    ).toBeInTheDocument();
  ...
  it("explains an empty itinerary instead of showing a bare Add button", async () => {
    renderEditor([]);
    expect(
      await screen.findByText(/No structured itinerary yet/),
    ).toBeInTheDocument();
...
describe("editing", () => {
  ...
  it("reorders legs, which the previous editor could not do at all", async () => {
    ...
    await user.click(
      screen.getByRole("button", { name: /move pickup earlier/i }),
    );
    // Order is array order, and the group labels carry the position.
    expect(
      await screen.findByRole("group", { name: /Leg 1: Pickup/ }),
    ).toBeInTheDocument();
    expect(
      screen.getByRole("group", { name: /Leg 2: Main carriage/ }),
    ).toBeInTheDocument();
  });
  ...
  it("cannot move the first leg up or the last leg down", async () => {
    renderEditor([
      leg(),
      leg({ itinerary_leg_id: "leg-2", seq: 2, leg_type: "PICKUP" }),
    ]);
    await screen.findByRole("group", { name: /Main carriage/ });
    expect(
      screen.getByRole("button", { name: /move main carriage earlier/i }),
    ).toBeDisabled();
    expect(
      screen.getByRole("button", { name: /move pickup later/i }),
    ).toBeDisabled();
  });
  ...
  it("removes a leg", async () => {
    ...
    expect(
      screen.queryByRole("group", { name: /Pickup/ }),
...
describe("places come from the picker", () => {
  it("an origin is chosen from the catalogue, not typed", async () => {
    ...
    await waitFor(() =>
      expect(screen.getByRole("button", { name: "Origin" })).toHaveTextContent(
        "Douala",
      ),
    );
...
describe("saving", () => {
  ...
  it("puts a per-leg error on the leg it belongs to", async () => {
    ...
    fixtures.current.routes!["/operations/d-1/itinerary"] = {
      __error: {
        ...
        fields: {
          "legs.1.origin": ['"Nowhere" is not a place in the catalogue yet.'],
        },
      },
    };
    await user.click(screen.getByRole("button", { name: /save itinerary/i }));
    ...
    expect(
      within(second).getByText(/is not a place in the catalogue yet/),
    ).toBeInTheDocument();
    const first = screen.getByRole("group", { name: /Leg 1:/ });
    expect(
      within(first).queryByText(/is not a place in the catalogue yet/),
    ).not.toBeInTheDocument();
  });
  ...
  it("shows the structural-leg refusal as a callout, not as a field error", async () => {
    ...
    await screen.findByRole("group", { name: /Main carriage/ });
    await user.click(
      screen.getByRole("button", { name: /remove main carriage/i }),
    );
    ...
    expect(
      await screen.findByText("This service type needs a leg you removed"),
    ).toBeInTheDocument();
    expect(
      screen.getAllByText(/Main carriage is required by this service type/)
        .length,
    ).toBeGreaterThan(0);
    // …and no leg row is blamed for it.
    expect(
      screen.queryByRole("group", { name: /Leg 1:/ }),
    ).not.toBeInTheDocument();
  });
});
