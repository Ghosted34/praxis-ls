...
describe("shipmentMode", () => {
  it("trusts the dossier's service_type key over any text", () => {
    ...
    expect(shipmentMode({ service_key: "AIR_FREIGHT_EXPORT" }, "", "")).toBe(
      "air",
    );
    expect(shipmentMode({ service_key: "SEA_FREIGHT_IMPORT" }, "", "")).toBe(
      "sea",
    );
  });
  ...
  it("falls back to text only when there is no service type", () => {
    ...
    expect(shipmentMode({ service: "road haulage" }, "", "Douala Garoua")).toBe(
      "road",
    );
  });
...
describe("toLiveShipment", () => {
  ...
  it("shows the current milestone, falling back to the vessel", () => {
    ...
    expect(toLiveShipment({ ...base, current_milestone: null }).stage).toBe(
      "Maersk Cotonou",
    );
  });
...
describe("toLanes", () => {
  ...
  it("plots only shipments whose BOTH endpoints resolved", () => {
    ...
    expect(
      toLanes([{ ...withCoords, coords: { from: withCoords.coords.from } }]),
    ).toHaveLength(0);
  });
  ...
  it("rejects non-finite coordinates rather than projecting NaN", () => {
    const broken = {
      ...withCoords,
      coords: {
        from: { latitude: "x", longitude: 4.4 },
        to: withCoords.coords.to,
      },
    };
...
describe("greeting", () => {
  it("changes with the time of day", () => {
    expect(greeting(new Date(2026, 7, 4, 9), "Amara")).toBe(
      "Good morning, Amara",
    );
    expect(greeting(new Date(2026, 7, 4, 13), "Amara")).toBe(
      "Good afternoon, Amara",
    );
    expect(greeting(new Date(2026, 7, 4, 19), "Amara")).toBe(
      "Good evening, Amara",
    );
  });
...
describe("firstNameOf", () => {
  it("prefers the display name, then the email local part", () => {
    expect(firstNameOf({ display_name: "Amara Nkeng", email: "a@b.cm" })).toBe(
      "Amara",
    );
    expect(firstNameOf({ display_name: "", email: "grace@smartls.cm" })).toBe(
      "grace",
    );
...
describe("money helpers", () => {
  it("isLockedFinal accepts only locked FINAL invoices", () => {
    expect(isLockedFinal({ type: "FINAL", status: "POSTED_LOCKED" })).toBe(
      true,
    );
    expect(isLockedFinal({ type: "FINAL", status: "DRAFT" })).toBe(false);
    expect(isLockedFinal({ type: "PROFORMA", status: "ISSUED_LOCKED" })).toBe(
      false,
    );
  });
