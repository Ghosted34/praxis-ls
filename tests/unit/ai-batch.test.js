...
function makeClient(runOrder, actionKeyById) {
  return {
    query: async (sql, params = []) => {
      ...
      if (/FROM ai_feature_flag/.test(sql))
      ...
      if (/action_run_id FROM ai_action_run WHERE batch_id/.test(sql))
        return { rows: runOrder.map((id) => ({ action_run_id: id })) };
...
describe("AI batch confirm (grouped, halt-on-failure)", () => {
  test("all-success executes every action in order", async () => {
    ...
    const out = await orchestrator.confirmBatch({
      client,
      user,
      batchId: "b1",
      registry,
  ...
  test("blocked when governance gate closed", async () => {
    const client = {
      query: async (sql) =>
        /ai_feature_flag/.test(sql)
          ? { rows: [{ is_enabled: false }] }
          : { rows: [] },
    };
    await expect(
      orchestrator.confirmBatch({ client, user, batchId: "b1", registry }),
