...
const statusTone = (s?: string | null): Tone => {
  ...
  if (u.includes("REJECT") || u.includes("OVERDUE") || u.includes("REVERSED"))
    return "bad";
  return "mute";
};
...
const MORE = SECTIONS.filter(
  (s) => !["invoices", "proformas", "receivables", "journals"].includes(s.key),
).map((s) => ({ slug: s.key, label: s.label }));
...
const CHIP_META: Record<
  ChipKey,
  { label: string; noun: string; route: string }
> = {
  invoices: {
    label: "Invoices",
    noun: "New invoice",
...
function AgeingPanel({ a }: { a: api.Ageing | null }) {
  const rows = [
    ...
    {
      label: "60+ days",
      v: Number(a?.d61_90 || 0) + Number(a?.d90_plus || 0),
      tone: "--bad",
    },
  ];
          ...
          <div key={r.label} className="flex items-center gap-3 text-sm">
            ...
            <span className="h-2.5 flex-1 rounded-full bg-[rgb(var(--ink-3)/0.15)]">
              <span
                ...
                style={{
                  width: `${Math.round((Number(r.v || 0) / max) * 100)}%`,
                  background: `rgb(var(${r.tone}))`,
                }}
              />
            </span>
            <span className="num w-32 text-right font-medium">
              {money(r.v)}
            </span>
          </div>
        ))}
...
function CashPanel({
  ...
  const groups = (["BANK", "MOMO", "CASH"] as const)
    .map((kind) => {
              ...
              >
                <span className="flex items-center gap-2">
                  <span
                    ...
                    style={{ background: `rgb(var(${g.tone}))` }}
                  />
...
function CommandCenter() {
  ...
  const invoices = useListPaged<api.InvoiceRow>(
    "/final-invoices",
    at("invoices"),
  );
  const proformas = useListPaged<api.ProformaRow>(
    "/proformas/advances",
    at("proformas"),
  );
  const receipts = useListPaged<api.Receipt>("/receivables", at("receipts"));
  const journals = useListPaged<api.JournalRow>(
    "/journal-entries",
    at("journals"),
  );
      ...
      () => [
        {
          ...
          label: active.noun,
          primary: true,
          onSelect: () => navigate(active.route),
        },
      ],
  ...
  const invCols: Column<api.InvoiceRow>[] = [
    {
      ...
      label: "Invoice",
      render: (r) => (
        <span className="num font-medium text-primary-ink">
          {r.doc_number || r.invoice_id.slice(0, 8)}
        </span>
      ),
    },
    {
      ...
      render: (r) =>
        r.dossier_id ? (
          <span className="num text-muted-foreground">
            {dossierRef[r.dossier_id] || r.dossier_id.slice(0, 8)}
          </span>
        ) : (
          "—"
        ),
    ...
    { key: "due", label: "Due", render: (r) => dateFmt(r.payment_due_on) },
    {
      ...
      label: "Status",
      render: (r) => (
        <Pill tone={statusTone(r.status)}>{enumLabel(r.status)}</Pill>
      ),
    },
  ];
  const pfCols: Column<api.ProformaRow>[] = [
      ...
      render: (r) => (
        <span className="num font-medium text-primary-ink">
          {r.created_at
            ...
            : `Advance ${r.advance_id.slice(0, 8)}`}
        </span>
      ),
    },
    ...
    },
    {
      ...
      label: "Applied",
      className: "num text-right",
      render: (r) => money(r.applied_amount),
    },
    {
      key: "status",
      label: "Status",
      render: (r) => (
        <Pill tone={statusTone(r.status)}>{enumLabel(r.status || "OPEN")}</Pill>
      ),
    },
  ];
  const rcCols: Column<api.Receipt>[] = [
    {
      key: "received",
      label: "Received",
      render: (r) => <span className="num">{dateFmt(r.received_on)}</span>,
    },
    { key: "client", label: "Client", render: (r) => clientOf(r.client_id) },
    {
      key: "method",
      label: "Method",
      render: (r) => <Pill tone="mute">{enumLabel(r.method)}</Pill>,
    },
    {
      key: "amt",
      label: "Amount · XAF",
  ...
  const jnCols: Column<api.JournalRow>[] = [
    ...
    { key: "ref", label: "Source ref", render: (r) => r.source_doc_ref || "—" },
    {
      key: "source",
      label: "Source",
      render: (r) => <Pill tone="mute">{enumLabel(r.source || "—")}</Pill>,
    },
    {
      key: "status",
      label: "Status",
    ...
    <section className={shell}>
      ...
      <header className="mb-5 border-b border-border pb-4">
        ...
        <p className="mt-1 max-w-2xl text-sm text-muted-foreground">
          Every money event posts to the SYSCOHADA ledger at source.
          Disbursement never inflate turnover.
        </p>
        <div className="mt-4 flex flex-wrap items-center justify-between gap-3">
          <div className="chips">
            {MORE.map((m) => (
              <button
                key={m.slug}
                onClick={() => navigate(`/finance/${m.slug}`)}
                className="chip"
              >
                {m.label}
              </button>
          ...
          <Button className="md:hidden" onClick={() => navigate(active.route)}>
            {active.noun}
          </Button>
        </div>
      </header>
      ...
      <div
        className={`mb-5 flex flex-wrap items-center justify-between gap-4 rounded-2xl border p-4 shadow-sm ${t?.balanced ? "border-[rgb(var(--ok))]/30 bg-[rgb(var(--ok)/0.07)]" : "border-[rgb(var(--warn))]/30 bg-[rgb(var(--warn)/0.07)]"}`}
      >
        <div className="flex items-center gap-3">
          <span
            className={`grid h-9 w-9 place-items-center rounded-lg ${t?.balanced ? "bg-[rgb(var(--ok)/0.18)] text-[rgb(var(--ok))]" : "bg-[rgb(var(--warn)/0.18)] text-[rgb(var(--warn))]"}`}
          >
            {t?.balanced ? "✓" : "!"}
          </span>
          <div>
            ...
            <div className="micro">
              Σ Débit = Σ Crédit · posted from validated journals
            </div>
          </div>
        </div>
        <div className="flex items-center gap-8">
          <div className="text-right">
            <div className="micro uppercase tracking-wide">Total débit</div>
            <div className="num font-display text-lg">{money(t?.debit)}</div>
          </div>
          <div className="text-right">
            <div className="micro uppercase tracking-wide">Total crédit</div>
            <div className="num font-display text-lg">{money(t?.credit)}</div>
          </div>
        </div>
      </div>
      ...
      {chip === "invoices" && (
        <DataList
          ...
          onRowClick={() => navigate("/finance/invoices")}
          empty={
            search
              ? {
      ...
      {chip === "proformas" && (
        <DataList
          ...
          loading={proformas.loading}
          rowKey={(r) => r.advance_id}
          onRowClick={() => navigate("/finance/proformas")}
          empty={
            search
              ? {
                  title: "No proformas match",
                  hint: `Nothing matches “${search}”.`,
                }
              : {
                  title: "No proformas",
                  hint: "Raise a proforma / advance request.",
                }
          }
        />
      )}
      {chip === "receipts" && (
        <DataList
          ...
          onRowClick={() => navigate("/finance/receivables")}
          empty={
            search
              ? {
              ...
              : { title: "No receipts", hint: "Log a customer payment." }
          }
        />
      )}
      {chip === "journals" && (
        <DataList
          ...
          rowKey={(r) => r.entry_id}
          onRowClick={() => navigate("/finance/journals")}
          empty={
            search
              ? {
              ...
              : {
                  ...
                  hint: "Postings land here as documents are locked.",
                }
          }
        />
      )}
