...
const client = { query: jest.fn().mockResolvedValue({ rows: [{ email: "u@x.com", full_name: "U" }] }) };
...
test("security notification pushes (unconditional), passing the tenant client", async () => {
  await svc.notify(client, { userId: "u", eventTypeKey: "auth.password_reset_completed", title: "Sec" });
  expect(push.sendToUser).toHaveBeenCalledTimes(1);
  expect(push.sendToUser).toHaveBeenCalledWith(client, expect.objectContaining({ user_id: "u", url: "/notifications" }));
});

test("delivered non-security notification pushes", async () => {
  repo.isChannelEnabled.mockImplementation(async (_c, _u, ch) => ch === "IN_APP");
  await svc.notify(client, { userId: "u", eventTypeKey: "invoice.posted", title: "Inv" });
  expect(push.sendToUser).toHaveBeenCalledTimes(1);
});

test("in-app suppressed by preference → no push", async () => {
  repo.isChannelEnabled.mockResolvedValue(false); // in-app off
  const out = await svc.notify(client, { userId: "u", eventTypeKey: "invoice.posted", title: "Inv" });
  expect(out).toBeNull();
