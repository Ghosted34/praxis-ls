...
function CapabilityForm({
  ...
  async function submit(e: React.FormEvent) {
    ...
    try {
      if (editing && cap)
        await tenant(`/capabilities/${cap.capability_id}`, {
          method: "PATCH",
          body: { name },
        });
      else
        await tenant("/capabilities", { method: "POST", body: { code, name } });
      onSaved();
      onClose();
    } catch (err) {
      setError(errMsg(err));
      ...
      <form className="space-y-4" onSubmit={submit}>
        ...
        <div className="flex justify-end gap-2 pt-2">
          ...
          <Button type="submit" loading={busy} disabled={busy || !name}>
            {editing ? "Save changes" : "Create capability"}
          </Button>
        </div>
      </form>
    </Modal>
...
export function CapabilitiesPage() {
  ...
  const [form, setForm] = React.useState<{ cap: Capability | null } | null>(
    null,
  );
  ...
  const columns: Column<Capability>[] = [
    {
      key: "code",
      label: "Code",
      render: (r) => <Pill tone="blue">{r.code}</Pill>,
    },
    {
      key: "name",
      label: "Name",
  ...
  return (
    <section className={shell}>
      ...
      <DataList
        ...
        onRowClick={(r) => setForm({ cap: r })}
        empty={{
          title: "No capabilities",
          hint: "The four standard capabilities are normally seeded.",
        }}
      />
      {form && (
        <CapabilityForm
      ...
      {del && (
        <ConfirmDelete
          ...
          path={`/capabilities/${del.capability_id}`}
          onClose={() => setDel(null)}
          onDone={reload}
        />
      )}
