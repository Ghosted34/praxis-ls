...
async function deliveryNoteData(client, recordId) {
  const { rows } = await client.query(
    `SELECT dn.*, d.ref AS dossier_ref, COALESCE(cm.name, cm.legal_name) AS client_name, au.full_name AS issued_by_name
...
async function transitOrderData(client, recordId) {
  const { rows } = await client.query(
    `SELECT t.*, d.ref AS dossier_ref, COALESCE(cm.name, cm.legal_name) AS client_name
