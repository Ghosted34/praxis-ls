...
- **Still partial / nice-to-have:**
  - `email-signatures` — the tenant brand template is built on the settings store; the _per-user_
...
> ⚠️ **Read "built" carefully (corrected 2026-07-19).** This section used to conflate _a screen exists at
> that route_ with _the screen is functional_. Several entries below were **read-only `ResourceList`
...
FinanceHub-shaped (overview + tab bar + section map), _not_ the shared `TabbedHub`, which expects each page
