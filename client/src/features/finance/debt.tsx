...
function DebtForm({
  ...
  const [f, setF] = React.useState({
    ...
    lender_kind: row?.lender_kind ?? "BANK",
    ...
    try {
      if (isNew) {
        await api.createDebt({
          ...
          lender_kind: f.lender_kind,
          lender_name: f.lender_name || undefined,
          principal: Number(f.principal),
          interest_rate:
            f.interest_rate === "" ? undefined : Number(f.interest_rate),
          started_on: f.started_on || undefined,
    ...
    >
      <form className="space-y-4" onSubmit={submit}>
        <div className="grid gap-4 sm:grid-cols-2">
          ...
          <Field label="Lender name" className="sm:col-span-2">
            <Input
              ...
              placeholder="Ecobank / director name"
            />
          </Field>
        ...
        <FormButtons
          busy={busy}
          disabled={
            busy || (isNew && !(f.entity_id && Number(f.principal) > 0))
          }
          onCancel={onClose}
          saveLabel={isNew ? "Create facility" : "Save changes"}
        />
      </form>
    </Modal>
  );
...
function RepayForm({
  ...
  async function submit(e: React.FormEvent) {
    ...
    try {
      await api.repayDebt(debt.debt_engagement_id, {
        ...
        principal_part:
          f.principal_part === "" ? undefined : Number(f.principal_part),
        interest_part:
          f.interest_part === "" ? undefined : Number(f.interest_part),
  ...
  return (
    <Modal
      ...
      description="Split principal and interest; posts the cash and interest entries."
    >
      <form className="space-y-4" onSubmit={submit}>
        <div className="grid gap-4 sm:grid-cols-2">
          ...
          <Field label="Interest part">
            <Input
              ...
              step="0.01"
              className="num text-right"
              value={f.interest_part}
              onChange={(e) => set("interest_part", e.target.value)}
            />
          </Field>
        </div>
        {error && <ErrorState message={error} />}
        <FormButtons
          ...
          disabled={
            busy ||
            !(Number(f.principal_part) > 0 || Number(f.interest_part) > 0)
          }
          onCancel={onClose}
          saveLabel="Post repayment"
        />
      </form>
    </Modal>
  );
...
function DebtDrawer({
  ...
  const d = useResource(
    () => api.getDebt(debt.debt_engagement_id),
    [debt.debt_engagement_id],
  );
  const x = d.data;
  return (
    <Modal
      ...
      onClose={onClose}
      size="lg"
      title={`${debt.lender_name || debt.lender_kind || "Facility"} · ${money(debt.principal)}`}
        ...
        <div className="space-y-4">
          <div className="grid gap-3 sm:grid-cols-3">
            <div className="rounded-lg border border-border bg-card/40 px-3.5 py-2.5">
              <div className="micro mb-1">Principal</div>
              <div className="num text-lg font-medium">
                {money(x.principal)}
              </div>
            </div>
            <div className="rounded-lg border border-border bg-card/40 px-3.5 py-2.5">
              <div className="micro mb-1">Repaid</div>
              <div className="num text-lg font-medium">
                {money(x.repaid?.principal)}
              </div>
            </div>
            <div className="rounded-lg border border-border bg-card/40 px-3.5 py-2.5">
...
export function DebtPage() {
  const { rows, error, loading, reload } =
    useList<api.DebtEngagement>("/financing");
  const [editing, setEditing] = React.useState<
    api.DebtEngagement | "new" | null
  >(null);
  ...
  const columns: Column<api.DebtEngagement>[] = [
    ...
    },
    {
      ...
      label: "Kind",
      render: (r) => (
        <Pill tone="mute">{(r.lender_kind || "—").replace(/_/g, " ")}</Pill>
      ),
    },
    {
      key: "principal",
    ...
    },
    {
      ...
      className: "num text-right",
      render: (r) =>
        r.interest_rate != null ? `${num(r.interest_rate)}%` : "—",
    },
    ...
    },
    {
      ...
      render: (r) => (
        ...
        <RowActions>
          ...
          {r.status === "ACTIVE" && (
            <Button size="sm" variant="outline" onClick={() => setRepay(r)}>
              Repay
            </Button>
          )}
        </RowActions>
  ...
  return (
    <section className={shell}>
      <PageHeader
        ...
        title="Financing"
        description="Loan facilities — drawdowns, repayments and outstanding principal."
        action={<Button onClick={() => setEditing("new")}>New facility</Button>}
      />
      <KpiRow>
      ...
      <DataList
        ...
        error={error}
        loading={loading}
        rowKey={(r) => r.debt_engagement_id}
        onRowClick={(r) => setView(r)}
        empty={{
          title: "No financing yet",
          hint: "Register a loan or credit facility to track drawdowns and repayments.",
        }}
      />
      {editing !== null && (
        <DebtForm
