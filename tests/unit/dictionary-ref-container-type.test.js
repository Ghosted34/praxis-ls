...
const {
  schemas,
} = require("../../src/modules/master/financial_dictionary/financial_dictionary.validator");
...
describe("dictionary_ref refCreate — container types", () => {
  test("the kind is accepted at all (it used to be rejected by the enum)", () => {
    const r = schemas.refCreate.safeParse({
      ...base,
      extra: { teu: 2.5, size: "50HC", family: "DRY" },
    });
  ...
  test("a container type with no `extra` at all is refused on both fields", () => {
    ...
    expect(errPaths(r)).toEqual(
      expect.arrayContaining(["extra.teu", "extra.size"]),
    );
  });
  ...
  test("the requirement is CONTAINER_TYPE-only — other kinds keep their free `extra`", () => {
    ...
    expect(r.success).toBe(true);
    const scoped = schemas.refCreate.safeParse({
      kind: "SUBCATEGORY",
      code: "PORT",
      name_fr: "Port",
      extra: { category: "debours" },
    });
...
describe("dictionary_ref refUpdate", () => {
  test("`extra` is patchable — a typo'd TEU was otherwise unfixable in the UI", () => {
    const r = schemas.refUpdate.safeParse({
