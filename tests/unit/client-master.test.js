...
const {
  kycComplete,
  creditStatus,
} = require("../../src/modules/master/client_master/client_master.rules");
...
describe("creditStatus", () => {
  it("null limit = unlimited (always within)", () => {
    const s = creditStatus(
      { credit_limit: null, cached_receivables: 5000000 },
      1000000,
    );
  ...
  it("computes available and within-limit", () => {
    const s = creditStatus(
      { credit_limit: 10000000, cached_receivables: 6000000 },
      2000000,
    );
  ...
  it("flags when exposure exceeds the limit", () => {
    const s = creditStatus(
      { credit_limit: 10000000, cached_receivables: 9000000 },
      2000000,
    );
