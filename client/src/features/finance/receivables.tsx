...
/* ── create receipt ── */
const KIND_FOR_METHOD: Record<string, string> = {
  BANK: "BANK",
  CHEQUE: "BANK",
  MOBILE_MONEY: "MOMO",
  CASH: "CASH",
...
function ReceiptForm({
  ...
  const accounts = (treasury || []).filter(
    (t) => t.kind === KIND_FOR_METHOD[f.method],
  );
    ...
    >
      <form className="space-y-4" onSubmit={submit}>
          ...
          {wantsAccount && (
            ...
            >
              ...
              >
                ...
                {accounts.map((t) => (
                  <option
                    key={t.treasury_account_id}
                    value={t.treasury_account_id}
                  >
                    {t.label} · {t.coa_code}
                  </option>
                ))}
        ...
        <FormButtons
          ...
          disabled={
            !(Number(f.amount) > 0) ||
            (wantsAccount && !f.treasury_account_id) ||
            busy
          }
          onCancel={onClose}
...
function PostForm({
  ...
  async function submit(e: React.FormEvent) {
    ...
    setError(null);
    try {
      await api.postReceipt(receipt.receipt_id, {
  ...
  return (
    <Modal
      ...
      onClose={onClose}
      title="Post receipt"
      description={`Allocate ${money(receipt.amount)} FIFO to open invoices and book the cash entry.`}
    >
      <form className="space-y-4" onSubmit={submit}>
        <div className="grid gap-4 sm:grid-cols-2">
          ...
          <Field label={tr("Source doc ref")} className="sm:col-span-2">
            <Input
              ...
              placeholder="Bank slip / transfer ref"
            />
          </Field>
        </div>
        {error && <ErrorState message={error} />}
...
function ReceiptDrawer({
  ...
  const d = useResource(
    () => api.getReceipt(receipt.receipt_id),
    [receipt.receipt_id],
  );
  const rec = d.data;
  ...
  const invoiceNo = React.useMemo(() => {
    const m: Record<string, string> = {};
    (invoices || []).forEach((iv) => {
      if (iv.doc_number) m[iv.invoice_id] = iv.doc_number;
    });
    return m;
  }, [invoices]);
  return (
    <Modal
      ...
      onClose={onClose}
      size="lg"
      title={`Receipt · ${money(receipt.amount)}`}
          ...
          <div>
            ...
            {(rec.allocations || []).length ? (
              ...
              <span className="micro">
                Not yet allocated — post the receipt to allocate it against open
                invoices.
              </span>
            )}
          </div>
...
export function ReceivablesPage() {
  ...
  const columns: Column<api.Receipt>[] = [
    ...
    },
    {
      key: "client_id",
      label: "Client",
      render: (r) => (r.client_id ? clientName[r.client_id] || "—" : "—"),
    },
    {
      key: "method",
      label: "Method",
    ...
    },
    {
      key: "status",
      label: "Status",
      render: (r) => <Pill tone={tone(r.status)}>{enumLabel(r.status)}</Pill>,
    },
    {
      key: "_a",
      label: "",
    ...
    <section className={shell}>
        ...
        <div className="mb-5 rounded-lg border border-[rgb(var(--warn))]/40 bg-[rgb(var(--warn))]/10 px-4 py-3">
          ...
          <ol className="space-y-1">
            {(dunning.data?.reminders || []).slice(0, 6).map((rm) => (
              ...
              >
                <span>
                  {rm.doc_number || rm.invoice_id.slice(0, 8)}
                  {rm.client_id ? ` · ${clientName[rm.client_id] || ""}` : ""}
                </span>
                <span className="flex items-center gap-3">
                  <span className="micro">{rm.days_overdue}d overdue</span>
                  <span className="num">{money(rm.outstanding)}</span>
                </span>
              </li>
            ))}
          </ol>
      ...
      <DataList
        ...
        empty={{
          ...
          hint: "Log a customer payment to start allocating against invoices.",
        }}
      />
      ...
      {view && (
        <ReceiptDrawer
          receipt={view}
          clientLabel={view.client_id ? clientName[view.client_id] || "—" : "—"}
