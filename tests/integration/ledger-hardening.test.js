...
d("ledger hardening triggers (real Postgres)", () => {
  ...
  beforeAll(async () => {
    ...
    const j = await pool.query(
      "SELECT journal_id FROM journal WHERE entity_id = $1 LIMIT 1",
      [entityId],
    );
    ...
    if (!journalId)
      throw new Error(
        `No journal seeded for TEST_ENTITY_ID=${entityId}. The CI fixture step seeds BQ/VT/AC/OD.`,
      );
    if (!period)
      throw new Error(
        `No OPEN accounting_period for TEST_ENTITY_ID=${entityId}. The CI fixture step seeds one covering today.`,
      );
  });
  afterAll(async () => {
  ...
  async function rawValidatedEntry({ periodId, entryDate, lines }) {
    ...
    try {
      ...
      await c.query(
        "UPDATE journal_entry SET status='validated', validated_at=now() WHERE entry_id=$1",
        [entryId],
      );
  ...
  it("no-compensation trigger rejects an account on both sides (§23.6)", async () => {
    await expect(
      rawValidatedEntry({
        ...
        lines: [
          { account_code: "5211", debit: 100, credit: 0 },
          { account_code: "5211", debit: 0, credit: 100 },
        ],
      }),
    ...
    try {
      const r = await c.query(
        "INSERT INTO accounting_period (entity_id, code, starts_on, ends_on, status) " +
          "VALUES ($1, 'TEST-FROZEN-" +
          Date.now() +
          "', '2099-01-01','2099-01-31','FROZEN') RETURNING period_id",
        [entityId],
      );
  ...
  it("double-reversal is rejected (service 409 + unique index backstop)", async () => {
    ...
    const posted = await withClient((c) =>
      service.post(c, {
        journalId,
        entityId,
        entryDate: period.starts_on,
        sourceDocRef: "test:reversal",
        lines: [
          { account_code: "5211", debit: 100, credit: 0 },
          { account_code: "4191", debit: 0, credit: 100 },
        ],
