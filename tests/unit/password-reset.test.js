...
describe("requestPasswordReset — no account enumeration", () => {
  test("active user: creates a token and sends the email", async () => {
    ...
    const out = await svc.requestPasswordReset(fakeClient(), {
      email: "user@acme.com",
      ip: "1.1.1.1",
      origin: "https://acme.praxisls.com",
    });
    ...
    expect(mail.html).toMatch(
      /https:\/\/acme\.praxisls\.com\/reset-password\?token=/,
    );
  });
...
describe("resetPassword — token validation + force logout", () => {
  ...
  test("valid token: sets password, marks used, kills ALL sessions", async () => {
    repo.findResetByHash.mockResolvedValue({
      reset_id: "r-1",
      user_id: "u-1",
      expires_at: future,
      used_at: null,
    ...
    const out = await svc.resetPassword(fakeClient(), {
      token: "raw-token",
      newPassword: "Zt9$vQ2m!pLwR",
      ip: "1.1.1.1",
    });
    expect(out).toEqual({ reset: true });
    expect(policy.assertStrongPassword).toHaveBeenCalledWith("Zt9$vQ2m!pLwR", {
      email: "user@acme.com",
    });
    expect(repo.setPasswordHash).toHaveBeenCalledTimes(1);
    expect(repo.markResetUsed).toHaveBeenCalledWith(expect.anything(), "r-1");
    expect(repo.killAllSessionsForUser).toHaveBeenCalledWith(
      expect.anything(),
      "u-1",
      "u-1",
    );
  ...
  test("weak password is rejected BEFORE any password write", async () => {
    ...
    repo.getUserSafe.mockResolvedValue(ACTIVE_USER);
    const err = Object.assign(new Error("weak"), {
      code: "WEAK_PASSWORD",
      status: 422,
    });
    policy.assertStrongPassword.mockRejectedValueOnce(err);
    await expect(
      svc.resetPassword(fakeClient(), { token: "raw", newPassword: "weak" }),
