...
describe("Compliance Checker rules (MOD-65)", () => {
  test("catalogue + severities", () => {
    expect(rules.ruleKeys()).toEqual(
      expect.arrayContaining([
        "cost_entry.missing_proof",
        "procurement.unmatched",
        "regie.aged_unjustified",
        "disbursement.tax_violation",
      ]),
    );
...
describe("Compliance Checker run (MOD-65)", () => {
  test("raises a RED flag for a débours-with-tax violation", async () => {
    const client = {
      query: async (sql, params = []) => {
        if (/FROM journal_line WHERE is_disbursement/.test(sql))
          return { rows: [{ line_id: "l1", entry_id: "e1" }] };
        if (/INTO compliance_flag/.test(sql))
          return {
            rows: [
              {
                flag_id: "f1",
        ...
        if (/FROM event_type/.test(sql))
          return { rows: [{ is_security_critical: false }] };
        return { rows: [] }; // BEGIN/COMMIT/DELETE/other scans/event_log/audit
      },
    };
    const out = await service.run(client, {
      rules: ["disbursement.tax_violation"],
    });
  ...
  test("clean run when no offenders", async () => {
    const client = {
      query: async (sql) =>
        /FROM event_type/.test(sql)
          ? { rows: [{ is_security_critical: false }] }
          : { rows: [] },
    };
    const out = await service.run(client, {
      rules: ["regie.aged_unjustified"],
    });
