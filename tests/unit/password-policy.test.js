...
const reject = async (pw, ctx) => {
  await expect(policy.assertStrongPassword(pw, ctx)).rejects.toMatchObject({ status: 422 });
};

describe("assertStrongPassword — complexity rules", () => {
  beforeEach(() => {
    // Default: HIBP says "not breached" so complexity is what's under test.
    global.fetch = jest.fn().mockResolvedValue({ ok: true, text: async () => "ABCDE:0\n" });
  });
  ...
  test("accepts a strong password", async () => {
    await expect(policy.assertStrongPassword("Zt9$vQ2m!pLwR", { email: "a@b.com" }))
...
describe("assertStrongPassword — HIBP breach check (k-anonymity)", () => {
  ...
  test("sends ONLY the 5-char SHA-1 prefix, never the full hash", async () => {
    const fetchMock = jest.fn().mockResolvedValue({ ok: true, text: async () => "0000:0\n" });
  ...
  test("rejects a password whose suffix appears in the HIBP range", async () => {
    ...
    const suffix = crypto.createHash("sha1").update(pw).digest("hex").toUpperCase().slice(5);
    global.fetch = jest.fn().mockResolvedValue({ ok: true, text: async () => `${suffix}:42\n` });
    await expect(policy.assertStrongPassword(pw)).rejects.toMatchObject({ code: "BREACHED_PASSWORD" });
  });

  test("fails OPEN when HIBP is unreachable (outage must not block reset)", async () => {
    global.fetch = jest.fn().mockRejectedValue(new Error("network down"));
    await expect(policy.assertStrongPassword("Zt9$vQ2m!pLwR"))
