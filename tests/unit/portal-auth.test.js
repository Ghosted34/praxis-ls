...
const mockCalls = {
  bumpFailed: [], touchLogin: [], setPassword: [], setStatus: [],
  createInvite: [], invalidateInvites: [], markInviteUsed: [], insert: [], mail: [],
};

const mockByEmail = (e) => mockUsers.find((u) => u.email === String(e || "").trim().toLowerCase()) || null;
...
jest.mock("../../src/modules/portal_auth/portal_auth.repo", () => ({
  ...
  insert: async (c, { email, passwordHash, fullName }) => {
    const row = { portal_user_id: `pu-${mockUsers.length + 1}`, email, password_hash: passwordHash, full_name: fullName, status: "ACTIVE" };
    mockUsers.push(row);
  ...
  touchLogin: async (c, id) => { mockCalls.touchLogin.push(id); },
  bumpFailed: async (c, id) => { mockCalls.bumpFailed.push(id); },
  list: async () => mockUsers,
  createInvite: async (c, inv) => { mockInvites.push({ invite_id: `inv-${mockInvites.length + 1}`, used_at: null, ...inv }); mockCalls.createInvite.push(inv); },
  invalidateInvites: async (c, id) => { mockCalls.invalidateInvites.push(id); for (const i of mockInvites) if (i.portalUserId === id) i.used_at = new Date().toISOString(); },
  findInviteByHash: async (c, hash) => {
    const i = mockInvites.find((x) => x.tokenHash === hash);
    if (!i) return null;
    const u = mockById(i.portalUserId);
    return { ...i, portal_user_id: i.portalUserId, expires_at: i.expiresAt, email: u ? u.email : null };
  },
...
const { portalAuth } = require("../../src/modules/portal_auth/portal_auth.middleware");
...
describe("portal auth (TC-C10)", () => {
  beforeEach(async () => {
    if (!goodHash) goodHash = await argon2.hash(GOOD, { type: argon2.argon2id });
    mockUsers = [{ portal_user_id: "pu-1", email: "client@acme.example", password_hash: goodHash, full_name: "Ada Client", status: "ACTIVE" }];
    mockInvites = [];
  ...
  describe("login", () => {
    it("issues a token for correct credentials and records the login", async () => {
      const out = await svc.login(client, { email: "client@acme.example", password: GOOD });
      expect(out.access_token).toBeTruthy();
      expect(out.portal_user).toEqual({ portal_user_id: "pu-1", email: "client@acme.example", full_name: "Ada Client" });
    ...
    it("counts a wrong password as a failed attempt", async () => {
      const err = await rejection(svc.login(client, { email: "client@acme.example", password: "wrong-but-strong-9!" }));
    ...
    it("gives an unknown email and a wrong password the SAME error", async () => {
      ...
      const unknown = await rejection(svc.login(client, { email: "nobody@acme.example", password: GOOD }));
      const wrong = await rejection(svc.login(client, { email: "client@acme.example", password: "wrong-but-strong-9!" }));
    ...
    it("refuses a disabled account with that same error, and does not count an attempt", async () => {
      ...
      const err = await rejection(svc.login(client, { email: "client@acme.example", password: GOOD }));
      expect(err.code).toBe("BAD_CREDENTIALS");
      expect(mockCalls.bumpFailed).toHaveLength(0);
      expect(mockCalls.touchLogin).toHaveLength(0);
    });
  ...
  describe("the portal/staff token boundary", () => {
    ...
    it("stamps typ:portal and carries identity only", async () => {
      const { access_token } = await svc.login(client, { email: "client@acme.example", password: GOOD });
    ...
    it("refuses a STAFF access token", async () => {
      ...
      // typ.
      const staff = jwt.sign({ sub: "user-1", jti: "j", sid: "s", typ: "access" }, config.JWT_ACCESS_SECRET, { expiresIn: "15m" });
      const err = await rejection(Promise.resolve().then(() => svc.verifyToken(staff)));
      expect(err.code).toBe("INVALID_TOKEN");
      expect(err.status).toBe(401);
    });

    it("refuses a 2FA-pending token", async () => {
      const pending = jwt.sign({ sub: "user-1", typ: "2fa_pending" }, config.JWT_ACCESS_SECRET, { expiresIn: "5m" });
      const err = await rejection(Promise.resolve().then(() => svc.verifyToken(pending)));
      expect(err.code).toBe("INVALID_TOKEN");
    });

    it("refuses a token signed with the refresh secret", async () => {
      const forged = jwt.sign({ sub: "pu-1", typ: "portal" }, config.JWT_REFRESH_SECRET, { expiresIn: "2h" });
      const err = await rejection(Promise.resolve().then(() => svc.verifyToken(forged)));
      expect(err.code).toBe("INVALID_TOKEN");
    });
    ...
    it("distinguishes an expired portal token from an invalid one", async () => {
      ...
      const expired = jwt.sign({ sub: "pu-1", typ: "portal" }, config.JWT_ACCESS_SECRET, { expiresIn: "-1s" });
      const err = await rejection(Promise.resolve().then(() => svc.verifyToken(expired)));
      expect(err.code).toBe("TOKEN_EXPIRED");
      expect(err.status).toBe(401);
    });

    it("refuses a malformed token", async () => {
      const err = await rejection(Promise.resolve().then(() => svc.verifyToken("garbage")));
      expect(err.code).toBe("INVALID_TOKEN");
    });
  });
  ...
  describe("password policy on external accounts (SEC H6)", () => {
    it("rejects the exact passwords that used to pass", async () => {
      ...
      for (const weak of ["password", "12345678"]) {
        ...
        const err = await rejection(svc.createUser(client, { email: "new@acme.example", password: weak, fullName: "New" }));
    ...
    it("rejects a long password that is missing a character class", async () => {
      const err = await rejection(svc.createUser(client, { email: "new@acme.example", password: "aaaaaaaaaaaaaaaaaaaa", fullName: "New" }));
      expect(err.code).toBe("WEAK_PASSWORD");
    });

    it("rejects a password containing the email local part", async () => {
      const err = await rejection(svc.createUser(client, { email: "hauling@acme.example", password: "Hauling#Rig4422", fullName: "New" }));
      expect(err.code).toBe("WEAK_PASSWORD");
    ...
    it("consults the breach corpus and fails open when it is unreachable", async () => {
      ...
      await svc.createUser(client, { email: "new@acme.example", password: GOOD, fullName: "New" });
    ...
    it("refuses a duplicate email", async () => {
      const err = await rejection(svc.createUser(client, { email: "client@acme.example", password: GOOD, fullName: "Dup" }));
      expect(err.code).toBe("EMAIL_TAKEN");
      expect(err.status).toBe(409);
    });

    it("applies the same policy to an admin set-password", async () => {
      const err = await rejection(svc.setPassword(client, { id: "pu-1", password: "password" }));
      expect(err.code).toBe("WEAK_PASSWORD");
      expect(mockCalls.setPassword).toHaveLength(0);
    });

    it("404s a set-password for an unknown user before hashing anything", async () => {
      const err = await rejection(svc.setPassword(client, { id: "pu-nope", password: GOOD }));
  ...
  describe("invitations and recovery (0482)", () => {
    ...
    async function invite(email = "invitee@acme.example", fullName = "Ivy Invitee") {
      const out = await svc.inviteUser(client, { email, fullName, origin: "https://portal.example" });
      const link = mockCalls.mail[mockCalls.mail.length - 1].text.match(/token=([a-f0-9]+)/);
    ...
    it("creates a login for a brand-new invitee with an unusable password", async () => {
      ...
      const created = mockCalls.insert[0];
      expect(created.password_hash).toMatch(/^\$argon2id\$/);
      const err = await rejection(svc.login(client, { email: "invitee@acme.example", password: GOOD }));
      expect(err.code).toBe("BAD_CREDENTIALS");
    });
    ...
    it("keeps the login and the token when the mail fails, and says so", async () => {
      mockMailFails = true;
      const out = await svc.inviteUser(client, { email: "invitee@acme.example", fullName: "Ivy" });
    ...
    it("gives an invite a week and a reset thirty minutes", async () => {
      ...
      expect(out.emailed).toBe(true);
      const inviteTtl = new Date(mockInvites[0].expiresAt).getTime() - Date.now();
      expect(inviteTtl).toBeGreaterThan(6 * 24 * 3600 * 1000);

      mockInvites = [];
      await svc.requestReset(client, { email: "client@acme.example" });
      const resetTtl = new Date(mockInvites[0].expiresAt).getTime() - Date.now();
    ...
    it("answers a reset for an unknown email identically, and mints nothing", async () => {
      const unknown = await svc.requestReset(client, { email: "nobody@acme.example" });
      const known = await svc.requestReset(client, { email: "client@acme.example" });
    ...
    it("mints nothing for a disabled user's reset", async () => {
      mockUsers[0].status = "DISABLED";
      const out = await svc.requestReset(client, { email: "client@acme.example" });
    ...
    it("accepts a valid invite, sets the password and signs the user in", async () => {
      const { token } = await invite();
      const out = await svc.acceptInvite(client, { token, password: OTHER_GOOD });

      expect(jwt.verify(out.access_token, config.JWT_ACCESS_SECRET).typ).toBe("portal");
      expect(mockCalls.markInviteUsed).toHaveLength(1);

      // The password that was set is the password that now works.
      const login = await svc.login(client, { email: "invitee@acme.example", password: OTHER_GOOD });
    ...
    it("burns the link — the same token cannot be used twice", async () => {
      ...
      const err = await rejection(svc.acceptInvite(client, { token, password: "Another#Pass99" }));
    ...
    it("gives unknown, used and expired tokens one indistinguishable error", async () => {
      ...
      mockInvites.find((i) => i.tokenHash && !i.used_at).expiresAt = new Date(Date.now() - 1000);

      const errs = [
        await rejection(svc.acceptInvite(client, { token: "deadbeef", password: OTHER_GOOD })),
        await rejection(svc.acceptInvite(client, { token: used, password: OTHER_GOOD })),
        await rejection(svc.acceptInvite(client, { token: expiring, password: OTHER_GOOD })),
        await rejection(svc.acceptInvite(client, { token: "", password: OTHER_GOOD })),
      ];
      for (const e of errs) {
    ...
    it("checks the token before the password, so policy errors leak no token state", async () => {
      ...
      // once existed.
      const err = await rejection(svc.acceptInvite(client, { token: "deadbeef", password: "password" }));
      expect(err.code).toBe("INVALID_INVITE");
    });
    ...
    it("does not consume the invite when the chosen password is rejected", async () => {
      ...
      const { token } = await invite();
      const err = await rejection(svc.acceptInvite(client, { token, password: "password" }));
      expect(err.code).toBe("WEAK_PASSWORD");
      expect(mockCalls.markInviteUsed).toHaveLength(0);

      const ok = await svc.acceptInvite(client, { token, password: OTHER_GOOD });
    ...
    it("refuses an invite whose user was disabled after it was sent", async () => {
      ...
      const err = await rejection(svc.acceptInvite(client, { token, password: OTHER_GOOD }));
      expect(err.code).toBe("INVALID_INVITE");
      expect(mockCalls.setPassword).toHaveLength(0);
    });
  });
  ...
  describe("the portalAuth middleware — where the grant is actually enforced", () => {
    ...
    it("rejects a non-bearer authorization header", async () => {
      const req = { headers: { authorization: "Basic Zm9vOmJhcg==" }, identityDb: (fn) => fn(client), tenantDb: (fn) => fn(client) };
    ...
    it("refuses a valid token that has no grant for the portal it asked for", async () => {
      ...
      mockAccessGrant = { allowed: false, grant: null };
      const err = await rejection(run(portalAuth("AUDITOR"), reqFor(await tokenFor())));
      expect(err.code).toBe("PORTAL_FORBIDDEN");
      expect(err.status).toBe(403);
    });

    it("attaches the freshly-read grant when access is allowed", async () => {
      mockAccessGrant = { allowed: true, grant: { client_id: "cl-9", portal: "CLIENT" } };
