// ai:none — universal reference data from @praxis/shared (no service, no repo, no tenant rows), and its routes carry NO requirePermission BY DESIGN: gating the ISO 20275 / OHADA legal-form list behind one master module would lock the other side's users out of their own picker. `services/ai/action-authz` fails CLOSED on a null required_permission, so a manifest here would advertise actions the runtime refuses, and picking a grant instead would enforce a DIFFERENT rule on the AI path from the HTTP one — which that file calls worse than enforcing none. The copilot gets this vocabulary through the modules that use it.
"use strict";
