...
`baseline_due` answers _"were we ever on plan?"_. `planned_due` is what the client
...
6. **Floor breach.** If Σ floors > horizon, the locked date is unreachable →
   apply the configured breach policy (§6.2). The schedule is _not_ silently
   made feasible.
...
8. **Attribution.** Slip is charged to the `owner_tier` of the stage that
   _slipped_, not the stage that revealed it, with a `cause_reason_code`
...
### 7.1 SEA_FREIGHT_IMPORT — _Fret Maritime Import_
...
### 7.2 SEA_FREIGHT_EXPORT — _Fret Maritime Export_
...
### 7.3 AIR_FREIGHT_IMPORT — _Fret Aérien Import_
...
### 7.4 AIR_FREIGHT_EXPORT — _Fret Aérien Export_
...
### 7.5 HINTERLAND_TRANSIT — _Transit Hinterland_
...
### 7.6 INLAND_TRANSPORTATION — _Transport Terrestre_
...
### 7.7 WAREHOUSING — _Entreposage_ (open-ended)
...
### 7.8 END_TO_END_AIR_FREIGHT — _Fret Aérien Porte-à-Porte_
...
### 7.9 END_TO_END_SEA_FREIGHT — _Fret Maritime Porte-à-Porte_
...
### 7.10 BUSINESS_REPRESENTATION — _Représentation Commerciale_ (open-ended)
...
### 7.11 CUSTOMS_BROKERAGE — _Dédouanement_
...
### 7.12 PROJECT_CARGO — _Cargaison Spéciale_
