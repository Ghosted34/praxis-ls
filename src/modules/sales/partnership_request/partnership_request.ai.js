...
module.exports = {
  ...
  writes: [
    ...
    { key: "review_partnership_request", service: (c, p, actor) => service.transition(c, { id: p.partnership_request_id, to: p.to, reason: p.reason, actor }), schema: validator.schemas.aiTransition, permission: { module: "MOD-25", action: "edit" }, confirm: true, describe: "Move an application to IN_REVIEW, reject it (a reason is required), or re-open a rejected one. Approval is a separate action — it can create a supplier." },
    { key: "approve_partnership_request", service: (c, p, actor) => service.approve(c, { id: p.partnership_request_id, create_supplier: p.create_supplier, supplier: p.supplier || {}, actor }), schema: validator.schemas.aiApprove, permission: { module: "MOD-25", action: "approve" }, confirm: true, describe: "Approve an application. A VENDOR_REGISTRATION always creates a DRAFT supplier (nothing payable until somebody verifies it); an AGENCY_PARTNERSHIP does so only when create_supplier is true. An existing supplier with the same name is reused, never duplicated." },
  ],
};
