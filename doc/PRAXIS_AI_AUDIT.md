...
**Next up for another engineer:** PR 6 (conversation management/Spaces — J1–J5) and PR 8 (eval harness/observability — B6, H1, H2, G3). Those two are what is left: PR 1–5 and PR 7 are all closed. The write-contract backlog is **empty** — `src/services/ai/write-contract-baseline.json` is `[]`, so the ratchet now fails any NEW non-conforming write rather than grandfathering a queue. **Whoever closes a milestone updates this section** (the milestone row, the finding rows, and Appendix A where relevant).
...
| B3, B6, G3, H1–H2, J1–J5 | — | ⬜ | Not started, and this is now the WHOLE of what is left. B3 is a `platform.ai_vendor_credential` repoint under PR 8, not code. G3 (regression cover for TTS / open-in-canvas / table→xlsx) belongs with PR 8's harness, which is where a regression pass can live. B6 + H1–H2 are PR 8; J1–J5 are PR 6. F1 is "good, preserve" — not a remediation item, and deliberately absent from this list. Every other finding A1–J5 has a ✅ row above, once each. |

**Recommended next PR:** PR 6 (conversation management & Spaces UX — J1–J5). Six of the eight milestones are closed and no P0 or P1 remains outside them, so the next lever is the one the review actually asked for by name: a sensitive thread that cannot be deleted or archived (J1), threads that cannot be pinned or renamed (J2/J3), and a Spaces list that cannot be collapsed so the conversation list has no room (J4). All five are user-visible, none depends on anything still open, and the migration they share (`deleted_at`/`archived_at`/`pinned_at` on `ai_conversation`) is one change.

Then PR 8 (evaluation & observability — B6, G3, H1, H2). It is last on purpose: a golden-set eval is only worth wiring once the behaviour it pins has stopped moving, and PRs 1–5 and 7 are what stopped it moving. Its grounding cases should include the A1 regression directly — a seeded figure ≥ 100,000,000 XAF reported exactly, and a `[NUM]` in an answer treated as a failure.
