...
function MiniTable({
  ...
  if (empty)
    return <div className="px-3 py-6 text-center micro">{emptyLabel}</div>;
  return (
...
function ReadinessBanner({
    ...
    return (
      <div className="rounded-lg border border-warn/40 bg-warn-fill/40 px-4 py-3">
        <div className="flex flex-wrap items-center justify-between gap-3">
          ...
          <Button size="sm" onClick={onPublishTemplate}>
  ...
  const items: { ok: boolean; label: string }[] = [
    {
      ok: readiness.has_active_template,
      label: `Active milestone template v${readiness.active_template_version}`,
    },
    {
    ...
    ...(readiness.ever_billed === null
      ...
      : [
          {
            ok: readiness.ever_billed === true,
            label: "Ever billed on a FINAL invoice",
          },
        ]),
  ];
  return (
          ...
          {items.map((it) => (
            <li key={it.label} className="flex items-center gap-1.5">
              <span
                aria-hidden
                className={
                  it.ok ? "text-[rgb(var(--ok))]" : "text-muted-foreground"
                }
              >
                {it.ok ? "✓" : "○"}
              </span>
      ...
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div className="min-w-0">
          ...
          <p className="mt-1 micro">
            {[
              st.key,
              st.name_en && st.name_fr && st.name_en !== st.name_fr
                ? st.name_fr
                : null,
            ]
              .filter(Boolean)
    ...
    <div className="space-y-3">
      ...
      {templates.length === 0 ? (
        <div className="rounded-lg border px-3 py-6 text-center micro">
          No template versions yet — publish one to give this service a
          milestone chain.
        </div>
      ) : (
        templates.map((t) => (
...
function DictionaryTab({
  ...
  const setTier = (d: api.ServiceTypeDictionaryItem, tier: api.Tier) =>
    act.run(d.dictionary_item_id, () =>
      api
        .setServiceTypeDictionaryTier(serviceTypeId, d.dictionary_item_id, tier)
        .then(reload),
    );
  ...
  const unlink = (d: api.ServiceTypeDictionaryItem) =>
    act.run(d.dictionary_item_id, () =>
      api
        .removeServiceTypeDictionaryTier(serviceTypeId, d.dictionary_item_id)
        .then(reload),
    );
  ...
  const renderRow =
    (showTier: boolean) => (d: api.ServiceTypeDictionaryItem) => (
      <tr key={d.dictionary_item_id}>
        <Td>
          <DeepLink href={dictHref(d.code)}>{d.code}</DeepLink>
        </Td>
        ...
        <Td>
          <Pill tone={CATEGORY_TONE[d.category] || "mute"}>{d.category}</Pill>
        </Td>
        <Td>{d.shipping_line || "—"}</Td>
        <Td r>
          {d.default_price != null && d.default_price !== ""
            ? money(d.default_price)
            : "—"}
        </Td>
        <Td>{d.currency || "—"}</Td>
        <Td>
    ...
    <div className="space-y-4">
      ...
      <div className="rounded-lg border">
        ...
        >
          <span>
            <span className="font-medium text-foreground">
              Also applicable — any service
            </span>
            <span className="ml-2 micro">
              {generic.length} line{generic.length === 1 ? "" : "s"} not scoped
...
function DossiersTab({
  ...
  return (
    <div className="space-y-4">
      ...
      >
        {dossiers.map((d) => (
          <tr key={d.dossier_id}>
            <Td>
              <DeepLink
                href={`/operations/files?ref=${encodeURIComponent(d.ref)}`}
              >
                {d.ref}
              </DeepLink>
            </Td>
            <Td>{d.client_name || "—"}</Td>
            <Td>
              <Pill tone={DOSSIER_TONE[d.status] || "mute"}>{d.status}</Pill>
            </Td>
            <Td>
              {d.milestone_total > 0
            ...
            <Td>
              {d.current_milestone ||
                (d.status === "COMPLETED" ? (
                  ...
                  <span className="text-warn">no chain</span>
                ) : (
                  "—"
                ))}
            </Td>
      ...
      {more > 0 && (
        <p className="micro">
          Showing 25 most recent — {more} older dossier{more === 1 ? "" : "s"}{" "}
          not listed here.{" "}
...
function CommercialTab({ d }: { d: api.ServiceTypeDossier }) {
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="rounded-xl border bg-card p-4">
        ...
        {masked ? (
          <p className="micro">
            Money figures are restricted — needs finance visibility (Treasury
            read, MOD-09).
          </p>
          ...
          !money_.actual_total ? (
          <p className="micro">
            No planned costings or FINAL invoices booked to this service type
            yet.
          </p>
        ) : (
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-1 gap-2 sm:grid-cols-2">
              <Stat
                label="Actuals (GL, XAF)"
                value={money(money_.actual_total)}
              />
              <Stat
                label="Ratio (actual / planned XAF)"
                value={(() => {
                  const plannedXaf = money_.planned.find(
                    (p) => p.currency === "XAF",
                  );
                  if (!plannedXaf || Number(plannedXaf.planned_total) === 0)
                    return "—";
                  const pct =
                    (money_.actual_total * 100) /
      ...
      <div>
        <div className="mb-2 flex flex-wrap items-center justify-between gap-2">
          ...
          <DeepLink href="/commercial/margin-simulation">
            Open Margin simulation →
          </DeepLink>
        </div>
        <MiniTable
          ...
          {d.margin_simulations.map((m) => (
            <tr key={m.margin_simulation_id}>
              <Td>
                {m.dossier_ref ? (
                  <DeepLink
                    href={`/operations/files?ref=${encodeURIComponent(m.dossier_ref)}`}
                  >
                    {m.dossier_ref}
                  </DeepLink>
                ) : (
              ...
              <Td r>
                {m.margin_percent != null
                  ? `${Number(m.margin_percent).toFixed(2)}%`
                  : "—"}
              </Td>
        ...
        {d.margin_simulations_more > 0 && (
          <p className="mt-2 micro">
            Showing 25 most recent — {d.margin_simulations_more} older not
            listed here.
          </p>
        )}
      </div>
      ...
      {!masked && (
        <div>
          <div className="mb-2 flex flex-wrap items-center justify-between gap-2">
            ...
            <DeepLink href="/finance/invoices">
          ...
          >
            {d.invoices.map((i) => (
              <tr key={i.invoice_id}>
                <Td>
                  <DeepLink
                    href={`/finance/invoices?focus=${encodeURIComponent(i.invoice_id)}`}
                  >
                ...
                <Td>
                  {i.dossier_ref ? (
                    <DeepLink
                      href={`/operations/files?ref=${encodeURIComponent(i.dossier_ref)}`}
                    >
                      {i.dossier_ref}
                    </DeepLink>
                  ) : (
    ...
    <div className="grid gap-4 lg:grid-cols-2">
      ...
      <div className="rounded-xl border bg-card p-4">
        ...
        <div className="grid grid-cols-2 gap-2">
          ...
          />
          <Stat
            label="Dictionary lines"
            value={num(d.stats.dictionary_items)}
          />
    ...
    <div className="space-y-4">
      ...
      <div className="rounded-xl border bg-card p-4">
        ...
        <ul className="space-y-1.5 text-sm text-muted-foreground">
          <li>
            <span className="font-mono text-xs">list_service_types</span> — the
            assistant lists the taxonomy with milestone-template coverage.
          </li>
          <li>
            <span className="font-mono text-xs">get_service_type</span> — one
            row by id.
          </li>
          <li>
            <span className="font-mono text-xs">get_service_type_360</span> —
            everything on this page, minus money for callers without finance
            visibility.
          </li>
        </ul>
        <p className="mt-2 micro">
...
export function ServiceTypeDossier({
  ...
  async function saveName(next: string) {
    ...
    try {
      await api.updateServiceType(
        serviceTypeId,
        usingEn ? { name_en: next } : { name_fr: next },
      );
      reload();
    } catch (e) {
      reportActionError(e);
  ...
  return (
    <div className="space-y-4">
      ...
      <KpiRow>
        ...
        <KpiTile
          label="Template"
          value={d.readiness.has_active_template ? "Active" : "None"}
          hint={templateHint}
          tone={d.readiness.has_active_template ? "ok" : "warn"}
        ...
        <KpiTile
          label="Margin sims"
          value={num(d.stats.margin_simulations)}
          onClick={() => setTab("Commercial")}
        />
        <KpiTile
          label={d.money.masked ? "Billed" : "Billed (XAF)"}
          value={
            d.money.masked
              ? "—"
              : (() => {
      ...
      {tab === "Assumptions" && (
        <ServiceTypeAssumptions serviceTypeId={serviceTypeId} />
      )}
      {tab === "Dictionary" && (
        <DictionaryTab
