...
type CargoLine = { inventory_item_id: string; label: string; packages: string; weight: string };
const blankCargo = (): CargoLine => ({ inventory_item_id: "", label: "", packages: "1", weight: "" });
...
function TransitForm({
  ...
  async function submit(e: React.FormEvent) {
    ...
    const body: api.TransitOrderInput = {
      ...
      declared_value: f.declared_value === "" ? undefined : Number(f.declared_value),
      ...
      <form className="space-y-4" onSubmit={submit}>
        ...
        {isNew && (
          <div className="space-y-2">
            ...
            <Button type="button" variant="outline" size="sm" onClick={() => setLines((s) => [...s, blankCargo()])}>
              Add cargo
            </Button>
          </div>
        )}
