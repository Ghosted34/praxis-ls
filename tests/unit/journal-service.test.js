...
function fakeClient() {
  return {
    calls: [],
    query(sql) {
      this.calls.push(String(sql).trim().split(/\s+/)[0].toUpperCase());
...
beforeEach(() => {
  ...
  repo.getPeriodForDate.mockResolvedValue({
    period_id: "P1",
    status: "OPEN",
    code: "2026-01",
  });
  ...
  repo.insertEntry.mockImplementation((_c, d) =>
    Promise.resolve({ entry_id: "E1", ...d }),
  );
  repo.insertLine.mockImplementation((_c, d) =>
    Promise.resolve({ line_id: "L" + d.line_no, ...d }),
  );
  repo.setStatus.mockImplementation((_c, id, patch) =>
    Promise.resolve({ entry_id: id, ...patch }),
  );
});
...
const baseInput = {
  ...
  sourceDocRef: "vault:doc-1",
  source: "SYSTEM_AUTO",
  lines: [
    { account_code: "521", debit: 1000, credit: 0 },
...
describe("posting service", () => {
  it("posts a balanced entry: draft insert, lines, validate, event", async () => {
    ...
    expect(emitEvent).toHaveBeenCalledWith(
      c,
      expect.objectContaining({ eventTypeKey: "journal.posted" }),
    );
  ...
  it("rolls back and rejects an unbalanced entry", async () => {
    ...
    const bad = {
      ...
      lines: [
        { account_code: "521", debit: 1000, credit: 0 },
        { account_code: "4191", debit: 0, credit: 999 },
      ],
    };
    await expect(service.post(c, bad)).rejects.toMatchObject({
  ...
  it("reverses a validated entry by swapping debit/credit (#23.16)", async () => {
    repo.getEntry.mockResolvedValue({
      entry_id: "E1",
      status: "validated",
      journal_id: "J1",
      entity_id: "ent-1",
      entry_no: 7,
      source_doc_ref: "vault:doc-1",
    });
    ...
    expect(linePayloads[1]).toMatchObject({
      account_code: "4191",
      debit: "1000.00",
      credit: "0.00",
    });
    expect(repo.insertEntry).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        ...
        corrects_entry_id: "E1",
      }),
    );
    expect(emitEvent).toHaveBeenCalledWith(
      c,
      expect.objectContaining({ eventTypeKey: "journal.reversed" }),
    );
  });
  ...
  it("refuses to reverse a draft entry", async () => {
    ...
    const c = fakeClient();
    await expect(
      service.reverse(c, { entryId: "E1", actor: {} }),
