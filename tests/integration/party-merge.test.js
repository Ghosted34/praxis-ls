...
d("governed merge (real Postgres)", () => {
  let pool; let merge; let accounting; let survivorId; let loserId;
  ...
  beforeAll(async () => {
    ...
    try {
      ...
      await c.query("INSERT INTO party_registration (client_id, kind, number) VALUES ($1,'NIU',$2)", [loserId, `NIU${uniq}`]);
      await c.query("INSERT INTO client_contact (client_id, name) VALUES ($1,$2)", [loserId, `Contact ${uniq}`]);
      await c.query("INSERT INTO compliance_flag (rule_key, entity_ref, severity, message) VALUES ('party.bank_unverified',$1,'ESCALATED','x')", [`client:${loserId}`]);
  ...
  it("reattaches the loser's registrations and contacts to the survivor", async () => {
    const regs = await pool.query("SELECT client_id FROM party_registration WHERE number = $1", [`NIU${uniq}`]);
    expect(regs.rows[0].client_id).toBe(survivorId);
    const contacts = await pool.query("SELECT client_id FROM client_contact WHERE name = $1", [`Contact ${uniq}`]);
  ...
  it("archives the loser (never deletes) and points merged_into_id at the survivor", async () => {
    const { rows } = await pool.query("SELECT registration_status, is_active, merged_into_id, deactivation_reason FROM client_master WHERE client_id = $1", [loserId]);
  ...
  it("preserves the loser's name as an alias under the survivor", async () => {
    const { rows } = await pool.query("SELECT alias FROM party_alias WHERE party_kind = 'client' AND party_id = $1", [survivorId]);
    expect(rows.map((r) => r.alias)).toContain(`Loser ${uniq}`);
  });

  it("deactivates the loser's aux account (never deletes it)", async () => {
    const { rows } = await pool.query("SELECT coa_aux_account FROM client_master WHERE client_id = $1", [loserId]);
    const code = rows[0].coa_aux_account;
    expect(code).toBeTruthy();
    const acct = await pool.query("SELECT is_active FROM chart_of_accounts WHERE code = $1", [code]);
    expect(acct.rows[0].is_active).toBe(false);
  });

  it("re-points the loser's open compliance flags to the survivor", async () => {
    const onLoser = await pool.query("SELECT count(*)::int AS n FROM compliance_flag WHERE entity_ref = $1 AND resolved_at IS NULL", [`client:${loserId}`]);
    const onSurvivor = await pool.query("SELECT count(*)::int AS n FROM compliance_flag WHERE entity_ref = $1 AND resolved_at IS NULL AND message = 'x'", [`client:${survivorId}`]);
