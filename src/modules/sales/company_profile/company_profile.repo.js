...
async function update(c, fields) {
 const keys=Object.keys(fields); if(!keys.length) return get(c); const vals=keys.map(k=>fields[k]);
...
async function replaceProjects(c,id,rows){await c.query("DELETE FROM company_profile_project WHERE company_profile_id=$1",[id]); for(let i=0;i<rows.length;i++){const p=rows[i]; await c.query("INSERT INTO company_profile_project(company_profile_id,name_en,name_fr,description_en,description_fr,metrics,sort_order) VALUES($1,$2,$3,$4,$5,$6,$7)",[id,p.name_en,p.name_fr||null,p.description_en||null,p.description_fr||null,p.metrics||{},p.sort_order??i]);}}
