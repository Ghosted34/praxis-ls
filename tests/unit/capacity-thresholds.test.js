...
jest.mock("../../src/services/tenant/pooler-stats.service", () => ({
  pools: jest.fn(),
}));
jest.mock("../../src/services/platform/provisioning.service", () => ({
  fleetSchemaStatus: jest.fn(),
}));
...
const at = (pct, thresholds) =>
  health.statusFor({ pool_utilisation_pct: pct }, thresholds);
...
describe("capacity thresholds default at the call site", () => {
  ...
  test("a garbage value falls back to the default rather than never firing", () => {
    expect(at(85, { healthPoolUtilisationAmber: "eighty" }).status).toBe(
      "AMBER",
    );
    expect(at(50, { healthPoolUtilisationAmber: "eighty" }).status).toBe(
      "GREEN",
    );
  });
  ...
  test("pooler maxwait: 0 in the vault does not amber a healthy pooler", () => {
    expect(
      health.statusFor(
        { pooler_maxwait_us: 1000 },
        { healthPoolerMaxwaitAmberMs: 0 },
      ).status,
    ).toBe("GREEN");
  });
