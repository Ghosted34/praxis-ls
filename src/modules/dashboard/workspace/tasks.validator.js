...
/** The stage set of 13930 — uuids, deduplicated by the service, bounded here. */
...
const taskCreate = z
  .object({
    ...
    // (13930). Supersedes the single column above when present; an empty list
...
const childCreate = z
  .object({
    ...
    entity_type: z.string().trim().max(40).nullable().optional(),
    entity_id: z.string().uuid().nullable().optional(),
    // Same terms for the operations file and its stages (13920, 13930):
