...
const STATE_TONE: Record<string, Tone> = {
  ...
  WARN: "warn",
  ESCALATED: "bad",
  SOFT_BLOCK_RECOMMENDATION: "orange",
  HARD_BLOCK: "bad",
};
...
const AVL_TONE: Record<string, Tone> = {
  PROSPECT: "mute",
  PENDING_KYC: "warn",
  APPROVED: "ok",
  CONDITIONAL: "orange",
    ...
    <Modal open onClose={onClose} title={title}>
      <div className="grid gap-3 sm:grid-cols-2">
        {fields.map((f) => (
          <label
            key={f.key}
            className={`space-y-1 text-sm ${f.type === "checkbox" ? "flex items-center gap-2" : ""}`}
          >
            ...
            {f.type === "country" ? (
              ...
              <Checkbox
                ...
                label={<span className="sr-only">{f.label}</span>}
              />
            ) : f.type === "select" ? (
              <Select
                value={(values[f.key] as string) || ""}
  ...
  async function save(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (file) {
        ...
        await api.documents.update(kind, partyId, created.document_id, {
    ...
    <Modal open onClose={onClose} title="Add document">
      <form onSubmit={save} className="space-y-4">
        <div className="grid gap-3 sm:grid-cols-2">
          ...
          >
            <Input
              value={values.physical_ref || ""}
              placeholder="Box A-12"
              onChange={(e) => set("physical_ref", e.target.value)}
            />
          </Field>
        </div>
...
function DeepLink({
  ...
  return (
    <button
      ...
      onClick={() => navigate(focusHref(route, id))}
    >
      {children}
    </button>
  );
...
function DuplicatesPanel({
  ...
  candidates: api.DedupeCandidate[];
  kind: api.PartyKind;
  onOpen: (id: string) => void;
  onMerge: (c: api.DedupeCandidate) => void;
  canMerge: boolean;
}) {
    ...
    <div className="rounded-xl border border-warn/40 bg-warn-fill/40 p-4">
      <div className="flex flex-wrap items-center gap-2">
        ...
        <span className="micro">
          This {kind} looks like {strong.length} existing record
          {strong.length === 1 ? "" : "s"} — review before trading.
        </span>
      </div>
      <ul className="mt-2.5 space-y-1.5">
        {strong.map((c) => (
          ...
          >
            ...
            {canMerge && (
              <Button size="sm" variant="outline" onClick={() => onMerge(c)}>
                Merge…
              </Button>
            )}
          </li>
...
function MergeModal({
  ...
  survivorId: string;
  survivorRef: string;
  loser: api.DedupeCandidate;
  onClose: () => void;
  onDone: (msg: string) => void;
}) {
  const [preview, setPreview] = React.useState<api.MergePreviewResult | null>(
    null,
  );
  ...
  React.useEffect(() => {
    api
      .mergePreview(kind, survivorId, survivorId, loser.id)
      .then(setPreview)
      .catch((e) => setError(errMsg(e)));
  }, [kind, survivorId, loser.id]);

  const refOk =
    confirm.trim() !== "" && confirm.trim() === (survivorRef || "").trim();
  async function run() {
    setBusy(true);
    setError(null);
    try {
      ...
      onDone(
        r && r.pending ? "Merge submitted for approval" : "Records merged",
      );
      onClose();
    } catch (e) {
      setError(errMsg(e));
  ...
  return (
    <Modal
      ...
      description="Everything on the losing record is reattached to the survivor, its aliases are preserved, and it is archived (never deleted). This cannot be undone from the UI."
    >
      <div className="space-y-3 text-sm">
        <div className="rounded-lg border p-3">
          <div>
            <span className="micro">Survivor (kept)</span>
            <div className="font-medium text-foreground">
              {preview?.survivor.name || survivorRef}
            </div>
          </div>
          <div className="mt-2">
            <span className="micro">Loser (archived)</span>
            <div className="font-medium text-foreground">
              {loser.name || loser.ref || loser.id.slice(0, 8)}
            </div>
          </div>
        </div>
        {preview &&
          (preview.moves.length === 0 ? (
            ...
            <div>
              <p className="mb-1 micro font-medium text-foreground">
                {preview.total} record{preview.total === 1 ? "" : "s"} will move
                to the survivor:
              </p>
              <ul className="space-y-0.5">
                {preview.moves.map((m) => (
                  <li key={`${m.table}.${m.column}`} className="micro">
                    {m.count} × {m.table.replace(/_/g, " ")}
                  </li>
                ))}
              </ul>
            </div>
        ...
        <label className="block space-y-1">
          <span className="font-medium text-foreground">
            Type the survivor ref{" "}
            <code className="micro">{survivorRef || "—"}</code> to confirm
          </span>
          <Input
            value={confirm}
            onChange={(e) => setConfirm(e.target.value)}
            placeholder={survivorRef || ""}
          />
        </label>
        {error && <ErrorState message={error} />}
      </div>
...
function ConvertModal({
  ...
  async function run() {
    ...
    try {
      await (kind === "client"
        ? api.convertFromClient(partyId)
        : api.convertFromSupplier(partyId));
    ...
    >
      <div className="space-y-3 text-sm">
        ...
        <div className="rounded-lg border border-warn/40 bg-warn-fill/30 p-3">
          <p className="mb-1 font-medium text-foreground">
            You&apos;ll need to complete before the draft can trade
          </p>
          <ul className="list-disc space-y-1 pl-4 text-muted-foreground">
            {CONVERT_TODO.map((t) => (
              <li key={t}>{t}</li>
      ...
      <div className="mt-4 flex justify-end gap-2">
        ...
        <Button variant="default" loading={busy} disabled={!ack} onClick={run}>
          Create draft {targetLabel}
        </Button>
      </div>
    </Modal>
  );
...
function SvgBars({
  ...
  const W = 260;
  const barX = 96;
  const barMax = W - barX - 4;
  return (
    <svg
      viewBox={`0 0 ${W} ${data.length * H}`}
      className="w-full"
      role="img"
      ...
      {data.map((d, i) => (
        <g key={d.label} transform={`translate(0 ${i * H})`}>
          ...
          <rect
            x={barX}
            y={H / 2 - 5}
            width={Math.max(0, Math.round((d.value / max) * barMax))}
            height="10"
            rx="2"
            className="fill-current text-primary-ink"
          />
          <text
            x={W}
            y={H / 2 + 3}
    ...
    <div className="rounded-xl border bg-card p-4">
      ...
      <div className="mt-3">
        <SvgBars
          ariaLabel={title}
          fmt={money}
          data={AGING_COLUMNS.map(({ key, label }) => ({
            label,
            value: aging[key],
          }))}
        />
...
function AgingDetailModal({
  ...
  kind: api.PartyKind;
  partyId: string;
  bucket: api.AgingBucket;
  label: string;
  onClose: () => void;
}) {
  const detail = useResource(
    () => api.agingDetail(kind, partyId, bucket),
    [kind, partyId, bucket],
  );
    ...
    >
      {detail.loading ? (
        ...
        <div className="space-y-3">
          <p className="micro">
            {detail.data.count} invoice{detail.data.count === 1 ? "" : "s"} ·{" "}
            {money(detail.data.total)} as of {dateFmt(detail.data.as_of)}
          </p>
          <MiniTable
            empty={false}
            head={
...
const SCORE_TONE = (v: number): Tone =>
...
function ScorecardCard({ s }: { s: api.Scorecard }) {
  const rows: [string, number][] = [
    ["On-time", s.score_on_time],
    ["Claims", s.score_claims],
    ["Disputes", s.score_disputes],
    ["Responsiveness", s.score_responsiveness],
    ["Safety", s.score_safety],
    ...
    <div className="rounded-xl border bg-card p-4">
      <div className="mb-3 flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          ...
          <Pill tone={SCORE_TONE(s.score_overall)}>{num(s.score_overall)}</Pill>
        </div>
      </div>
        ...
        {rows.map(([label, v]) => (
          <div key={label} className="rounded-lg border p-2">
            <div className="flex items-center justify-between">
              ...
              <Pill tone={SCORE_TONE(v)}>{num(v)}</Pill>
            </div>
      ...
      {s.last_evaluated_at && (
        <p className="mt-2 micro">Evaluated {dateFmt(s.last_evaluated_at)}</p>
      )}
    </div>
  );
}
  ...
  return (
    <div className="rounded-xl border border-warn/40 bg-warn-fill/40 p-4">
      <h4 className="mb-2 flex items-center gap-2 text-sm font-semibold text-foreground">
        <Pill tone="orange">Pending approval</Pill> Sensitive changes
      </h4>
      <ul className="space-y-2">
        {items.map((c) => (
          <li
            key={c.change_request_id}
            className="flex flex-wrap items-center justify-between gap-2 text-sm"
          >
            ...
            <div className="flex gap-2">
              <Button
                ...
                onClick={() =>
                  onAct(
                    () => api.approveChange(kind, partyId, c.change_request_id),
                    "Change approved",
                  )
                }
              >
              ...
              <Button
                ...
                onClick={() =>
                  onAct(
                    () => api.rejectChange(kind, partyId, c.change_request_id),
                    "Change rejected",
                  )
                }
              >
      ...
      <p className="mt-2 micro">
        Maker-checker: the person who raised a change cannot approve it.
      </p>
    </div>
  );
}
...
function KpiDetails({
  ...
  if (isClient) {
    ...
    const toInvoiceRow = (
      i: api.Client360["invoices"][number],
    ): KpiDetailRow => ({
      id: i.invoice_id,
    ...
    if (key === "outstanding") {
      ...
      description =
        "Invoices past their payment due date with an open balance.";
      rows = invoices
        .filter(
          (i) =>
            isOpenInvoiceStatus(i.status) && isOverdueDue(i.payment_due_on),
        )
        .map(toInvoiceRow);
    ...
    const toBillRow = (
      i: api.Supplier360["supplier_invoices"][number],
    ): KpiDetailRow => ({
    ...
    if (key === "payables") {
      ...
      rows = supInvoices
        .filter((i) => isOpenInvoiceStatus(i.status) && isOverdueDue(i.due_on))
        .map(toBillRow);
...
export function PartyDossier({
  ...
  const dossier = useResource<api.Client360 | api.Supplier360>(
    () =>
      kind === "client"
        ? api.clientDossier(partyId)
        : api.supplierDossier(partyId),
    [kind, partyId],
  );
  const docTypes = useResource(
  ...
  const [busy, setBusy] = React.useState(false);
  const [error, setError] = React.useState<string | null>(null);
  const [kpiOpen, setKpiOpen] = React.useState<
    ClientKpiKind | SupplierKpiKind | null
  >(null);
  ...
  const docTypeOptions = (docTypes.data || []).map((t) => ({
  ...
  const openParty = (targetKind: api.PartyKind, id?: string | null) =>
    navigate(
      `/master/${targetKind === "client" ? "clients" : "suppliers"}${id ? `?focus=${encodeURIComponent(id)}` : ""}`,
    );
  // Explicit copy-from-origin (§6) — only offered on a converted party.
  const copySection = (section: api.CloneSection) =>
    act(async () => {
      const r = await api.cloneFromOrigin(kind, partyId, [section]);
      return r;
    }, "Copied from origin");
  /**
  ...
  async function revealBankNumber(bankId: string) {
    setError(null);
    try {
      const r = await api.revealBank(kind, partyId, bankId);
      setRevealed((s) => ({
        ...s,
        [bankId]: r.account_number || r.iban || r.swift_bic || "—",
      }));
    } catch (e) {
      setError(errMsg(e));
    }
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="rounded-xl border bg-card p-5">
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div className="min-w-0">
            <div className="flex flex-wrap items-center gap-2">
              ...
              {p.compliance_state && (
                <Pill tone={STATE_TONE[p.compliance_state] || "mute"}>
                  {enumLabel(p.compliance_state)}
                </Pill>
              )}
              {p.registration_status && (
                <Pill tone={REG_TONE[p.registration_status] || "mute"}>
                  {enumLabel(p.registration_status)}
                </Pill>
              )}
            ...
            <p className="mt-1 micro">
              {[
                p.ref,
                p.category_name,
                p.coa_aux_account && `Aux ${p.coa_aux_account}`,
                p.niu && `NIU ${p.niu}`,
              ]
                .filter(Boolean)
                .join(" · ") || "—"}
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            {comp.can_verify && p.verification_status !== "VERIFIED" && (
              <Button
                ...
                loading={busy}
                onClick={() =>
                  act(() => api.verifyParty(kind, partyId), "Verified")
                }
              >
                Verify
              </Button>
            ...
            {p.registration_status === "ACTIVE" ? (
              <Button
                ...
                onClick={() =>
                  act(
                    ...
                    isClient ? "Client deactivated" : "Supplier deactivated",
                  )
                }
              >
                Deactivate
              ...
              <Button
                ...
                onClick={() =>
                  act(
                    () => api.setRegistrationStatus(kind, partyId, "ACTIVE"),
                    isClient ? "Client activated" : "Supplier activated",
                  )
                }
              >
                Activate
            ...
            {blocked ? (
              <Button
                ...
                loading={busy}
                onClick={() =>
                  act(() => api.unblockParty(kind, partyId), "Unblocked")
                }
              >
                Unblock
              </Button>
            ...
            {linkedId ? (
              <Button
                ...
                variant="ghost"
                onClick={() =>
                  openParty(isClient ? "supplier" : "client", linkedId)
                }
              >
        ...
        {blocked && p.hard_block_reason && (
          <div className="mt-3 rounded-lg border border-bad/40 bg-bad-fill/40 px-3 py-2 text-sm text-foreground">
            <span className="font-medium">Hard-blocked</span> —{" "}
            {p.hard_block_reason}
          </div>
        )}
        {error && (
      ...
      <DuplicatesPanel
        candidates={d.duplicate_candidates || []}
        kind={kind}
        onOpen={(id) => openParty(kind, id)}
        onMerge={(c) => setMerging(c)}
        canMerge
      />
      <PendingChangesCard
        items={d.pending_changes || []}
        kind={kind}
        partyId={partyId}
        onAct={act}
      />
      ...
      {isClient ? (
          ...
          <KpiTile
            ...
            hint={
              d.kpis.oldest_due_date
                ? `oldest ${dateFmt(d.kpis.oldest_due_date)}`
                : undefined
            }
            onClick={() => setKpiOpen("overdue")}
          />
          {/* No drill-in — the number is a limit minus a balance, not a list. */}
          <KpiTile
            label="Credit available"
            value={
              d.kpis.credit_available != null
                ? money(d.kpis.credit_available)
                : "—"
            }
          />
          <KpiTile
            label="YTD revenue"
            value={money(d.kpis.ytd_revenue)}
        ...
        <KpiRow>
          ...
          <KpiTile
            label="YTD spend"
            value={money(d.kpis.ytd_spend)}
            onClick={() => setKpiOpen("ytd_spend")}
          />
          <KpiTile
            label="Open POs"
            value={num(d.kpis.open_purchase_orders)}
            onClick={() => setKpiOpen("open_pos")}
          />
        </KpiRow>
      )}
      <KpiDetails
      ...
      <div className="flex flex-wrap gap-1 border-b">
        {(isClient ? CLIENT_TABS : SUPPLIER_TABS).map((t) => {
          ...
          const counts: Partial<Record<Tab, number>> = {
            ...
            Addresses: d.addresses?.length ?? 0,
            Banks: d.banks?.length ?? 0,
          ...
          <div className="rounded-xl border bg-card p-4">
            ...
            {(() => {
              ...
              if (comp.flags.length === 0)
                return <p className="micro">No open flags.</p>;
              return (
                <div className="space-y-3">
                  {onboarding.length > 0 && (
                    <div>
                  ...
                  {issues.length > 0 && (
                    <div>
                      {onboarding.length > 0 && (
                        <p className="mb-1.5 micro font-medium text-foreground">
                          Compliance issues
                        </p>
                      )}
                      <ul className="space-y-1.5">
                        {issues.map((f) => (
                          <li
                            ...
                            className="flex items-center gap-2 text-sm"
                          >
                            <Pill tone={SEVERITY_TONE[f.severity] || "mute"}>
                              {f.severity}
                            </Pill>
                            <span className="text-muted-foreground">
                              {f.message || f.rule_key}
            ...
            <p className="mt-3 micro">
              ...
              {d.gl_parity.flagged ? (
                <span className="text-bad">
                  mismatch {money(d.gl_parity.mismatch)}
                </span>
              ) : (
                "clean"
              )}{" "}
              · docs {money(d.gl_parity.docTotal)} vs GL {money(d.gl_parity.gl)}
            </p>
          </div>
          <AgingCard
            title={isClient ? "Aging" : "Aging (payables)"}
            aging={d.kpis.aging}
            onOpen={(bucket, label) => setAging({ bucket, label })}
          />
      ...
      {tab === "Contacts" && (
        <Section
          ...
          onCopy={linkedId ? () => copySection("contacts") : undefined}
        >
          {d.contacts.length === 0 ? (
            <Empty />
          ) : (
                  ...
                  <div className="mt-2 flex flex-wrap gap-2">
                    ...
                    {isClient && (
                      <button
                        ...
                        onClick={() =>
                          navigate(
                            `/comms?client=${encodeURIComponent(partyId)}`,
                          )
                        }
                      >
                        Message
      ...
      {tab === "Addresses" && (
        <Section
          ...
          onCopy={linkedId ? () => copySection("addresses") : undefined}
        >
              ...
              <tr key={b.bank_account_id}>
                ...
                <Td>
                  ...
                  {b.masked && !revealed[b.bank_account_id] && (
                    <button
                      className="ml-2 text-primary-ink underline micro"
                      onClick={() => revealBankNumber(b.bank_account_id)}
                    >
                      Reveal
                    </button>
                  )}
      ...
      {tab === "Operations" && (
        <div className="space-y-3">
                ...
                <tr key={ds.dossier_id}>
                  <Td>
                    <button
                      ...
                      className="text-primary-ink underline underline-offset-2 hover:opacity-80"
                      onClick={() =>
                        navigate(dossierHref(ds.ref, ds.dossier_id))
                      }
                    >
                      {ds.ref || ds.dossier_id.slice(0, 8)}
                    </button>
                  ...
                  <Td>
                    {ds.status ? (
                      <Pill tone={DOSSIER_TONE[ds.status] || "mute"}>
                        {enumLabel(ds.status)}
                      </Pill>
                    ) : (
                      "—"
          ...
          {d.dossiers.length >= 25 && (
            <p className="micro">
              Showing the 25 most recent — open Operations for the full list.
            </p>
          )}
        </div>
      )}
      ...
      {tab === "Financial" &&
        (isClient ? (
          <div className="space-y-4">
            ...
            >
              {d.invoices.map((i: Client360Invoice) => (
                <tr key={i.invoice_id}>
                  <Td>
                    <DeepLink route={MODULE_ROUTE.invoice} id={i.invoice_id}>
                      {i.doc_number || i.invoice_id.slice(0, 8)}
                    </DeepLink>
                  </Td>
                  <Td>{enumLabel(i.type)}</Td>
                  <Td>{enumLabel(i.status)}</Td>
                  <Td>{dateFmt(i.payment_due_on)}</Td>
                  <Td r>{money(i.total_ttc)}</Td>
                </tr>
              ))}
            </MiniTable>
          </div>
          ...
          <MiniTable
            empty={d.supplier_invoices.length === 0}
            head={
              <>
                <Th>Bill</Th>
                <Th>{tr("Status")}</Th>
          ...
          >
            {d.supplier_invoices.map((i: SupplierBill) => (
              <tr key={i.supplier_invoice_id}>
                <Td>
                  <DeepLink
                    route={MODULE_ROUTE.supplier_invoice}
                    id={i.supplier_invoice_id}
                  >
                    {i.doc_number || i.supplier_invoice_id.slice(0, 8)}
                  </DeepLink>
                ...
                <Td>{enumLabel(i.status)}</Td>
                <Td>{dateFmt(i.due_on)}</Td>
                <Td r>{money(i.wht_total)}</Td>
                <Td r>{money(i.amount_ttc)}</Td>
              </tr>
            ))}
          </MiniTable>
        ))}
      ...
      {adding === "contact" && (
        <AddModal
          ...
          onSubmit={async (v) => {
            await api.contacts.create(kind, partyId, v as Partial<api.Contact>);
        ...
        <AddModal
          ...
          onSubmit={async (v) => {
            await api.beneficialOwners.create(
              kind,
              partyId,
              v as Partial<api.BeneficialOwner>,
            );
      ...
      {blocking && (
        <BlockModal
          ...
          onSubmit={async (reason) => {
            await act(
              () => api.blockParty(kind, partyId, reason),
              "Party blocked",
            );
          }}
        />
    ...
    >
      <label className="space-y-1 text-sm">
        ...
        <Input
          ...
          placeholder="e.g. sanctions match pending review"
        />
      </label>
      <div className="mt-4 flex justify-end gap-2">
        <Button variant="ghost" onClick={onClose}>
