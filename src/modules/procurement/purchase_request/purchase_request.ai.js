...
module.exports = {
  ...
  writes: [
    ...
    { key: "transition_purchase_request", service: (c, p, actor) => service.transition(c, { id: p.purchase_request_id, to: p.to, entityId: p.entity_id, date: p.date, actor }), schema: validator.schemas.aiTransition, permission: { module: "MOD-62", action: "approve" }, confirm: true, describe: "Advance a purchase request by id one step along DRAFT→SUBMITTED→APPROVED→ORDERED (REJECTED from SUBMITTED). Cannot skip states." },
  ],
};
