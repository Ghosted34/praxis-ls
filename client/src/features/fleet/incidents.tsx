...
function NewIncidentForm({
  ...
  return (
    <Modal
      ...
      description="Log a fleet incident. It opens for review."
    >
...
export function IncidentsPage() {
  ...
  async function toStatus(i: api.Incident, status: string) {
    ...
    try {
      await api.setIncidentStatus(i.fleet_incident_id, status);
      inc.reload();
    } catch (e) {
      reportActionError(e);
    } finally {
  ...
  const cols: Column<api.Incident>[] = [
    {
      ...
      render: (i) => (
        <span className="num text-muted-foreground">
          {i.occurred_at ? dateTimeFmt(i.occurred_at) : "—"}
        </span>
      ),
    },
    {
    ...
    },
    {
      ...
      label: "Driver",
      render: (i) => (
        <span className="text-muted-foreground">{i.driver_name || "—"}</span>
      ),
    },
    {
      key: "sev",
      label: "Severity",
      render: (i) => (
        <Pill tone={SEV_TONE[i.severity || ""] || "mute"}>
          {enumLabel(i.severity)}
        </Pill>
      ),
    },
    {
      ...
      label: "Status",
      render: (i) => (
        <Pill tone={STATUS_TONE[i.status || ""] || "mute"}>
          {enumLabel(i.status)}
        </Pill>
      ),
    },
    {
      ...
      render: (i) => (
        <TransitionButtons
          items={(TRANSITIONS[i.status || ""] || []).map((s) => ({
            to: s,
            label: STATUS_LABEL[s] || s,
            variant: s === "CLOSED" ? "outline" : "default",
            loading: busy === i.fleet_incident_id + s,
          }))}
  ...
  return (
    <section className={shell}>
      <PageHeader
        ...
        title="Incidents"
        description="Report fleet incidents and work them through review to closure."
        action={
          <Button onClick={() => setCreating(true)}>Report incident</Button>
        }
      />
      <HubTabs />
      <DataList
        ...
        error={inc.error}
        loading={inc.loading}
        rowKey={(i) => i.fleet_incident_id}
        empty={{
          title: "No incidents",
          hint: "A clean record — report one if something happens.",
        }}
      />
      {creating && (
        <NewIncidentForm
