...
- **Mail now accepts the attachment sizes its composer promises.** The upload endpoint carried files as base64 JSON but fell through to the API's 2 MB body parser, so an ordinary 2 MB Word document expanded to about 2.7 MB on the wire and failed with `413 request entity too large` before the mail service saw it. That exact upload path now has a 35 MB parser budget, enough for the advertised 25 MiB decoded aggregate cap after base64 inflation; all other mail routes keep the 2 MB default, and the decoded 25 MiB total remains enforced by the service. The validator's data-URL ceiling now also covers a full 25 MiB file instead of stopping just short.
