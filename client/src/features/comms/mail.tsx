...
function ThreadMessage({
  ...
  async function sendReply() {
    ...
    setError(null);
    try {
      await api.replyMail(id, {
        connectionId: m.email_connection_id,
        text: reply,
      });
      setSent(true);
      setReply("");
  ...
  return (
    <Modal
      ...
      description={
        m
          ? `${m.direction === "OUT" ? "To" : "From"} ${m.direction === "OUT" ? m.to_address : m.from_address}`
          : ""
      }
    >
      ...
      {m && (
        <div className="space-y-3 text-sm">
          ...
          {m.body_html ? (
            <div
              className="prose prose-sm max-w-none rounded-lg border border-border bg-card/40 px-3 py-2"
              dangerouslySetInnerHTML={{ __html: m.body_html }}
            />
          ) : (
            <div className="rounded-lg border border-border bg-card/40 px-3 py-2 whitespace-pre-wrap">
          ...
          <div className="border-t border-border pt-3">
            {m.email_connection_id ? (
              ...
              <p className="micro">
                This is a legacy message with no linked mailbox — replies aren’t
                available.
              </p>
            )}
          </div>
...
export function ComposeModal({
  ...
  const conns = (connections ?? owned.data ?? []).filter(
    (c) => c.status === "CONNECTED",
  );
  const defaultId =
    conns.find((c) => c.is_default)?.email_connection_id ||
    conns[0]?.email_connection_id ||
    "";
  const [connId, setConnId] = React.useState("");
  React.useEffect(() => {
    if (!connId && defaultId) setConnId(defaultId);
  }, [defaultId, connId]);
  ...
  function pickRecipient(r: api.Recipient) {
    setF((s) => {
      const i = s.to.lastIndexOf(",");
      const head = i >= 0 ? `${s.to.slice(0, i + 1)} ` : "";
      return { ...s, to: `${head}${r.email}, ` };
    });
    setResults([]);
  }
  ...
  return (
    <Modal
      ...
      size="lg"
      title="New message"
      description="Sent from your default mailbox — switch below if you have more than one."
    >
      <form className="space-y-3" onSubmit={submit}>
        ...
        <Field
          ...
          hint={
            locked
              ? undefined
              : "Type a name or email to find a client, supplier, staff or lead — or enter any address."
          }
        >
          <div className="relative">
            <Input
              ...
              onChange={set("to")}
              disabled={locked}
              placeholder="name or recipient@company.cm"
            />
...
function ThreadsSection() {
  ...
  const thread = useResource(
    () => api.listThread(connId || undefined),
    [connId],
  );
  ...
  const cols: Column<api.ThreadMsg>[] = [
    {
      ...
      label: "",
      render: (m) => (
        <Pill tone={m.direction === "OUT" ? "blue" : "mute"}>
          {m.direction === "OUT" ? "sent" : "in"}
        </Pill>
      ),
    },
    {
      ...
      render: (m) => (
        <span
          className={`num ${m.is_read || m.direction === "OUT" ? "text-muted-foreground" : "font-medium text-foreground"}`}
        >
          {m.direction === "OUT" ? m.to_address : m.from_address}
        </span>
      ),
    },
    {
    ...
    },
    {
      ...
      render: (m) =>
        m.entity_ref ? (
          <span className="num text-muted-foreground">{m.entity_ref}</span>
        ) : (
          "—"
        ),
    },
  ...
  return (
    <>
      <div className="mb-4 flex items-center justify-between gap-3">
          ...
          {list
            ...
            .map((c) => (
              <button
                ...
                onClick={() => setConnId(c.email_connection_id)}
                className={`chip ${connId === c.email_connection_id ? "on" : ""}`}
              >
                {c.email_address}
              </button>
            ))}
        ...
        <div className="flex items-center gap-2">
          <Button
            ...
            title={
              connected.length ? "Send a new email" : "Connect a mailbox first"
            }
          >
            Compose
          </Button>
          <Button size="sm" variant="outline" onClick={() => thread.reload()}>
            Refresh
          </Button>
        </div>
      </div>
      <DataList
        ...
        rowKey={(m) => m.email_inbound_id}
        onRowClick={(m) => setViewId(m.email_inbound_id)}
        empty={{
          title: "No messages yet",
          hint: "Connect a mailbox under Mailboxes, then messages sync in here.",
        }}
      />
      {viewId && (
        <ThreadMessage
      ...
      <div className="mb-4 inline-flex gap-1 rounded-xl border bg-muted p-1">
        {MODES.map((m) => (
          <button
            ...
            className={`rounded-lg px-3 py-1.5 text-sm transition-colors ${mode === m.key ? "bg-primary font-semibold text-primary-foreground shadow-sm" : "text-muted-foreground hover:text-foreground"}`}
          >
            {m.label}
          </button>
        ))}
