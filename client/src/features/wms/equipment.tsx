...
const COL_TONE: Record<string, Tone> = {
  AVAILABLE: "ok",
  IN_USE: "blue",
  MAINTENANCE: "warn",
  OUT_OF_SERVICE: "bad",
...
function CheckoutModal({
  ...
  return (
    <Modal
      ...
      onClose={onClose}
      title="Check out"
      description={`Assign ${item.label} to an operator.`}
    >
      ...
      <form className="space-y-4" onSubmit={submit}>
        ...
        <Field label="Location">
          <Select
            value={f.location_id}
            onChange={(e) =>
              setF((s) => ({ ...s, location_id: e.target.value }))
            }
          >
            <option value="">—</option>
            {locations.map((l) => (
        ...
        <div className="flex justify-end gap-2 pt-2">
          ...
          <Button type="submit" loading={busy} disabled={!f.label || busy}>
            Add
          </Button>
        </div>
      </form>
...
export function EquipmentPage() {
  ...
  const userMap = React.useMemo(() => {
    ...
    (users || []).forEach((u) => {
      m[u.user_id] = u.full_name || u.user_id.slice(0, 8);
    });
    return m;
  }, [users]);
  const slot = (e: api.Equipment) =>
    [e.zone, e.aisle, e.rack, e.bin].filter(Boolean).join(" · ");

  async function move(e: api.Equipment, status: string) {
    setBusy(e.wms_equipment_id);
    try {
      await api.setEquipmentStatus(e.wms_equipment_id, status);
      equipment.reload();
    } catch (e) {
      reportActionError(e);
    } finally {
  ...
  function actions(e: api.Equipment) {
    ...
    const byStatus: Record<string, Transition[]> = {
      AVAILABLE: [
        ...
        { to: "MAINTENANCE", label: "Maint.", variant: "outline" },
      ],
      IN_USE: [
        { to: "AVAILABLE", label: "Return", loading: b },
        { to: "MAINTENANCE", label: "Maint.", variant: "outline" },
      ],
      MAINTENANCE: [
        { to: "AVAILABLE", label: "Available", loading: b },
        { to: "OUT_OF_SERVICE", label: "Retire", variant: "outline" },
      ],
    };
    const items = byStatus[e.status] || [
      { to: "AVAILABLE", label: "Restore", variant: "outline", loading: b },
    ];
    return (
      <TransitionButtons
        items={items}
        onTransition={(to) =>
          to === "CHECKOUT" ? setCheckout(e) : move(e, to)
        }
                ...
                {equipment.loading ? (
                  ...
                  (byStatus[col] || []).map((e) => (
                    ...
                    >
                      ...
                      <div className="mt-0.5 micro">
                        {e.status === "IN_USE" && e.assigned_to
                          ? userMap[e.assigned_to] || "assigned"
                          : slot(e) || "—"}
                      </div>
