...
describe("campaign send fan-out", () => {
  ...
  test("queues one email per active subscriber, using the template's sender as From", async () => {
    ...
    repo.getTemplate.mockResolvedValue({
      template_id: "t1",
      subject: "Hello",
      body_html: "<p>hi</p>",
      from_sender_id: "s1",
    });
    repo.getSender.mockResolvedValue({
      sender_id: "s1",
      from_name: "Praxis LS",
      from_address: "news@ex.cm",
    });
    ...
    const res = await service.sendCampaign(client, {
      id: "c1",
      templateId: "t1",
      tenantMeta: { slug: "t" },
      env: "live",
  ...
  test("no sender → no From override; subject falls back to the campaign name", async () => {
    ...
    await service.sendCampaign(client, {
      id: "c1",
      templateId: "t1",
      tenantMeta: {},
      env: "live",
  ...
  test("ENDED campaign is rejected and queues nothing", async () => {
    ...
    await expect(
      service.sendCampaign(client, { id: "c1", templateId: "t1" }),
    ).rejects.toMatchObject({ code: "CAMPAIGN_ENDED" });
  ...
  test("missing campaign → NOT_FOUND", async () => {
    ...
    await expect(
      service.sendCampaign(client, { id: "x", templateId: "t1" }),
  ...
  test("unknown template → BAD_TEMPLATE", async () => {
    ...
    await expect(
      service.sendCampaign(client, { id: "c1", templateId: "bad" }),
    ).rejects.toMatchObject({ code: "BAD_TEMPLATE" });
    expect(enqueue).not.toHaveBeenCalled();
  });
  ...
  test("merge fields are substituted per recipient in subject and body", async () => {
    ...
    repo.getTemplate.mockResolvedValue({
      template_id: "t1",
      subject: "Hi {{name}}",
      body_html: "<p>Hello {{name}} ({{email}}) — {{campaign}}</p>",
      from_sender_id: null,
    });
    repo.listActiveSubscriberEmails.mockResolvedValue([
  ...
  test("merge values are HTML-escaped in the body but not in the subject", async () => {
    ...
    repo.listActiveSubscriberEmails.mockResolvedValue([
      { email: "x@ex.cm", name: "<script>alert(1)</script>" },
    ]);
