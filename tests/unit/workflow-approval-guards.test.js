 ...
 */
const {
  stepApplies, applicableSteps, act, assertEligible, KIND_ACTIONS,
} = require("../../src/services/workflow/executor");
const scopeService = require("../../src/modules/security/scope/scope.service");

const step = (over = {}) => ({
  workflow_step_id: "s1", step_seq: 1, step_kind: "APPROVE",
  min_amount_xaf: null, max_amount_xaf: null, role_id: null, scope_id: null, ...over,
});
...
describe("unknown amount routing (W10)", () => {
  const banded = [
    step({ workflow_step_id: "s1", step_seq: 1, min_amount_xaf: 1, max_amount_xaf: 1000000 }),
    step({ workflow_step_id: "s2", step_seq: 2, min_amount_xaf: 1000001, max_amount_xaf: null }),
  ];
  ...
  it("null amount applies to every step, so the full chain runs", () => {
    ...
    expect(applicableSteps(banded, null).map((s) => s.step_seq)).toEqual([1, 2]);
  });
  ...
  it("a real zero is still a zero and matches only bands that include it", () => {
    ...
    expect(stepApplies(step({ min_amount_xaf: 0, max_amount_xaf: 100 }), 0)).toBe(true);
  ...
  it("a known amount still routes by band", () => {
    ...
    expect(applicableSteps(banded, 5000000).map((s) => s.step_seq)).toEqual([2]);
  });
});
...
describe("action must match step_kind (W9)", () => {
  ...
  it("a VALIDATE step cannot be cleared with approve", async () => {
    ...
    await expect(act(c, { approvalTaskId: "t1", action: "approve", actor: { user_id: "u1" } }))
  ...
  it("an APPROVE step cannot be cleared with validate", async () => {
    ...
    await expect(act(c, { approvalTaskId: "t1", action: "validate", actor: { user_id: "u1" } }))
      .rejects.toMatchObject({ code: "WRONG_ACTION_FOR_STEP" });
  });

  it("reject is allowed on either kind", async () => {
    const r = await act(taskClient({ step_kind: "VALIDATE" }), {
      approvalTaskId: "t1", action: "reject", actor: { user_id: "u1" },
    });
...
describe("eligibility (W2/W6)", () => {
  const holder = { user_id: "u1", role_ids: ["role-fin"], scope_ids: ["sc-dla"] };

  it("an unrestricted step is open to anyone — the UI-built case today", () => {
    expect(() => assertEligible({ step_role_id: null, step_scope_id: null }, holder)).not.toThrow();
  });

  it("a role-bound step admits a holder of that role", () => {
    expect(() => assertEligible({ step_role_id: "role-fin", step_scope_id: null }, holder)).not.toThrow();
  });

  it("a role-bound step refuses someone who does not hold it", () => {
    expect(() => assertEligible({ step_role_id: "role-hr", step_scope_id: null }, holder))
  ...
  it("a scoped step admits someone whose closure contains that node", () => {
    ...
    expect(() => assertEligible({ step_role_id: null, step_scope_id: "sc-dla" }, holder)).not.toThrow();
  });

  it("a scoped step refuses someone outside that part of the organisation", () => {
    expect(() => assertEligible({ step_role_id: null, step_scope_id: "sc-ydé" }, holder))
  ...
  it("no scope assignment satisfies unscoped steps only — absence is not omnipresence", () => {
    ...
    expect(() => assertEligible({ step_role_id: null, step_scope_id: null }, unassigned)).not.toThrow();
    expect(() => assertEligible({ step_role_id: null, step_scope_id: "sc-dla" }, unassigned)).toThrow();
  });

  it("both gates must pass, not either", () => {
    expect(() => assertEligible({ step_role_id: "role-hr", step_scope_id: "sc-dla" }, holder)).toThrow();
    expect(() => assertEligible({ step_role_id: "role-fin", step_scope_id: "sc-ydé" }, holder)).toThrow();
    expect(() => assertEligible({ step_role_id: "role-fin", step_scope_id: "sc-dla" }, holder)).not.toThrow();
  });

  it("the CEO bypasses both, as with requirePermission", () => {
    const ceo = { user_id: "u0", role_ids: [], scope_ids: [], is_ceo: true };
    expect(() => assertEligible({ step_role_id: "role-hr", step_scope_id: "sc-ydé" }, ceo)).not.toThrow();
  });

  it("a missing role_ids/scope_ids array is treated as empty, not as a bypass", () => {
    expect(() => assertEligible({ step_role_id: "role-fin", step_scope_id: null }, { user_id: "u2" }))
      .toThrow(/role you do not hold/);
  });
});
...
describe("maker-checker (W5)", () => {
  ...
  it("allows a different person", async () => {
    const c = taskClient({}, { maker: "u1" });
    const r = await act(c, { approvalTaskId: "t1", action: "approve", actor: { user_id: "u2" } });
  ...
  it("is NOT waived for the CEO — segregation of duties has no bypass", async () => {
    const c = taskClient({}, { maker: "u1" });
    await expect(act(c, {
      approvalTaskId: "t1", action: "approve", actor: { user_id: "u1", is_ceo: true },
    })).rejects.toMatchObject({ code: "SELF_APPROVAL" });
  });
  ...
  it("an unattributable maker is an absence, not a bypass — the decision proceeds", async () => {
    ...
    const r = await act(c, { approvalTaskId: "t1", action: "approve", actor: { user_id: "u1" } });
    expect(r.completed).toBe(true);
  });
});
...
describe("scope cycle guard (A4)", () => {
  ...
  it("rejects a scope parented to itself", async () => {
    await expect(scopeService.assertNoCycle(treeClient({}), "a", "a"))
  ...
  it("rejects an indirect loop the FE dropdown cannot catch (A→B→A)", async () => {
    ...
    await expect(scopeService.assertNoCycle(treeClient({ b: "a" }), "a", "b"))
      .rejects.toMatchObject({ code: "SCOPE_CYCLE" });
  });

  it("rejects a deeper loop (A→B→C→A)", async () => {
    await expect(scopeService.assertNoCycle(treeClient({ c: "b", b: "a" }), "a", "c"))
      .rejects.toMatchObject({ code: "SCOPE_CYCLE" });
  });

  it("allows a legitimate re-parent", async () => {
    await expect(scopeService.assertNoCycle(treeClient({ hq: null }), "dla", "hq")).resolves.toBeUndefined();
  });

  it("no-ops for a root and for a create with no id yet", async () => {
    await expect(scopeService.assertNoCycle(treeClient({}), "a", null)).resolves.toBeUndefined();
    await expect(scopeService.assertNoCycle(treeClient({}), null, "hq")).resolves.toBeUndefined();
  });
});
...
function taskClient(stepOver = {}, { maker = null } = {}) {
  const task = {
    approval_task_id: "t1", status: "PENDING", entity_ref: "invoice:1", amount_xaf: 500,
    step_seq: 1, workflow_id: "w1", step_kind: "APPROVE",
    step_role_id: null, step_scope_id: null, step_capability_code: null, ...stepOver,
  };
  return {
    task,
    query: async (sql) => {
      if (/JOIN workflow_step ws ON/.test(sql)) return { rows: [task] };
      if (/FROM event_log/.test(sql)) return { rows: maker ? [{ actor_user_id: maker }] : [] };
