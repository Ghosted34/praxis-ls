-- ============================================================================
-- TENANT DB — 0497 record the `shell` preference section
--
-- 0496 built `user_preference` as (user_id, section, key) → jsonb precisely so
...
COMMENT ON TABLE user_preference IS
  'Per-user overrides of tenant-wide settings, keyed (user_id, section, key) to mirror the `setting` table (0496). section=''appearance'' holds font_display/font_body/font_mono; section=''shell'' holds ribbon_pinned/rail_pins/rail_hint_seen (the user''s chrome arrangement). A NULL/absent row means "no choice made" — inherit the tenant value for appearance, use the client''s starter set for the rail.';

-- DOWN
-- Restores 0496's wording. Nothing but the comment changes either way, so this
