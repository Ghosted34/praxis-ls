...
const { scoreCandidate, rankCandidates, normalizeInput, jaccard, normReg } = require("../../src/modules/master/_shared/dedup.service");

const cand = (over) => ({ id: "x", name: "X", ref: "R", status: "ACTIVE", name_norm: "", regNumbers: [], email_norm: null, phone_digits: null, bank_last4: [], ...over });

describe("normalizeInput", () => {
  it("normalizes name, email, phone, and registration numbers", () => {
    const n = normalizeInput({ legal_name: "  Bolloré   Transport  SA ", email: "AP@Bollore.CM", phone: "+237 6 99 00 11 22", registrations: [{ kind: "NIU", number: "p 012-345/678" }] });
  ...
  it("keeps only 4-digit bank tails and drops the rest", () => {
    expect(normalizeInput({ bank_last4: ["1234", "00007890", "12"] }).bank_last4).toEqual(["1234", "7890"]);
  });
});
...
describe("scoreCandidate", () => {
  const input = normalizeInput({ legal_name: "Bolloré Transport SA", email: "ap@bollore.cm", phone: "237699001122", registrations: [{ kind: "NIU", number: "P0123456789A" }] });
  ...
  it("scores an identical normalized name, and a fuzzy token-set name", () => {
    expect(scoreCandidate(input, cand({ name_norm: "bolloré transport sa" })).score).toBe(40);
    const fuzzy = scoreCandidate(input, cand({ name_norm: "bolloré transport" }));
  ...
  it("adds email and phone matches", () => {
    expect(scoreCandidate(input, cand({ email_norm: "ap@bollore.cm" })).reasons).toContain("Same email");
    expect(scoreCandidate(input, cand({ phone_digits: "237699001122" })).reasons).toContain("Same phone");
  });

  it("does NOT match an unrelated candidate (no false positive)", () => {
    expect(scoreCandidate(input, cand({ name_norm: "acme water supplies", email_norm: "x@acme.cm" })).score).toBe(0);
  });

  it("compares bank last-4 server-side but never leaks the number", () => {
    const withBank = normalizeInput({ legal_name: "X", bank_last4: ["1234"] });
    const r = scoreCandidate(withBank, cand({ name_norm: "x", bank_last4: ["1234"] }));
    expect(r.reasons).toContain("Matching bank account");
  });
});

describe("rankCandidates", () => {
  const input = normalizeInput({ legal_name: "Bolloré Transport SA", registrations: [{ kind: "NIU", number: "P0123456789A" }] });

  it("keeps only >= min, sorts by score desc, caps, and returns ONLY safe fields", () => {
    const candidates = [
      cand({ id: "a", name: "Bolloré Transport SA", ref: "S-1", name_norm: "bolloré transport sa", regNumbers: ["P0123456789A"], email_norm: "leak@x.cm", bank_last4: ["9999"] }),
    ...
    expect(Object.keys(out[0]).sort()).toEqual(["id", "name", "reasons", "ref", "score", "status"]);
  ...
  it("returns nothing when no candidate crosses the threshold", () => {
    expect(rankCandidates(input, [cand({ name_norm: "totally different" })])).toEqual([]);
  });
});
