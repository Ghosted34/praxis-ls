 ...
 * schema and the invariants added by migrations 13930/13931/13932. Skipped
 ...
 *   - the single-base invariant is enforced (13930's partial unique index);
 ...
 *     index-covered (13932), so the scan plans as index scans not seq scans;
...
d("currency & FX lifecycle (real Postgres)", () => {
  ...
  it("every currency-FK usage column is index-covered (13932)", async () => {
    ...
    for (const c of cols) {
      ...
      expect(idx.length).toBeGreaterThanOrEqual(1); // covered by 13932 (or a prior index)
    }
  });
