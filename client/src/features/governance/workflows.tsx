...
function StepForm({
  ...
  const [f, setF] = React.useState({
    step_seq: String(nextSeq),
    step_kind: "APPROVE",
    capability_code: "APPROVER",
    role_id: "",
    scope_id: "",
    min_amount_xaf: "",
    max_amount_xaf: "",
  ...
  const scopeTree = React.useMemo(
    () => buildScopeTree(scopes.data || []),
    [scopes.data],
  );
  ...
  async function submit(e: React.FormEvent) {
    ...
    try {
      await wf.addStep(workflowId, {
        step_seq: Number(f.step_seq),
        step_kind: f.step_kind as "VALIDATE" | "APPROVE",
        capability_code: f.capability_code as "VALIDATOR" | "APPROVER",
        role_id: f.role_id || undefined,
        scope_id: f.scope_id || undefined,
        min_amount_xaf:
          f.min_amount_xaf === "" ? undefined : Number(f.min_amount_xaf),
        max_amount_xaf:
          f.max_amount_xaf === "" ? undefined : Number(f.max_amount_xaf),
      });
      onSaved();
      onClose();
    } catch (err) {
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          ...
          <Field label="Kind" required>
            <Select
              value={f.step_kind}
              onChange={(e) => {
                set("step_kind", e.target.value);
                set(
                  "capability_code",
                  e.target.value === "VALIDATE" ? "VALIDATOR" : "APPROVER",
                );
              }}
            >
            ...
            >
              ...
              {flatScopes.map((s) => (
                <option
                  key={s.scope_id}
                  value={s.scope_id}
                >{`${"  ".repeat(s.depth)}${s.code} · ${s.name}`}</option>
              ))}
            </Select>
          </Field>
          <Field
...
function WorkflowDrawer({
  ...
  const steps = useResource(
    () => wf.listSteps(workflow.workflow_id),
    [workflow.workflow_id],
  );
  const [adding, setAdding] = React.useState(false);
  const [busy, setBusy] = React.useState<string | null>(null);
  const chain = (steps.data || [])
    .slice()
  ...
  const nextSeq = chain.length
    ? Math.max(...chain.map((s) => s.step_seq)) + 1
    : 1;

  async function remove(s: wf.WorkflowStep) {
    setBusy(s.workflow_step_id);
    try {
      await wf.removeStep(workflow.workflow_id, s.workflow_step_id);
      steps.reload();
      onChanged();
    } catch (e) {
      reportActionError(e);
  ...
  return (
    <Modal
      ...
      title={workflow.name}
      description={
        workflow.event_type_key
          ? `On event: ${workflow.event_type_key}`
          : undefined
      }
    >
      <div className="space-y-4">
        ...
        {steps.loading ? (
          ...
          <ol className="space-y-2">
            {chain.map((s) => (
              <li
                key={s.workflow_step_id}
                className="flex items-center justify-between rounded-lg border border-border px-3 py-2"
              >
                <span className="flex flex-wrap items-center gap-x-3 gap-y-1">
                  <span className="grid h-6 w-6 place-items-center rounded-full bg-primary/15 text-xs font-semibold text-primary-ink">
                    {s.step_seq}
                  </span>
                  <Pill tone={s.step_kind === "VALIDATE" ? "blue" : "ok"}>
                    {s.step_kind}
                  </Pill>
                  {/* An unbound step is open to anyone — say so plainly rather than
                      showing a capability that reads like a restriction. */}
                  <span className="text-sm">
                    {roleName(s.role_id) || "Anyone"}
                  </span>
                  <span className="micro">
                    · {scopeName(s.scope_id) || "company-wide"}
                  </span>
                  {s.capability_code && (
                    <span className="micro">· {s.capability_code}</span>
                  )}
                ...
                <Button
                  size="sm"
                  variant="ghost"
                  loading={busy === s.workflow_step_id}
                  onClick={() => remove(s)}
                >
                  Remove
                </Button>
...
function WorkflowForm({
  ...
  async function submit(e: React.FormEvent) {
    ...
    setError(null);
    try {
      await wf.createWorkflow({
        name: f.name,
        event_type_key: f.event_type_key,
      });
      onSaved();
      onClose();
    } catch (err) {
      ...
      <form className="space-y-4" onSubmit={submit}>
        ...
        <div className="flex justify-end gap-2 pt-2">
          ...
          <Button
            type="submit"
            loading={busy}
            disabled={!f.name || !f.event_type_key || busy}
          >
            Create workflow
          </Button>
        </div>
...
export function WorkflowsPage() {
  ...
  async function toggleActive(w: wf.Workflow) {
    ...
    try {
      await wf.updateWorkflow(w.workflow_id, { is_active: !w.is_active });
      reload();
    } catch (e) {
      reportActionError(e);
    } finally {
  ...
  const columns: Column<wf.Workflow>[] = [
    ...
    },
    {
      ...
      render: (w) => (
        <RowActions>
          <Button size="sm" variant="outline" onClick={() => setView(w)}>
            Edit chain
          </Button>
        </RowActions>
      ),
  ...
  return (
    <section className={pageShell.wide}>
      ...
      <DataList
        ...
        onRowClick={(w) => setView(w)}
        empty={{
          title: "No workflows",
          hint: "Create a chain to route approvals for an event.",
        }}
      />
      {creating && (
        <WorkflowForm onClose={() => setCreating(false)} onSaved={reload} />
      )}
      {view && (
        <WorkflowDrawer
          workflow={view}
