  ...
  async function submit(e: React.FormEvent) {
    ...
    try {
      const items = itemsText
        ...
        .map((s) => s.trim())
        .filter(Boolean);
      await tenant("/onboarding", {
        method: "POST",
        body: { employee_id: employeeId || undefined, items },
      });
      onSaved();
      onClose();
    } catch (err) {
    ...
    >
      <form className="space-y-4" onSubmit={submit}>
        ...
        <Field
          ...
          hint="Optional — you can add more later."
        >
          <Textarea
            value={itemsText}
...
function OnboardingDetail({
  ...
  const detail = useResource(
    () => tenant<ChecklistDetail>(`/onboarding/${id}`),
    [id],
  );
  ...
  async function toggle(item: OnbItem) {
    ...
    try {
      await tenant(`/onboarding/items/${item.onboarding_item_id}`, {
        method: "PATCH",
  ...
  async function add(e: React.FormEvent) {
    ...
    setBusy("add");
    try {
      await tenant(`/onboarding/${id}/items`, {
        method: "POST",
        body: { label: newItem.trim() },
      });
      setNewItem("");
      detail.reload();
      onChanged();
  ...
  async function complete() {
    setBusy("complete");
    try {
      await tenant(`/onboarding/${id}/complete`, { method: "POST" });
      detail.reload();
      onChanged();
    } catch (e) {
      reportActionError(e);
  ...
  return (
    <Modal
      ...
      title={
        c?.employee_name
          ? `Onboarding · ${c.employee_name}`
          : "Onboarding checklist"
      }
      description={c ? `${done}/${(c.items || []).length} done` : undefined}
    >
      {detail.error ? (
    ...
    <>
      ...
      {board.error ? (
        ...
        <EmptyState
          ...
          hint="Start a checklist for a new hire."
        />
      ) : (
        <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
          {list.map((c) => {
            const pct = c.total_items
              ? Math.round((c.done_items / c.total_items) * 100)
              : 0;
            return (
              <button
