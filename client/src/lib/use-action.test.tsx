 ...
 */
function harness<A extends unknown[], R>(
  fn: (...a: A) => Promise<R>,
  opts: Parameters<typeof useAction<A, R>>[1] = {},
) {
  const handle: { current: ReturnType<typeof useAction<A, R>> | null } = {
    current: null,
  };
  function Probe() {
...
describe("useAction — resolved calls", () => {
  ...
  it("changed:false fires idle toast (info role, not error)", async () => {
    ...
    const fn = vi.fn(async () => ({
      ok: true as const,
      changed: false,
      message: "Server said already revoked",
    }));
    const h = harness(fn, {
...
describe("useAction — expected failures are NEVER reported", () => {
  ...
  it("422 is not reported", async () => {
    const report = vi
      ...
      .mockImplementation(() => {});
    const fn = vi.fn(async () => {
      throw new ApiError("VALIDATION_ERROR", "bad", 422, { email: "required" });
    });
    const h = harness(fn, { success: "…" });
    await rtlAct(async () => {
      await h.current!.run();
  ...
  it("CONFIG_MISSING (424) is not reported — it is unfinished setup", async () => {
    const report = vi
      ...
      .mockImplementation(() => {});
    const fn = vi.fn(async () => {
      throw new ApiError("CONFIG_MISSING", "SMTP not set", 424);
    });
    const h = harness(fn, { success: "…" });
    await rtlAct(async () => {
      await h.current!.run();
  ...
  it("NETWORK_DOWN is not reported — the network dropped, that is not our fault", async () => {
    const report = vi
      ...
      .mockImplementation(() => {});
    const fn = vi.fn(async () => {
      throw new ApiError(NETWORK_DOWN, "offline", 0);
    });
    const h = harness(fn, { success: "…" });
    await rtlAct(async () => {
      await h.current!.run();
...
describe("useAction — mechanics", () => {
  it("busy toggles true during, false after", async () => {
    ...
    rtlAct(() => {
      pending = h.current!.run();
    });
    await waitFor(() => expect(h.current!.busy).toBe(true));
    rtlAct(() => {
      resolve({ ok: true, changed: true });
    });
    await rtlAct(async () => {
      await pending;
    });
