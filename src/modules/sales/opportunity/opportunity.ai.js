...
module.exports = {
  ...
  reads: [
    { key: "list_opportunities", service: (c, p) => service.list(c, p), describe: "List sales opportunities (filter stage/status/owner/source/date/search)." },
    { key: "pipeline_board", service: (c, p) => service.board(c, p), describe: "Kanban board: open opportunities + value/weighted value per stage." },
    { key: "pipeline_metrics", service: (c, p) => service.metrics(c, p), describe: "Pipeline KPIs: open pipeline value, weighted forecast, won value, and BOTH win rates — win_rate_settled (won / won+lost, the headline) and win_rate_all_deals (won / every deal, the legacy denominator). Null means no deals in the denominator, which is not the same as 0%." },
